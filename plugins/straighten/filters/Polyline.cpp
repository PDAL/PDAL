...
Polyline::Polyline(OGRGeometryH g) : Geometry(g) { init(); }
...
void Polyline::init() {
  ...
  if (!m_geom) {
    OGRGeometry *newGeom;
    OGRGeometryFactory::createFromWkt("LINESTRING ZM EMPTY", nullptr, &newGeom);
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (i + 1 != size)
      m_view->setField(Dimension::Id::Azimuth, i,
                       Utils::azimuth(p->getX(i), p->getY(i), p->getX(i + 1),
                                      p->getY(i + 1)));
    else
      m_view->setField(Dimension::Id::Azimuth, i,
...
Polyline::Polyline(const Polyline &poly) : Geometry(poly) { init(); }
...
void Polyline::interpolate(const PointRef &point, double &x, double &y,
  ...
  const double currentM = currentPoint.getFieldAs<double>(Dimension::Id::Roll);
...
double Polyline::closestSegment(const PointRef &point, double &x, double &y,
  ...
  for (int i = 1; i < size; ++i) {
    ...
    if (testDist < sqrDist) {
      ...
      offset = 0.0;
      // list[i-1] if the first point of the current segment in the polyline
      // list[i] = list[i-1]+1
      // this is where we stop accumulating distances
      for (PointId j = 1; j < list[i]; ++j) {
        const double dx = m_view->getFieldAs<double>(Dimension::Id::X, j) -
                          m_view->getFieldAs<double>(Dimension::Id::X, j - 1);
        const double dy = m_view->getFieldAs<double>(Dimension::Id::Y, j) -
      ...
      const double tx = currentX - prevX;
      const double ty = currentY - prevY;
      const double ratio = std::sqrt((dx * dx + dy * dy) / (tx * tx + ty * ty));
      offset += std::sqrt((dx * dx + dy * dy));
      ...
      azimuth = Utils::angularRatio(
          ...
          m_view->getFieldAs<double>(Dimension::Id::Azimuth, list[i]), ratio);
      m = Utils::angularRatio(
          m_view->getFieldAs<double>(Dimension::Id::Roll, list[i - 1]),
          m_view->getFieldAs<double>(Dimension::Id::Roll, list[i]), ratio);
    }
  }
  return sqrDist;
}
