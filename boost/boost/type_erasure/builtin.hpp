...
struct copy_constructible :
    ::pdalboost::mpl::vector<constructible<T(const T&)>, destructible<T> >
{};
...
struct concept_interface<assignable<T, U>, Base, T> : Base
{
    ...
    assignable<T, U>* _boost_type_erasure_deduce_assign(
        typename ::pdalboost::type_erasure::rebind_any<Base, const U&>::type)
    {
        return 0;
    }
};
...
template<class T>
struct get_null_vtable_entry< ::pdalboost::type_erasure::typeid_<T> >
{
    typedef typeid_<void> type;
};

struct null_destroy {
    static void value(::pdalboost::type_erasure::detail::storage&) {}
};

template<class T>
struct get_null_vtable_entry< ::pdalboost::type_erasure::destructible<T> >
{
    typedef ::pdalboost::type_erasure::detail::null_destroy type;
};
