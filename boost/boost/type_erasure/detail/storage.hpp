...
{
    return *static_cast<typename ::pdalboost::remove_reference<T>::type*>(arg.data);
}

template<class T>
T extract(const storage& arg)
{
    return *static_cast<const typename ::pdalboost::remove_reference<T>::type*>(arg.data);
}
...
T extract(storage&& arg)
{
    return std::move(*static_cast<typename ::pdalboost::remove_reference<T>::type*>(arg.data));
}
