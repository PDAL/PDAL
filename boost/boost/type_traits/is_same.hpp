...
struct is_same_impl
{
    enum { value = pdalboost::detail::is_same_part_1<T1>::template part_2<T2>::value };
};

#else // generic "no-partial-specialization" version

template <typename T>
::pdalboost::type_traits::yes_type
BOOST_TT_DECL is_same_tester(T*, T*);

::pdalboost::type_traits::no_type
...
struct is_same_impl
{
   ...
   BOOST_STATIC_CONSTANT(bool, value =
      (::pdalboost::type_traits::ice_and<
         (sizeof(type_traits::yes_type) == sizeof(pdalboost::detail::is_same_tester(&t,&u))),
         (::pdalboost::is_reference<T>::value == ::pdalboost::is_reference<U>::value),
         (sizeof(T) == sizeof(U))
        >::value));
};
...
BOOST_TT_AUX_BOOL_TRAIT_DEF2(is_same,T,U,(::pdalboost::detail::is_same_impl<T,U>::value))
