...
#define BOOST_MP_USING_ABS using pdalboost::multiprecision::detail::abs;
...
template <class S>
void format_float_string(S& str, pdalboost::intmax_t my_exp, pdalboost::intmax_t digits, std::ios_base::fmtflags f, bool iszero)
{
   ...
   else if(!fixed || (my_exp >= 0))
   {
      ...
      //
      pdalboost::intmax_t chars = str.size();
      chars = digits - chars;
      if(scientific)
         ++chars;
      if(chars > 0)
   ...
   {
      if(1 + my_exp > static_cast<pdalboost::intmax_t>(str.size()))
      {
      ...
      else if(my_exp + 1 < static_cast<pdalboost::intmax_t>(str.size()))
      {
         if(my_exp < 0)
         {
      ...
      if(fixed)
      {
         ...
         pdalboost::intmax_t l = str.find('.') + 1;
         l = digits - (str.size() - l);
         if(l > 0)
   ...
   else
   {
      ...
      if(showpoint || (str.size() > 1))
         str.insert(1, 1, '.');
      str.append(1, 'e');
      S e = pdalboost::lexical_cast<S>(abs(my_exp));
...
template <class tag, class A1, class A2, class A3, class A4>
struct promote_arg<pdalboost::multiprecision::detail::expression<tag, A1, A2, A3, A4> >
{
   typedef typename pdalboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type type;
};

template <class R, class B, pdalboost::multiprecision::expression_template_option ET>
inline R real_cast(const pdalboost::multiprecision::number<B, ET>& val)
{
   return val.template convert_to<R>();
}

template <class R, class tag, class A1, class A2, class A3, class A4>
inline R real_cast(const pdalboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>& val)
{
   typedef typename pdalboost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type val_type;
