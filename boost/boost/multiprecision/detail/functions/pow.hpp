...
{
   detail::pow_imp(result, t, p, pdalboost::is_signed<U>());
}
...
void hyp0F0(T& H0F0, const T& x)
{
   ...
   BOOST_ASSERT(&H0F0 != &x);
   long tol = pdalboost::multiprecision::detail::digits2<number<T, et_on> >::value;
   T t;
...
void eval_exp(T& result, const T& x)
{
   ...
   typedef typename T::exponent_type exp_type;
   typedef typename pdalboost::multiprecision::detail::canonical<exp_type, T>::type canonical_exp_type;
   typedef typename pdalboost::multiprecision::detail::canonical<float, T>::type float_type;
   ...
   typename pdalboost::multiprecision::detail::canonical<pdalboost::intmax_t, T>::type ll;
...
inline void eval_pow(T& result, const T& x, const T& a)
{
   ...
   typename pdalboost::multiprecision::detail::canonical<pdalboost::intmax_t, T>::type an;
   T fa;
   try
   {
   ...
   catch(const std::exception&)
   {
      ...
      an = (std::numeric_limits<pdalboost::intmax_t>::max)();
   }
...
namespace detail{
   ...
   void small_sinh_series(T x, T& result)
   {
      ...
      T lim(x);
      eval_ldexp(lim, lim, 1 - pdalboost::multiprecision::detail::digits2<number<T, et_on> >::value);
