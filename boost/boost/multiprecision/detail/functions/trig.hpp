...
{
   typedef typename pdalboost::multiprecision::detail::canonical<pdalboost::int32_t, T>::type si_type;
   typedef typename pdalboost::multiprecision::detail::canonical<pdalboost::uint32_t, T>::type ui_type;
   ...
   T tol;
   tol = ui_type(1);
   eval_ldexp(tol, tol, 1 - pdalboost::multiprecision::detail::digits2<number<T, et_on> >::value);
...
void eval_sin(T& result, const T& x)
{
   ...
   else
   {
      ...
      for(pdalboost::int32_t k = static_cast<pdalboost::int32_t>(0); k < n_scale; k++)
      {
...
void hyp2F1(T& result, const T& a, const T& b, const T& c, const T& x)
{
  ...
  // There are no checks on input range or parameter boundaries.
   ...
   T lim;
   eval_ldexp(lim, result, 1 - pdalboost::multiprecision::detail::digits2<number<T, et_on> >::value);
...
void eval_atan(T& result, const T& x)
{
   ...
   T s, c, t;
   for(pdalboost::int32_t digits = double_digits10_minus_a_few; digits <= std::numeric_limits<number<T, et_on> >::digits10; digits *= 2)
   {
      eval_sin(s, result);
      eval_cos(c, result);
