...
public:
   ...
   {
      m_limbs = static_cast<pdalboost::uint16_t>((std::min)(new_size, internal_limb_count));
...
#endif
//
// Traits classes to figure out a native type with N bits, these vary from pdalboost::uint_t<N> only
...
{
   typedef typename pdalboost::uint_t<N>::least type;
};
...
protected:
   template <class T>
   typename pdalboost::disable_if_c<std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= (int)MinBits)>::type
...
public:
   ...
   //
   template <class SI>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(SI i, typename pdalboost::enable_if_c<is_signed<SI>::value && (Checked == unchecked) >::type const* = 0) BOOST_NOEXCEPT_IF((Checked == unchecked))
   ...
   template <class SI>
   BOOST_MP_FORCEINLINE cpp_int_base(SI i, typename pdalboost::enable_if_c<is_signed<SI>::value && (Checked == checked) >::type const* = 0) BOOST_NOEXCEPT_IF((Checked == unchecked))
   ...
   template <class UI>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(UI i, typename pdalboost::enable_if_c<is_unsigned<UI>::value && (Checked == unchecked)>::type const* = 0) BOOST_NOEXCEPT_IF((Checked == unchecked))
      : m_data(static_cast<local_limb_type>(i) & limb_mask), m_sign(false) {}
   template <class UI>
   BOOST_MP_FORCEINLINE cpp_int_base(UI i, typename pdalboost::enable_if_c<is_unsigned<UI>::value && (Checked == checked)>::type const* = 0) BOOST_NOEXCEPT_IF((Checked == unchecked))
      : m_data(static_cast<local_limb_type>(i) & limb_mask), m_sign(false) { check_in_range(i); }
   template <class F>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(F i, typename pdalboost::enable_if_c<is_floating_point<F>::value && (Checked == unchecked)>::type const* = 0) BOOST_NOEXCEPT_IF((Checked == unchecked))
      : m_data(static_cast<local_limb_type>(std::fabs(i)) & limb_mask), m_sign(i < 0) {}
   template <class F>
   BOOST_MP_FORCEINLINE cpp_int_base(F i, typename pdalboost::enable_if_c<is_floating_point<F>::value && (Checked == checked)>::type const* = 0) BOOST_NOEXCEPT_IF((Checked == unchecked))
...
public:
   ...
   template <class F>
   BOOST_MP_FORCEINLINE cpp_int_base(F i, typename pdalboost::enable_if<is_floating_point<F> >::type const* = 0) BOOST_NOEXCEPT_IF((Checked == unchecked))
...
public:
   ...
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(const cpp_int_backend& o) BOOST_NOEXCEPT_IF(pdalboost::is_void<Allocator>::value) : base_type(o) {}
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
   ...
   //
   template <class Arg>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(Arg i, typename pdalboost::enable_if_c<is_allowed_cpp_int_base_conversion<Arg, base_type>::value >::type const* = 0)BOOST_NOEXCEPT_IF((Checked == unchecked) && pdalboost::is_void<Allocator>::value)
      : base_type(i) {}
...
private:
   ...
   void do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other, mpl::false_ const&, mpl::true_ const &)
   {
       ...
       *this = static_cast<
            typename pdalboost::multiprecision::detail::canonical<
...
public:
   ...
   cpp_int_backend(
      ...
      typename pdalboost::enable_if_c<is_implicit_cpp_int_conversion<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>, self_type>::value>::type* = 0)
      : base_type()
   {
      do_assign(
         other,
   ...
   explicit cpp_int_backend(
      const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other,
      typename pdalboost::disable_if_c<is_implicit_cpp_int_conversion<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>, self_type>::value>::type* = 0)
      : base_type()
   {
      do_assign(
         other,
   ...
   BOOST_MP_FORCEINLINE cpp_int_backend& operator = (const cpp_int_backend& o) BOOST_NOEXCEPT_IF((Checked == unchecked) && pdalboost::is_void<Allocator>::value)
   {
      this->assign(o);
      return *this;
   }
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
   BOOST_MP_FORCEINLINE cpp_int_backend& operator = (cpp_int_backend&& o) BOOST_NOEXCEPT_IF(pdalboost::is_void<Allocator>::value)
   {
...
private:
   template <class A>
   typename pdalboost::enable_if<is_unsigned<A> >::type do_assign_arithmetic(A val, const mpl::true_&)
   {
   ...
   template <class A>
   typename pdalboost::disable_if<is_unsigned<A> >::type do_assign_arithmetic(A val, const mpl::true_&)
   {
...
private:
   ...
   std::string do_get_string(std::ios_base::fmtflags f, const mpl::false_&)const
   {
      ...
      else
      {
         ...
         if(this->size() == 1)
         {
            result = pdalboost::lexical_cast<std::string>(t.limbs()[0]);
         }
         else
         {
...
public:
   ...
   template <class Arithmetic>
   BOOST_MP_FORCEINLINE typename pdalboost::enable_if<is_arithmetic<Arithmetic>, int>::type compare(Arithmetic i)const
   {
      // braindead version:
      cpp_int_backend t;
      t = i;
...
using pdalboost::multiprecision::backends::cpp_int_backend;
