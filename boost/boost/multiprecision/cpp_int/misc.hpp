...
{
   typedef typename pdalboost::multiprecision::detail::canonical<R, CppInt>::type cast_type;
   if(val.sign())
   {
   ...
   eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& backend) BOOST_NOEXCEPT_IF((is_non_throwing_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value))
{
   ...
   *result = static_cast<R>(backend.limbs()[0]);
   ...
   if(backend.sign())
   {
      check_is_negative(pdalboost::is_signed<R>());
      *result = negate_integer(*result, pdalboost::is_signed<R>());
   }
}
   ...
   eval_lsb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   //
   unsigned result = pdalboost::multiprecision::detail::find_lsb(a.limbs()[index]);
   ...
   eval_msb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   //
   return (a.size() - 1) * cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::limb_bits + pdalboost::multiprecision::detail::find_msb(a.limbs()[a.size() - 1]);
}
...
inline limb_type integer_gcd_reduce(limb_type u, limb_type v)
{
   do
   {
      ...
      if(u == v)
         break;
      v -= u;
      v >>= pdalboost::multiprecision::detail::find_lsb(v);
   } while(true);
   return u;
}
      ...
      limb_type v)
{
   ...
   unsigned vs = pdalboost::multiprecision::detail::find_lsb(v);
   shift = (std::min)(us, vs);
...
{
   *result.limbs() = pdalboost::math::gcd(*a.limbs(), *b.limbs());
}
...
{
   *result.limbs() = pdalboost::math::lcm(*a.limbs(), *b.limbs());
   ...
   eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& val)
{
   ...
   if(val.isneg())
   {
      check_is_negative(mpl::bool_<pdalboost::is_signed<R>::value || pdalboost::is_floating_point<R>::value>());
      *result = negate_integer(*result, mpl::bool_<pdalboost::is_signed<R>::value || pdalboost::is_floating_point<R>::value>());
   }
}
   ...
   eval_lsb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   // Find the index of the least significant bit within that limb:
   //
   return pdalboost::multiprecision::detail::find_lsb(*a.limbs());
}
   ...
   eval_msb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   // Find the index of the least significant bit within that limb:
   //
   return pdalboost::multiprecision::detail::find_msb(*a.limbs());
}
