...
*/
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost { namespace polygon { namespace detail {
...
struct minkowski_offset {
  ...
  polygon_set_data<T>::resize(coordinate_type resizing, bool corner_fill_arc, unsigned int num_circle_segments) {
    using namespace ::pdalboost::polygon::operators;
    if(!corner_fill_arc) {
      if(resizing < 0)
    ...
    extents(rect);
    if(resizing < 0) {
      ::pdalboost::polygon::bloat(rect, 10);
