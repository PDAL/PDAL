...
class rbtree_best_fit
{
   ...
   typedef typename pdalboost::intrusive::pointer_traits<char_ptr>::difference_type difference_type;
   typedef typename pdalboost::make_unsigned<difference_type>::type     size_type;
   ...
   {
      //-----------------------
      pdalboost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
   ...
   template<class T>
   std::pair<T *, bool>
      allocation_command  (pdalboost::interprocess::allocation_type command,   size_type limit_size,
                           size_type preferred_size,size_type &received_size,
                           T *reuse_ptr = 0);

   std::pair<void *, bool>
     raw_allocation_command  (pdalboost::interprocess::allocation_type command,   size_type limit_object,
   ...
   std::pair<void*, bool>
      priv_allocation_command(pdalboost::interprocess::allocation_type command,   size_type limit_size,
                        size_type preferred_size,size_type &received_size,
                        void *reuse_ptr, size_type sizeof_object);


   //!Real allocation algorithm with min allocation option
   std::pair<void *, bool> priv_allocate(pdalboost::interprocess::allocation_type command
   ...
   void* priv_expand_both_sides(pdalboost::interprocess::allocation_type command
                               ,size_type min_size
                               ,size_type preferred_size
                               ,size_type &received_size
                               ,void *reuse_ptr
   ...
   static const size_type Alignment = !MemAlignment
      ? size_type(::pdalboost::alignment_of< ::pdalboost::detail::max_align>::value)
   ...
   BOOST_STATIC_ASSERT((Alignment >= ::pdalboost::alignment_of<void_pointer>::value));
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::shrink_to_fit()
{
   ...
   if(priv_next_block(first_block) == old_end_block){
      ...
      size_type ignore;
      unique_buffer = priv_allocate(pdalboost::interprocess::allocate_new, 0, 0, ignore).first;
   ...
   allocate(size_type nbytes)
{
   ...
   //-----------------------
   size_type ignore;
   void * ret = priv_allocate(pdalboost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
   return ret;
}
                        ...
                        T *reuse_ptr)
{
   ...
   BOOST_ASSERT(0 == ((std::size_t)ret.first % ::pdalboost::alignment_of<T>::value));
...
inline std::pair<void*, bool> rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   raw_allocation_command  (pdalboost::interprocess::allocation_type command,   size_type limit_objects,
                        size_type preferred_objects,size_type &received_objects,
                        void *reuse_ptr, size_type sizeof_object)
{
   if(!sizeof_object)
      return std::pair<void *, bool>(static_cast<void*>(0), false);
   if(command & pdalboost::interprocess::try_shrink_in_place){
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_expand_both_sides(pdalboost::interprocess::allocation_type command
                         ,size_type min_size
                         ,size_type preferred_size
                         ,size_type &received_size
                         ,void *reuse_ptr
                         ...
                         ,size_type backwards_multiple)
{
   ...
   if(command & pdalboost::interprocess::expand_fwd){
   ...
   if(command & pdalboost::interprocess::expand_bwd){
      //Obtain the real size of the block
...
std::pair<void *, bool> rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_allocate(pdalboost::interprocess::allocation_type command
                ,size_type limit_size
                ,size_type preferred_size
                ,size_type &received_size
                ,void *reuse_ptr
               ,size_type backwards_multiple)
{
   ...
   //command &= (~pdalboost::interprocess::expand_bwd);

   if(command & pdalboost::interprocess::shrink_in_place){
      bool success =
   ...
   //Expand in place
   if(reuse_ptr && (command & (pdalboost::interprocess::expand_fwd | pdalboost::interprocess::expand_bwd))){
   ...
   if(command & pdalboost::interprocess::allocate_new){
