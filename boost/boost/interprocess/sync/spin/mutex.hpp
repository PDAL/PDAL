...
class spin_mutex
{
   ...
   void take_ownership(){};
   private:
   volatile pdalboost::uint32_t m_s;
};
...
inline void spin_mutex::lock(void)
{
   spin_wait swait;
   do{
      pdalboost::uint32_t prev_s = ipcdetail::atomic_cas32(const_cast<pdalboost::uint32_t*>(&m_s), 1, 0);
...
inline bool spin_mutex::timed_lock(const pdalboost::posix_time::ptime &abs_time)
{
...
{  ipcdetail::atomic_cas32(const_cast<pdalboost::uint32_t*>(&m_s), 0, 1);   }
