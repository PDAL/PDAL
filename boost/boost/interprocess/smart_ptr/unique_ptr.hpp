...
class unique_ptr
{
   ...
   unique_ptr(BOOST_RV_REF(unique_ptr) u)
      : ptr_(u.release(), pdalboost::forward<D>(u.get_deleter()))
   {}
   ...
   unique_ptr(BOOST_RV_REF_2_TEMPL_ARGS(unique_ptr, U, E) u,
      typename ipcdetail::enable_if_c<
            ...
            nat
            >::type = nat())
      : ptr_(const_cast<unique_ptr<U,E>&>(u).release(), pdalboost::move<D>(u.get_deleter()))
   {}
   ...
   {
      reset(u.release());
      ptr_.second() = pdalboost::move(u.get_deleter());
      return *this;
   }
   ...
   /// @cond
   private:
   pdalboost::compressed_pair<pointer, D> ptr_;
...
private:
    ...
    template <class U, class E> unique_ptr(U p, E,
        typename pdalboost::enable_if<pdalboost::is_convertible<U, pointer> >::type* = 0);
...
//!of type T with pdalboost::interprocess::deleter deleter
...
//!with pdalboost::interproces::deleter from a pointer
