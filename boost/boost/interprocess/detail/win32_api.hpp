...
{
   const unsigned long high_size(file_offset >> 32), low_size((pdalboost::uint32_t)file_offset);
...
class nt_query_mem_deleter
{
   ...
   static const std::size_t rename_suffix = (SystemTimeOfDayInfoLength + sizeof(unsigned long) + sizeof(pdalboost::uint32_t))*2;
...
inline bool unlink_file(const char *filename)
{
   ...
   try{
      ...
      handle_closer h_closer(fh);
      {
         ...
         static volatile pdalboost::uint32_t u32_count = 0;
         interlocked_decrement(reinterpret_cast<volatile long*>(&u32_count));
         buffer_to_wide_str(const_cast<const pdalboost::uint32_t *>(&u32_count), sizeof(pdalboost::uint32_t), &pfri->FileName[filename_string_length]);
         filename_string_length += sizeof(pdalboost::uint32_t)*2;
