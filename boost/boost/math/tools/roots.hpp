...
std::pair<T, T> bisect(F f, T min, T max, Tol tol, pdalboost::uintmax_t& max_iter, const Policy& pol)
{
   T fmin = f(min);
   T fmax = f(max);
   if(fmin == 0)
   ...
   // Error checking:
   //
   static const char* function = "pdalboost::math::tools::bisect<%1%>";
   if(min >= max)
   {
      policies::raise_evaluation_error(function, 
         "Arguments in wrong order in pdalboost::math::tools::bisect (first arg=%1%)", min, pol);
   }
   if(fmin * fmax >= 0)
   {
      policies::raise_evaluation_error(function, 
         "No change of sign in pdalboost::math::tools::bisect, either there is no root to find, or there are multiple roots in the interval (f(min) = %1%).", fmin, pol);
   }
   ...
   static pdalboost::uintmax_t max_count = 0;
...
inline std::pair<T, T> bisect(F f, T min, T max, Tol tol, pdalboost::uintmax_t& max_iter)
{
...
{
   pdalboost::uintmax_t m = (std::numeric_limits<pdalboost::uintmax_t>::max)();
   return bisect(f, min, max, tol, m, policies::policy<>());
}

template <class F, class T>
T newton_raphson_iterate(F f, T guess, T min, T max, int digits, pdalboost::uintmax_t& max_iter)
{
   ...
   do{
      ...
      delta2 = delta1;
      delta1 = delta;
      pdalboost::math::tie(f0, f1) = f(result);
      if(0 == f0)
         break;
      if(f1 == 0)
      {
...
template <class F, class T>
T halley_iterate(F f, T guess, T min, T max, int digits, pdalboost::uintmax_t& max_iter)
{
   BOOST_MATH_STD_USING
   ...
   do{
      ...
      delta2 = delta1;
      delta1 = delta;
      pdalboost::math::tie(f0, f1, f2) = f(result);
...
template <class F, class T>
T schroeder_iterate(F f, T guess, T min, T max, int digits, pdalboost::uintmax_t& max_iter)
{
   BOOST_MATH_STD_USING
