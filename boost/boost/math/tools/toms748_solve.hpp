...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost{ namespace math{ namespace tools{
...
void bracket(F f, T& a, T& b, T c, T& fa, T& fb, T& d, T& fd)
{
   ...
   //
   if(pdalboost::math::sign(fa) * pdalboost::math::sign(fc) < 0)
   {
      d = b;
      fd = fb;
      b = c;
                           ...
                           unsigned count)
{
   ...
   //
   T c;
   if(pdalboost::math::sign(A) * pdalboost::math::sign(fa) > 0)
   {
      c = a;
   }
   else
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, pdalboost::uintmax_t& max_iter, const Policy& pol)
{
   //
   ...
   static const char* function = "pdalboost::math::tools::toms748_solve<%1%>";

   pdalboost::uintmax_t count = max_iter;
   ...
   if(pdalboost::math::sign(fa) * pdalboost::math::sign(fb) > 0)
      policies::raise_domain_error(
         function, 
...
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, pdalboost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, fax, fbx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, pdalboost::uintmax_t& max_iter, const Policy& pol)
{
   max_iter -= 2;
...
template <class F, class T, class Tol>
inline std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, Tol tol, pdalboost::uintmax_t& max_iter)
{
   return toms748_solve(f, ax, bx, tol, max_iter, policies::policy<>());
}

template <class F, class T, class Tol, class Policy>
std::pair<T, T> bracket_and_solve_root(F f, const T& guess, T factor, bool rising, Tol tol, pdalboost::uintmax_t& max_iter, const Policy& pol)
{
   BOOST_MATH_STD_USING
   static const char* function = "pdalboost::math::tools::bracket_and_solve_root<%1%>";
   //
...
template <class F, class T, class Tol>
inline std::pair<T, T> bracket_and_solve_root(F f, const T& guess, const T& factor, bool rising, Tol tol, pdalboost::uintmax_t& max_iter)
{
