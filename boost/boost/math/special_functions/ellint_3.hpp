...
T ellint_pi_imp(T v, T phi, T k, T vc, const Policy& pol)
{
    ...
    static const char* function = "pdalboost::math::ellint_3<%1%>(%1%,%1%,%1%)";
    ...
    if(k == 0)
    {
       ...
       else
       {
          ...
          T vcr = sqrt(-vc);
          T arg = vcr * tan(phi);
          return (pdalboost::math::log1p(arg, pol) - pdalboost::math::log1p(-arg, pol)) / (2 * vcr);
       }
    }
    ...
    {
       T rphi = pdalboost::math::tools::fmod_workaround(T(fabs(phi)), T(constants::half_pi<T>()));
       T m = pdalboost::math::round((fabs(phi) - rphi) / constants::half_pi<T>());
       int sign = 1;
...
T ellint_pi_imp(T v, T k, T vc, const Policy& pol)
{
    ...
    static const char* function = "pdalboost::math::ellint_pi<%1%>(%1%,%1%)";
    ...
    if(v == 0)
    {
       return (k == 0) ? pdalboost::math::constants::pi<T>() / 2 : ellint_k_imp(k, pol);
    }
    ...
    if(v < 0)
    {
       ...
       T result = pdalboost::math::detail::ellint_pi_imp(N, k, Nm1, pol);
...
{
   return pdalboost::math::ellint_3(k, v, phi, policies::policy<>());
}
...
inline typename tools::promote_args<T1, T2>::type ellint_3(T1 k, T2 v, const Policy& pol, const mpl::true_&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_pi_imp(
         ...
         pol), "pdalboost::math::ellint_3<%1%>(%1%,%1%)");
}
...
inline typename tools::promote_args<T1, T2, T3>::type ellint_3(T1 k, T2 v, T3 phi, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_pi_imp(
         ...
         static_cast<value_type>(k),
         static_cast<value_type>(1-v),
         pol), "pdalboost::math::ellint_3<%1%>(%1%,%1%,%1%)");
}
