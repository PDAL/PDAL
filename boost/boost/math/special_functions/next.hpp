...
inline double float_next(const double& val, const Policy& pol)
{
   ...
   if(!(pdalboost::math::isfinite)(val) && (val > 0))
...
inline typename tools::promote_args<T, U>::type nextafter(const T& val, const U& direction, const Policy& pol)
{
   ...
   return val < direction ? pdalboost::math::float_next<result_type>(val, pol) : val == direction ? val : pdalboost::math::float_prior<result_type>(val, pol);
}
...
{
   return pdalboost::math::float_distance(a, b, policies::policy<>());
}
...
{
   return pdalboost::math::float_advance(val, distance, policies::policy<>());
}
