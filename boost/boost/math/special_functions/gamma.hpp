...
template <class T>
inline bool is_odd(T v, const pdalboost::true_type&)
{
   int i = static_cast<int>(v);
   return i&1;
}
template <class T>
inline bool is_odd(T v, const pdalboost::false_type&)
{
...
{
   return is_odd(v, ::pdalboost::is_convertible<T, int>());
}
...
T sinpx(T z)
{
   ...
   if(dist > 0.5)
      dist = 1 - dist;
   T result = sin(dist*pdalboost::math::constants::pi<T>());
...
T gamma_imp(T z, const Policy& pol, const Lanczos& l)
{
...
#endif
   static const char* function = "pdalboost::math::tgamma<%1%>(%1%)";
   ...
   if(z <= 0)
   {
      ...
      if(z <= -20)
      {
         ...
         if((fabs(result) < 1) && (tools::max_value<T>() * fabs(result) < pdalboost::math::constants::pi<T>()))
            return policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
         result = -pdalboost::math::constants::pi<T>() / result;
         if(result == 0)
            return policies::raise_underflow_error<T>(function, "Result of tgamma is too small to represent.", pol);
         if((pdalboost::math::fpclassify)(result) == (int)FP_SUBNORMAL)
   ...
   else
   {
      ...
      T zgh = (z + static_cast<T>(Lanczos::g()) - pdalboost::math::constants::half<T>());
      T lzgh = log(zgh);
      BOOST_MATH_INSTRUMENT_VARIABLE(result);
      ...
      {
         BOOST_MATH_INSTRUMENT_VARIABLE(zgh);
         BOOST_MATH_INSTRUMENT_VARIABLE(pow(zgh, z - pdalboost::math::constants::half<T>()));
         BOOST_MATH_INSTRUMENT_VARIABLE(exp(zgh));
         result *= pow(zgh, z - pdalboost::math::constants::half<T>()) / exp(zgh);
         BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
   }
   return result;
...
T lgamma_imp(T z, const Policy& pol, const Lanczos& l, int* sign = 0)
{
   ...
   static const char* function = "pdalboost::math::lgamma<%1%>(%1%)";
   ...
   if(z <= 0)
   {
      ...
      result = log(pdalboost::math::constants::pi<T>()) - lgamma_imp(z, pol, l) - log(t);
   }
   else if(z < 15)
   {
   ...
   {
      // regular evaluation:
      T zgh = static_cast<T>(z + Lanczos::g() - pdalboost::math::constants::half<T>());
      result = log(zgh) - 1;
      result *= z - 0.5f;
...
inline T upper_gamma_fraction(T a, T z, T eps)
{
   ...
   return 1 / (z - a + 1 + pdalboost::math::tools::continued_fraction_a(f, eps));
}
...
inline T lower_gamma_series(T a, T z, const Policy& pol, T init_value = 0)
{
   ...
   pdalboost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   T factor = policies::get_epsilon<T, Policy>();
   T result = pdalboost::math::tools::sum_series(s, factor, max_iter, init_value);
   policies::check_series_iterations<T>("pdalboost::math::detail::lower_gamma_series<%1%>(%1%)", max_iter, pol);
   return result;
}
...
T gamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   {
      prefix = prefix * pow(z / pdalboost::math::constants::e<T>(), z);
      BOOST_MATH_INSTRUMENT_CODE(prefix);
      T sum = detail::lower_gamma_series(z, z, pol) / z;
      BOOST_MATH_INSTRUMENT_CODE(sum);
      sum += detail::upper_gamma_fraction(z, z, ::pdalboost::math::policies::get_epsilon<T, Policy>());
      BOOST_MATH_INSTRUMENT_CODE(sum);
...
T lgamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos& l, int*sign)
{
   ...
   if(z <= 0)
   {
      ...
      result = log(pdalboost::math::constants::pi<T>()) - lgamma_imp(z, pol, l, 0) - log(t);
   }
   else if((z != 1) && (z != 2))
   {
      ...
      sum += detail::upper_gamma_fraction(z, limit, ::pdalboost::math::policies::get_epsilon<T, Policy>());
...
T tgammap1m1_imp(T dz, Policy const& pol, const Lanczos& l)
{
   ...
   if(dz < 0)
   {
      if(dz < -0.5)
      {
         ...
         result = pdalboost::math::tgamma(1+dz, pol) - 1;
         BOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
         // Use expm1 on lgamma:
         result = pdalboost::math::expm1(-pdalboost::math::log1p(dz, pol) 
   ...
   else
   {
      ...
      {
         // Use expm1 on lgamma:
         result = pdalboost::math::expm1(lgamma_small_imp<T>(dz+1, dz, dz-1, tag_type(), pol, l), pol);
         BOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
...
inline T tgammap1m1_imp(T dz, Policy const& pol,
                 const ::pdalboost::math::lanczos::undefined_lanczos& l)
{
   ...
   BOOST_MATH_INSTRUMENT_CODE((ldexp(1.0, pdalboost::math::policies::digits<T, Policy>()) * fabs(result) < 1e34));
   if((dz > -0.5) && (dz < 2) && (ldexp(1.0, pdalboost::math::policies::digits<T, Policy>()) * fabs(result) < 1e34))
   {
      result = tgammap1m1_imp(dz, pol, pdalboost::math::lanczos::lanczos24m113());
      BOOST_MATH_INSTRUMENT_CODE(result);
   }
   return result;
}
...
T full_igamma_prefix(T a, T z, const Policy& pol)
{
   ...
   //
   if((pdalboost::math::fpclassify)(prefix) == (int)FP_INFINITE)
      policies::raise_overflow_error<T>("pdalboost::math::detail::full_igamma_prefix<%1%>(%1%, %1%)", "Result of incomplete gamma function is too large to represent.", pol);
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const Lanczos& l)
{
   ...
   else if((fabs(d*d*a) <= 100) && (a > 150))
   {
      ...
      prefix = a * pdalboost::math::log1pmx(d, pol) + z * static_cast<T>(0.5 - Lanczos::g()) / agh;
      prefix = exp(prefix);
   }
   else
   {
   ...
   prefix *= sqrt(agh / pdalboost::math::constants::e<T>()) / Lanczos::lanczos_sum_expG_scaled(a);
   return prefix;
}
//
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   sum += detail::upper_gamma_fraction(a, limit, ::pdalboost::math::policies::get_epsilon<T, Policy>());
...
inline T tgamma_small_upper_part(T a, T x, const Policy& pol, T* pgam = 0, bool invert = false, T* pderivative = 0)
{
   ...
   //
   T result;
   result = pdalboost::math::tgamma1pm1(a, pol);
   if(pgam)
      *pgam = (result + 1) / a;
   T p = pdalboost::math::powm1(x, a, pol);
   result -= p;
   result /= a;
   detail::small_gamma2_series<T> s(a, x);
   pdalboost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>() - 10;
   p += 1;
   if(pderivative)
      *pderivative = p / (*pgam * exp(x));
   T init_value = invert ? *pgam : 0;
   result = -p * tools::sum_series(s, pdalboost::math::policies::get_epsilon<T, Policy>(), max_iter, (init_value - result) / p);
   policies::check_series_iterations<T>("pdalboost::math::tgamma_small_upper_part<%1%>(%1%, %1%)", max_iter, pol);
   if(invert)
      result = -result;
   return result;
}
...
inline T finite_gamma_q(T a, T x, Policy const& pol, T* pderivative = 0)
{
   ...
   if(pderivative)
   {
      *pderivative = e * pow(x, a) / pdalboost::math::unchecked_factorial<T>(itrunc(T(a - 1), pol));
   }
   return sum;
}
//
...
T finite_half_gamma_q(T a, T x, T* p_derivative, const Policy& pol)
{
   ...
   //
   BOOST_MATH_STD_USING
   T e = pdalboost::math::erfc(sqrt(x), pol);
   if((e != 0) && (a > 1))
   {
...
{
   static const char* function = "pdalboost::math::gamma_p<%1%>(%1%, %1%)";
   if(a <= 0)
   ...
   case 0:
      {
         ...
         if(normalised == false)
            result *= pdalboost::math::tgamma(a, pol);
         break;
      }
   case 1:
      {
   ...
   case 2:
      {
         ...
         if(result != 0)
         {
            ...
            if(invert)
            {
               init_value = -a * (normalised ? 1 : pdalboost::math::tgamma(a, pol)) / result;
            }
   ...
   if(invert)
   {
      T gam = normalised ? 1 : pdalboost::math::tgamma(a, pol);
      result = gam - result;
   }
   if(p_derivative)
   {
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const Lanczos&)
{
   ...
   if(fabs(delta) < 10)
   {
      result = exp((constants::half<T>() - z) * pdalboost::math::log1p(delta / zgh, pol));
   }
   else
   {
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   if(delta < 10)
   {
      prefix *= exp(-z * pdalboost::math::log1p(delta / z, pol));
   }
   else
   {
      prefix *= pow(z / zd, z);
   ...
   sumd += detail::upper_gamma_fraction(zd, zd, ::pdalboost::math::policies::get_epsilon<T, Policy>());
   sum /= sumd;
   if(fabs(tools::max_value<T>() / prefix) < fabs(sum))
      return policies::raise_overflow_error<T>("pdalboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Result of tgamma is too large to represent.", pol);
   return sum * prefix;
}
...
T tgamma_delta_ratio_imp(T z, T delta, const Policy& pol)
{
   ...
   if(z <= 0)
      policies::raise_domain_error<T>("pdalboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got a=%1%).", z, pol);
   if(z+delta <= 0)
      policies::raise_domain_error<T>("pdalboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got b=%1%).", z+delta, pol);
...
T tgamma_ratio_imp(T x, T y, const Policy& pol)
{
   ...
   if((x <= tools::min_value<T>()) || (pdalboost::math::isinf)(x))
      policies::raise_domain_error<T>("pdalboost::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got a=%1%).", x, pol);
   if((y <= tools::min_value<T>()) || (pdalboost::math::isinf)(y))
      policies::raise_domain_error<T>("pdalboost::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got b=%1%).", y, pol);
   ...
   if((x < max_factorial<T>::value) && (y < max_factorial<T>::value))
   {
      ...
      return pdalboost::math::tgamma(x, pol) / pdalboost::math::tgamma(y, pol);
   }
   T prefix = 1;
   if(x < 1)
   {
      if(y < 2 * max_factorial<T>::value)
      {
         ...
         return prefix * pdalboost::math::tgamma(x, pol) / pdalboost::math::tgamma(y, pol);
      }
      //
      // result is almost certainly going to underflow to zero, try logs just in case:
      //
      return exp(pdalboost::math::lgamma(x, pol) - pdalboost::math::lgamma(y, pol));
   }
   if(y < 1)
   {
   ...
   //
   return pdalboost::math::tgamma_delta_ratio(x, y - x, pol);
}
...
T gamma_p_derivative_imp(T a, T x, const Policy& pol)
{
   ...
   //
   if(a <= 0)
      policies::raise_domain_error<T>("pdalboost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument a to the incomplete gamma function must be greater than zero (got a=%1%).", a, pol);
   if(x < 0)
      policies::raise_domain_error<T>("pdalboost::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument x to the incomplete gamma function must be >= 0 (got x=%1%).", x, pol);
   //
   // Now special cases:
   //
   if(x == 0)
   {
      return (a > 1) ? 0 :
         (a == 1) ? 1 : policies::raise_overflow_error<T>("pdalboost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
   }
   //
   // Normal case:
   //
   ...
   {
      // overflow:
      return policies::raise_overflow_error<T>("pdalboost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
   }
   ...
   tgamma(T z, const Policy& /* pol */, const mpl::true_)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "pdalboost::math::tgamma<%1%>(%1%)");
}
...
struct igamma_initializer
{
   struct init
   {
      ...
      {
         pdalboost::math::gamma_p(static_cast<T>(400), static_cast<T>(400), Policy());
      }
...
struct lgamma_initializer
{
   struct init
   {
      ...
      {
         pdalboost::math::lgamma(static_cast<T>(2.5), Policy());
         pdalboost::math::lgamma(static_cast<T>(1.25), Policy());
         pdalboost::math::lgamma(static_cast<T>(1.75), Policy());
      }
      static void do_init(const mpl::int_<113>&)
      {
         pdalboost::math::lgamma(static_cast<T>(2.5), Policy());
         pdalboost::math::lgamma(static_cast<T>(1.25), Policy());
         pdalboost::math::lgamma(static_cast<T>(1.5), Policy());
         pdalboost::math::lgamma(static_cast<T>(1.75), Policy());
      }
   ...
   tgamma(T1 a, T2 z, const Policy&, const mpl::false_)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      ...
      forwarding_policy(), static_cast<value_type*>(0)), "pdalboost::math::tgamma<%1%>(%1%, %1%)");
}
   ...
   lgamma(T z, int* sign, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::lgamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type(), sign), "pdalboost::math::lgamma<%1%>(%1%)");
}
...
{
   return ::pdalboost::math::lgamma(x, 0, pol);
}
...
inline typename tools::promote_args<T>::type 
   lgamma(T x)
{
   return ::pdalboost::math::lgamma(x, 0, policies::policy<>());
}
   ...
   tgamma1pm1(T z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<typename remove_cv<result_type>::type, forwarding_policy>(detail::tgammap1m1_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "pdalboost::math::tgamma1pm1<%!%>(%1%)");
}
   ...
   tgamma_delta_ratio(T1 z, T2 delta, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_delta_ratio_imp(static_cast<value_type>(z), static_cast<value_type>(delta), forwarding_policy()), "pdalboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
   ...
   tgamma_ratio(T1 a, T2 b, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_ratio_imp(static_cast<value_type>(a), static_cast<value_type>(b), forwarding_policy()), "pdalboost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
   ...
   gamma_p_derivative(T1 a, T2 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_p_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(x), forwarding_policy()), "pdalboost::math::gamma_p_derivative<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type 
