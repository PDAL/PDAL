...
inline T factorial(unsigned i, const Policy& pol)
{
   ...
   T result = pdalboost::math::tgamma(static_cast<T>(i+1), pol);
...
T double_factorial(unsigned i, const Policy& pol)
{
   ...
   if(i & 1)
   {
      ...
      //
      T result = pdalboost::math::tgamma(static_cast<T>(i) / 2 + 1, pol) / sqrt(constants::pi<T>());
   ...
   //
   return policies::raise_overflow_error<T>("pdalboost::math::double_factorial<%1%>(unsigned)", 0, pol);
}
...
T rising_factorial_imp(T x, int n, const Policy& pol)
{
   ...
   // use case:
   //
   return 1 / pdalboost::math::tgamma_delta_ratio(x, static_cast<T>(n), pol);
}
...
inline T falling_factorial_imp(T x, unsigned n, const Policy& pol)
{
   ...
   if(x < n-1)
   {
      ...
      if(n2 == xp1)
         return 0;
      T result = pdalboost::math::tgamma_delta_ratio(xp1, -static_cast<T>(n2), pol);
      x -= n2;
      result *= x;
      ++n2;
      if(n2 < n)
   ...
   // for that use case:
   //
   return pdalboost::math::tgamma_delta_ratio(x + 1, -static_cast<T>(n), pol);
}
