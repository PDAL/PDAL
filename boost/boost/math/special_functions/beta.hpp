...
T beta_imp(T a, T b, const Lanczos&, const Policy& pol)
{
   ...
   if(a <= 0)
      policies::raise_domain_error<T>("pdalboost::math::beta<%1%>(%1%,%1%)", "The arguments to the beta function must be greater than zero (got a=%1%).", a, pol);
   if(b <= 0)
      policies::raise_domain_error<T>("pdalboost::math::beta<%1%>(%1%,%1%)", "The arguments to the beta function must be greater than zero (got b=%1%).", b, pol);
   ...
   if((c == a) && (b < tools::epsilon<T>()))
      return pdalboost::math::tgamma(b, pol);
   else if((c == b) && (a < tools::epsilon<T>()))
      return pdalboost::math::tgamma(a, pol);
   if(b == 1)
      return 1/a;
   else if(a == 1)
      return 1/b;
   ...
   */
   ...
   if((fabs(b * ambh) < (cgh * 100)) && (a > 100))
   {
      ...
      result *= exp(ambh * pdalboost::math::log1p(-b / cgh, pol));
   }
   else
   {
   ...
   result *= sqrt(pdalboost::math::constants::e<T>() / bgh);
...
T beta_imp(T a, T b, const lanczos::undefined_lanczos& /* l */, const Policy& pol)
{
   ...
   sa += detail::upper_gamma_fraction(a, la, ::pdalboost::math::policies::get_epsilon<T, Policy>());
   T sb = detail::lower_gamma_series(b, lb, pol) / b;
   sb += detail::upper_gamma_fraction(b, lb, ::pdalboost::math::policies::get_epsilon<T, Policy>());
   T sc = detail::lower_gamma_series(c, lc, pol) / c;
   sc += detail::upper_gamma_fraction(c, lc, ::pdalboost::math::policies::get_epsilon<T, Policy>());
                        ...
                        const Policy& pol)
{
   ...
   if(((std::min)(fabs(l1), fabs(l2)) < 0.2))
   {
      ...
      if((l1 * l2 > 0) || ((std::min)(a, b) < 1))
      {
         ...
         if(fabs(l1) < 0.1)
         {
            result *= exp(a * pdalboost::math::log1p(l1, pol));
         ...
         if(fabs(l2) < 0.1)
         {
            result *= exp(b * pdalboost::math::log1p(l2, pol));
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
         else
         {
      ...
      else if((std::max)(fabs(l1), fabs(l2)) < 0.5)
      {
         ...
         {
            T l3 = pdalboost::math::expm1(ratio * pdalboost::math::log1p(l2, pol), pol);
            l3 = l1 + l3 + l3 * l1;
            l3 = a * pdalboost::math::log1p(l3, pol);
            result *= exp(l3);
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
         else
         {
            T l3 = pdalboost::math::expm1(pdalboost::math::log1p(l1, pol) / ratio, pol);
            l3 = l2 + l3 + l3 * l2;
            l3 = b * pdalboost::math::log1p(l3, pol);
            result *= exp(l3);
            BOOST_MATH_INSTRUMENT_VARIABLE(result);
         }
      }
      else if(fabs(l1) < fabs(l2))
      {
         ...
         T l = a * pdalboost::math::log1p(l1, pol)
            + b * log((y * cgh) / bgh);
         result *= exp(l);
         BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
      else
      {
         ...
         T l = b * pdalboost::math::log1p(l2, pol)
            + a * log((x * cgh) / agh);
         result *= exp(l);
         BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
   ...
   result *= sqrt(bgh / pdalboost::math::constants::e<T>());
   result *= sqrt(agh / cgh);
   result *= prefix;
...
T ibeta_power_terms(T a,
                        ...
                        T x,
                        T y,
                        const pdalboost::math::lanczos::undefined_lanczos&,
                        bool normalised,
                        const Policy& pol)
{
   BOOST_MATH_STD_USING
   ...
   else
   {
      ...
      if((fabs(b1 - 1) * a < 10) && (a > 1))
         p1 = exp(a * pdalboost::math::log1p((x * b - y * la) / la, pol));
      else
         p1 = pow(b1, a);
      if((fabs(b2 - 1) * b < 10) && (b > 1))
         p2 = exp(b * pdalboost::math::log1p((y * a - x * lb) / lb, pol));
      else
         p2 = pow(b2, b);
      T p3 = exp(e1);
      result = p1 * p2 / p3;
...
T ibeta_series(T a, T b, T x, T s0, const Lanczos&, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   ...
   if(normalised)
   {
      ...
      result = Lanczos::lanczos_sum_expG_scaled(c) / (Lanczos::lanczos_sum_expG_scaled(a) * Lanczos::lanczos_sum_expG_scaled(b));
      if(a * b < bgh * 10)
         result *= exp((b - 0.5f) * pdalboost::math::log1p(a / bgh, pol));
      else
         result *= pow(cgh / bgh, b - 0.5f);
      result *= pow(x * cgh / agh, a);
      result *= sqrt(agh / pdalboost::math::constants::e<T>());
   ...
   result = pdalboost::math::tools::sum_series(s, pdalboost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
   policies::check_series_iterations<T>("pdalboost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (with lanczos)", max_iter, pol);
   return result;
}
//
...
//
template <class T, class Policy>
T ibeta_series(T a, T b, T x, T s0, const pdalboost::math::lanczos::undefined_lanczos&, bool normalised, T* p_derivative, T y, const Policy& pol)
{
   BOOST_MATH_STD_USING
   ...
   if(normalised)
   {
      ...
      else
      {
         result = pow(b1, a);
         if(a * b < lb * 10)
            result *= exp(b * pdalboost::math::log1p(a / lb, pol));
         else
            result *= pow(b2, b);
         result /= exp(e1);
      }
   ...
   pdalboost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   result = pdalboost::math::tools::sum_series(s, pdalboost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
   policies::check_series_iterations<T>("pdalboost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (without lanczos)", max_iter, pol);
   return result;
}
...
inline T ibeta_fraction2(T a, T b, T x, T y, const Policy& pol, bool normalised, T* p_derivative)
{
   ...
   T fract = pdalboost::math::tools::continued_fraction_b(f, pdalboost::math::policies::get_epsilon<T, Policy>());
...
struct Pn_size
{
   ...
   BOOST_STATIC_ASSERT(::pdalboost::math::max_factorial<T>::value >= 100);
};
template <>
struct Pn_size<float>
{
   BOOST_STATIC_CONSTANT(unsigned, value = 15); // ~8-15 digit accuracy
   BOOST_STATIC_ASSERT(::pdalboost::math::max_factorial<float>::value >= 30);
};
template <>
struct Pn_size<double>
{
   BOOST_STATIC_CONSTANT(unsigned, value = 30); // 16-20 digit accuracy
   BOOST_STATIC_ASSERT(::pdalboost::math::max_factorial<double>::value >= 60);
};
template <>
struct Pn_size<long double>
{
   BOOST_STATIC_CONSTANT(unsigned, value = 50); // ~35-50 digit accuracy
   BOOST_STATIC_ASSERT(::pdalboost::math::max_factorial<long double>::value >= 100);
};
...
T beta_small_b_large_a_series(T a, T b, T x, T y, T s0, T mult, const Policy& pol, bool normalised)
{
   ...
   if(normalised)
   {
      prefix = h / pdalboost::math::tgamma_delta_ratio(a, b, pol);
      prefix /= pow(t, b);
   }
   else
   {
   ...
   //
   T p[ ::pdalboost::math::detail::Pn_size<T>::value ] = { 1 };  // see 9.3.
   //
   // Now an initial value for J, see 9.6:
   //
   T j = pdalboost::math::gamma_q(b, u, pol) / h;
   //
   ...
   for(unsigned n = 1; n < sizeof(p)/sizeof(p[0]); ++n)
   {
      ...
      */
      ...
      {
         mbn = m * b - n;
         p[n] += mbn * p[n-m] / pdalboost::math::unchecked_factorial<T>(tmp1);
         tmp1 += 2;
      }
      p[n] /= n;
      p[n] += bm1 / pdalboost::math::unchecked_factorial<T>(tnp1);
      //
...
{
   static const char* function = "pdalboost::math::ibeta<%1%>(%1%, %1%, %1%)";
   typedef typename lanczos::lanczos<T, Policy>::type lanczos_type;
   ...
   if(x == 0)
   {
      ...
      return (invert ? (normalised ? T(1) : pdalboost::math::beta(a, b, pol)) : T(0));
   }
   if(x == 1)
   {
      if(p_derivative)
      ...
      return (invert == 0 ? (normalised ? 1 : pdalboost::math::beta(a, b, pol)) : 0);
   }
   if((a == 0.5f) && (b == 0.5f))
   {
   ...
   if((std::min)(a, b) <= 1)
   {
      ...
      if((std::max)(a, b) <= 1)
      {
         ...
         if((a >= (std::min)(T(0.2), b)) || (pow(x, a) <= 0.9))
         {
            ...
            {
               fract = -(normalised ? 1 : pdalboost::math::beta(a, b, pol));
               invert = false;
   ...
   else
   {
      ...
      if(b < 40)
      {
         if((floor(a) == a) && (floor(b) == b) && (a < (std::numeric_limits<int>::max)() - 100))
         {
            ...
            if(!normalised)
               fract *= pdalboost::math::beta(a, b, pol);
            BOOST_MATH_INSTRUMENT_VARIABLE(fract);
         }
         else if(b * x <= 0.7)
         {
   ...
   return invert ? (normalised ? 1 : pdalboost::math::beta(a, b, pol)) - fract : fract;
...
T ibeta_derivative_imp(T a, T b, T x, const Policy& pol)
{
   ...
   {
      return (a > 1) ? 0 : 
         (a == 1) ? 1 / pdalboost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, 0, pol);
   }
   else if(x == 1)
   {
      return (b > 1) ? 0 :
         (b == 1) ? 1 / pdalboost::math::beta(a, b, pol) : policies::raise_overflow_error<T>(function, 0, pol);
   }
   //
   ...
   beta(RT1 a, RT2 b, const Policy&, const mpl::true_*)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::beta_imp(static_cast<value_type>(a), static_cast<value_type>(b), evaluation_type(), forwarding_policy()), "pdalboost::math::beta<%1%>(%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   beta(RT1 a, RT2 b, RT3 x, const mpl::false_*)
{
   return pdalboost::math::beta(a, b, x, policies::policy<>());
}
} // namespace detail
   ...
   beta(RT1 a, RT2 b, A arg)
{
   ...
   return pdalboost::math::detail::beta(a, b, arg, static_cast<tag*>(0));
}
...
inline typename tools::promote_args<RT1, RT2>::type 
   beta(RT1 a, RT2 b)
{
   return pdalboost::math::beta(a, b, policies::policy<>());
}
   ...
   beta(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), false, false), "pdalboost::math::beta<%1%>(%1%,%1%,%1%)");
}
   ...
   betac(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), true, false), "pdalboost::math::betac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   betac(RT1 a, RT2 b, RT3 x)
{
   return pdalboost::math::betac(a, b, x, policies::policy<>());
}
   ...
   ibeta(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), false, true), "pdalboost::math::ibeta<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   ibeta(RT1 a, RT2 b, RT3 x)
{
   return pdalboost::math::ibeta(a, b, x, policies::policy<>());
}
   ...
   ibetac(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy(), true, true), "pdalboost::math::ibetac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   ibetac(RT1 a, RT2 b, RT3 x)
{
   return pdalboost::math::ibetac(a, b, x, policies::policy<>());
}
   ...
   ibeta_derivative(RT1 a, RT2 b, RT3 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::ibeta_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(b), static_cast<value_type>(x), forwarding_policy()), "pdalboost::math::ibeta_derivative<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type 
   ibeta_derivative(RT1 a, RT2 b, RT3 x)
{
   return pdalboost::math::ibeta_derivative(a, b, x, policies::policy<>());
}
