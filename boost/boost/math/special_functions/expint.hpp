...
T expint_1_rational(const T& z, const mpl::int_<53>&)
{
   ...
   else if(z < -pdalboost::math::tools::log_min_value<T>())
   {
...
inline T expint_as_fraction(unsigned n, T z, const Policy& pol)
{
   ...
   T result = tools::continued_fraction_b(
      f, 
      pdalboost::math::policies::get_epsilon<T, Policy>(),
      max_iter);
   policies::check_series_iterations<T>("pdalboost::math::expint_continued_fraction<%1%>(unsigned,%1%)", max_iter, pol);
...
inline T expint_as_series(unsigned n, T z, const Policy& pol)
{
   ...
   result += pow(-z, static_cast<T>(n - 1)) 
      * (pdalboost::math::digamma(static_cast<T>(n)) - log(z)) / fact;
   BOOST_MATH_INSTRUMENT_VARIABLE(result)

   expint_series<T> s(k, z, x_k, denom, fact);
   result = tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, result);
   policies::check_series_iterations<T>("pdalboost::math::expint_series<%1%>(unsigned,%1%)", max_iter, pol);
   BOOST_MATH_INSTRUMENT_VARIABLE(result)
   BOOST_MATH_INSTRUMENT_VARIABLE(max_iter)
   return result;
}
...
{
   BOOST_MATH_STD_USING
   static const char* function = "pdalboost::math::expint<%1%>(unsigned, %1%)";
   if(z < 0)
...
T expint_i_as_series(T z, const Policy& pol)
{
   ...
   pdalboost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
   result = tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, result);
   policies::check_series_iterations<T>("pdalboost::math::expint_i_series<%1%>(%1%)", max_iter, pol);
   return result;
}
...
{
   static const char* function = "pdalboost::math::expint<%1%>(%1%)";
   if(z < 0)
...
T expint_i_imp(T z, const Policy& pol, const mpl::int_<53>& tag)
{
   ...
   if(z <= 6)
   {
      ...
      if(fabs(t) < 0.1)
      {
         result += pdalboost::math::log1p(t / r);
      }
      else
      {
         result += log(z / r);
...
struct expint_i_initializer
{
   struct init
   {
      ...
      {
         pdalboost::math::expint(T(5));
         pdalboost::math::expint(T(7));
         pdalboost::math::expint(T(18));
         pdalboost::math::expint(T(38));
         pdalboost::math::expint(T(45));
      }
      ...
      static void do_init(const mpl::int_<113>&)
      {
         pdalboost::math::expint(T(5));
         pdalboost::math::expint(T(7));
         pdalboost::math::expint(T(17));
         pdalboost::math::expint(T(25));
         pdalboost::math::expint(T(40));
         pdalboost::math::expint(T(50));
         pdalboost::math::expint(T(80));
         pdalboost::math::expint(T(200));
         pdalboost::math::expint(T(220));
      }
...
struct expint_1_initializer
{
   struct init
   {
      ...
      static void do_init(const mpl::int_<53>&)
      {
         pdalboost::math::expint(1, T(0.5));
         pdalboost::math::expint(1, T(2));
      }
      static void do_init(const mpl::int_<64>&)
      {
         pdalboost::math::expint(1, T(0.5));
   ...
   expint_forwarder(T z, const Policy& /*pol*/, mpl::true_ const&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::expint_i_imp(
      ...
      forwarding_policy(),
      tag_type()), "pdalboost::math::expint<%1%>(%1%)");
}
...
{
   return pdalboost::math::expint(n, z, policies::policy<>());
}
   ...
   expint(unsigned n, T z, const Policy& /*pol*/)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::expint_imp(
      ...
      static_cast<value_type>(z),
      forwarding_policy(),
      tag_type()), "pdalboost::math::expint<%1%>(unsigned, %1%)");
}
