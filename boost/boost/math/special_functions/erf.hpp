...
struct erf_asympt_series_t
{
   erf_asympt_series_t(T z) : xx(2 * -z * z), tk(1)
   {
      ...
      result = -exp(-z * z) / sqrt(pdalboost::math::constants::pi<T>());
      result /= z;
   }
...
T erf_imp(T z, bool invert, const Policy& pol, const Tag& t)
{
   ...
   if(!invert && (z > detail::erf_asymptotic_limit<T, Policy>()))
   {
      ...
      result = pdalboost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, 1);
      policies::check_series_iterations<T>("pdalboost::math::erf<%1%>(%1%, %1%)", max_iter, pol);
   }
   else
   {
      T x = z * z;
      if(x < 0.6)
      {
         // Compute P:
         result = z * exp(-x);
         result /= sqrt(pdalboost::math::constants::pi<T>());
         if(result != 0)
...
struct erf_initializer
{
   struct init
   {
      ...
      {
         pdalboost::math::erf(static_cast<T>(1e-12), Policy());
         pdalboost::math::erf(static_cast<T>(0.25), Policy());
         pdalboost::math::erf(static_cast<T>(1.25), Policy());
         pdalboost::math::erf(static_cast<T>(2.25), Policy());
         pdalboost::math::erf(static_cast<T>(4.25), Policy());
         pdalboost::math::erf(static_cast<T>(5.25), Policy());
      }
      ...
      {
         pdalboost::math::erf(static_cast<T>(1e-22), Policy());
         pdalboost::math::erf(static_cast<T>(0.25), Policy());
         pdalboost::math::erf(static_cast<T>(1.25), Policy());
         pdalboost::math::erf(static_cast<T>(2.125), Policy());
         pdalboost::math::erf(static_cast<T>(2.75), Policy());
         pdalboost::math::erf(static_cast<T>(3.25), Policy());
         pdalboost::math::erf(static_cast<T>(5.25), Policy());
         pdalboost::math::erf(static_cast<T>(7.25), Policy());
         pdalboost::math::erf(static_cast<T>(11.25), Policy());
         pdalboost::math::erf(static_cast<T>(12.5), Policy());
      }
...
inline typename tools::promote_args<T>::type erf(T z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::erf_imp(
      ...
      false,
      forwarding_policy(),
      tag_type()), "pdalboost::math::erf<%1%>(%1%, %1%)");
}
...
inline typename tools::promote_args<T>::type erfc(T z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::erf_imp(
      ...
      true,
      forwarding_policy(),
      tag_type()), "pdalboost::math::erfc<%1%>(%1%, %1%)");
}

template <class T>
inline typename tools::promote_args<T>::type erf(T z)
{
   return pdalboost::math::erf(z, policies::policy<>());
}

template <class T>
inline typename tools::promote_args<T>::type erfc(T z)
{
   return pdalboost::math::erfc(z, policies::policy<>());
}
