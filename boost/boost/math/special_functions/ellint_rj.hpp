...
T ellint_rj_imp(T x, T y, T z, T p, const Policy& pol)
{
    ...
    static const char* function = "pdalboost::math::ellint_rj<%1%>(%1%,%1%,%1%)";
    ...
    if (p < 0)
    {
       ...
       p = pmy + y;
       value = pdalboost::math::ellint_rj(x, y, z, p, pol);
       value *= pmy;
       value -= 3 * pdalboost::math::ellint_rf(x, y, z, pol);
       value += 3 * sqrt((x * y * z) / (x * z + p * q)) * pdalboost::math::ellint_rc(x * z + p * q, p * q, pol);
       value /= (y + q);
       return value;
    }
    ...
    do
    {
        ...
        sigma += factor * pdalboost::math::ellint_rc(alpha, beta, pol);
        factor /= 4;
        x = (x + lambda) / 4;
        y = (y + lambda) / 4;
        z = (z + lambda) / 4;
   ...
   ellint_rj(T1 x, T2 y, T3 z, T4 p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_rj_imp(
         ...
         pol), "pdalboost::math::ellint_rj<%1%>(%1%,%1%,%1%,%1%)");
}
