...
struct zeta_series_cache_size
{
   ...
   // to zeta for example.
   //
   typedef typename pdalboost::math::policies::precision<T,Policy>::type precision_type;
...
T zeta_series_imp(T s, T sc, const Policy&)
{
   ...
   return sum * 1 / -pdalboost::math::powm1(T(2), sc);
}
...
inline T zeta_series2_imp(T s, const Policy& pol)
{
   ...
   T result = tools::sum_series(
      ...
      max_iter);
   policies::check_series_iterations<T>("pdalboost::math::zeta_series2<%1%>(%1%)", max_iter, pol);
   return result;
}
#endif
...
T zeta_polynomial_series(T s, T sc, Policy const &)
{
   ...
   //
   BOOST_MATH_STD_USING
   int n = itrunc(T(log(pdalboost::math::tools::epsilon<T>()) / -2));
   T sum = 0;
   T two_n = ldexp(T(1), n);
   int ej_sign = 1;
   for(int j = 0; j < n; ++j)
...
T zeta_imp(T s, T sc, const Policy& pol, const Tag& tag)
{
   ...
   else if(s < 0)
   {
      ...
      {
         result = pdalboost::math::sin_pi(0.5f * sc, pol)
            * 2 * pow(2 * constants::pi<T>(), -s) 
            * pdalboost::math::tgamma(s, pol) 
...
struct zeta_initializer
{
   struct init
   {
      ...
      {
         pdalboost::math::zeta(static_cast<T>(0.5), Policy());
         pdalboost::math::zeta(static_cast<T>(1.5), Policy());
         pdalboost::math::zeta(static_cast<T>(3.5), Policy());
         pdalboost::math::zeta(static_cast<T>(6.5), Policy());
         pdalboost::math::zeta(static_cast<T>(14.5), Policy());
         pdalboost::math::zeta(static_cast<T>(40.5), Policy());
      }
      static void do_init(const mpl::int_<113>&)
      {
         ...
         pdalboost::math::zeta(static_cast<T>(1.5), Policy());
         pdalboost::math::zeta(static_cast<T>(3.5), Policy());
         pdalboost::math::zeta(static_cast<T>(5.5), Policy());
         pdalboost::math::zeta(static_cast<T>(9.5), Policy());
         pdalboost::math::zeta(static_cast<T>(16.5), Policy());
         pdalboost::math::zeta(static_cast<T>(25), Policy());
         pdalboost::math::zeta(static_cast<T>(70), Policy());
      }
...
inline typename tools::promote_args<T>::type zeta(T s, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::zeta_imp(
      ...
      forwarding_policy(),
      tag_type()), "pdalboost::math::zeta<%1%>(%1%)");
}
