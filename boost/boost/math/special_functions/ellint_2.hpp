...
T ellint_e_imp(T phi, T k, const Policy& pol)
{
    ...
    if(phi >= tools::max_value<T>())
    {
       ...
       result = policies::raise_overflow_error<T>("pdalboost::math::ellint_e<%1%>(%1%,%1%)", 0, pol);
    }
    ...
    else
    {
       ...
       //
       T rphi = pdalboost::math::tools::fmod_workaround(phi, T(constants::half_pi<T>()));
       T m = pdalboost::math::round((phi - rphi) / constants::half_pi<T>());
       int s = 1;
       if(pdalboost::math::tools::fmod_workaround(m, T(2)) > 0.5)
       {
          m += 1;
          s = -1;
...
T ellint_e_imp(T k, const Policy& pol)
{
    ...
    if (abs(k) > 1)
    {
       return policies::raise_domain_error<T>("pdalboost::math::ellint_e<%1%>(%1%)",
...
inline typename tools::promote_args<T>::type ellint_2(T k, const Policy& pol, const mpl::true_&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_e_imp(static_cast<value_type>(k), pol), "pdalboost::math::ellint_2<%1%>(%1%)");
}
...
{
   return pdalboost::math::ellint_2(k, phi, policies::policy<>());
}
...
inline typename tools::promote_args<T1, T2>::type ellint_2(T1 k, T2 phi, const Policy& pol)
{
   ...
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_e_imp(static_cast<value_type>(phi), static_cast<value_type>(k), pol), "pdalboost::math::ellint_2<%1%>(%1%,%1%)");
}
