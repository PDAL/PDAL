...
struct big_int_type
{
   operator pdalboost::uintmax_t()const;
};
...
struct largest_cbrt_int_type
{
   typedef typename mpl::if_<
      pdalboost::is_convertible<big_int_type, T>,
      pdalboost::uintmax_t,
      unsigned int
   >::type type;
};
...
T cbrt_imp(T z, const Policy& pol)
{
   ...
   if(!(pdalboost::math::isfinite)(z))
   {
      return policies::raise_domain_error("pdalboost::math::cbrt<%1%>(%1%)", "Argument to function must be finite but got %1%.", z, pol);
   }
