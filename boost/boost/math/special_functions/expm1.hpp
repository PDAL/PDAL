...
T expm1_imp(T x, const mpl::int_<0>&, const Policy& pol)
{
   ...
   if(a > T(0.5f))
   {
      ...
      {
         if(x > 0)
            return policies::raise_overflow_error<T>("pdalboost::math::expm1<%1%>(%1%)", 0, pol);
         return -1;
      }
      return exp(x) - T(1);
   }
...
#endif
   policies::check_series_iterations<T>("pdalboost::math::expm1<%1%>(%1%)", max_iter, pol);
   return result;
}
...
inline typename tools::promote_args<T>::type expm1(T x, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::expm1_imp(
      ...
      tag_type(), forwarding_policy()), "pdalboost::math::expm1<%1%>(%1%)");
}
