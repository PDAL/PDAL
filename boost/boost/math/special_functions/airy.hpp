...
T airy_ai_imp(T x, const Policy& pol)
{
   ...
   if(x < 0)
   {
      T p = (-x * sqrt(-x) * 2) / 3;
      T v = T(1) / 3;
      T j1 = pdalboost::math::cyl_bessel_j(v, p, pol);
      T j2 = pdalboost::math::cyl_bessel_j(-v, p, pol);
      T ai = sqrt(-x) * (j1 + j2) / 3;
      //T bi = sqrt(-x / 3) * (j2 - j1);
      return ai;
   }
   ...
   {
      T tg = pdalboost::math::tgamma(constants::twothirds<T>(), pol);
      T ai = 1 / (pow(T(3), constants::twothirds<T>()) * tg);
      //T bi = 1 / (sqrt(pdalboost::math::cbrt(T(3))) * tg);
      return ai;
   }
   else
   {
      T p = 2 * x * sqrt(x) / 3;
      T v = T(1) / 3;
      //T j1 = pdalboost::math::cyl_bessel_i(-v, p, pol);
      //T j2 = pdalboost::math::cyl_bessel_i(v, p, pol);
      //
      ...
      //
      T ai = cyl_bessel_k(v, p, pol) * sqrt(x / 3) / pdalboost::math::constants::pi<T>();  //sqrt(x) * (j1 - j2) / 3;
      //T bi = sqrt(x / 3) * (j1 + j2);
      return ai;
   }
}
...
T airy_ai_prime_imp(T x, const Policy& pol)
{
   ...
   {
      T tg = pdalboost::math::tgamma(constants::third<T>(), pol);
      T aip = 1 / (pdalboost::math::cbrt(T(3)) * tg);
      return -aip;
   }
   else
   {
      ...
      // as we're subtracting two very large values, so use the Bessel K relation instead:
      //
      T aip = -cyl_bessel_k(v, p, pol) * x / (pdalboost::math::constants::root_three<T>() * pdalboost::math::constants::pi<T>());
      return aip;
   }
}
...
T airy_bi_prime_imp(T x, const Policy& pol)
{
   ...
   {
      T tg = pdalboost::math::tgamma(constants::third<T>(), pol);
      T bip = sqrt(pdalboost::math::cbrt(T(3))) / tg;
      return bip;
   }
   else
   {
      ...
      T j1 = pdalboost::math::cyl_bessel_i(-v, p, pol);
      T j2 = pdalboost::math::cyl_bessel_i(v, p, pol);
      T aip = x * (j1 + j2) / pdalboost::math::constants::root_three<T>();
      return aip;
   }
}
...
T airy_ai_zero_imp(unsigned m, const Policy& pol)
{
   ...
   if(m == 0U)
   {
      return policies::raise_domain_error<T>("pdalboost::math::airy_ai_zero<%1%>(%1%,%1%)",
        "The requested rank of the zero is %1%, but must be 1 or more !", static_cast<T>(m), pol);
   }

   // Set up the initial guess for the upcoming root-finding.
   const T guess_root = pdalboost::math::detail::airy_zero::airy_ai_zero_detail::initial_guess<T>(m);

   // Select the maximum allowed iterations, being at least 24.
   pdalboost::uintmax_t number_of_iterations = (std::max)(24, int(std::numeric_limits<T>::digits10));
   ...
   const T am =
      pdalboost::math::tools::newton_raphson_iterate(
         pdalboost::math::detail::airy_zero::airy_ai_zero_detail::function_object_ai_and_ai_prime<T, Policy>(pol),
         guess_root,
...
T airy_bi_zero_imp(unsigned m, const Policy& pol)
{
   ...
   if(m == 0U)
   {
      return policies::raise_domain_error<T>("pdalboost::math::airy_bi_zero<%1%>(%1%,%1%)",
        "The requested rank of the zero is %1%, but must be 1 or more !", static_cast<T>(m), pol);
   }
   // Set up the initial guess for the upcoming root-finding.
   const T guess_root = pdalboost::math::detail::airy_zero::airy_bi_zero_detail::initial_guess<T>(m);
   ...
   const T bm =
      pdalboost::math::tools::newton_raphson_iterate(
         pdalboost::math::detail::airy_zero::airy_bi_zero_detail::function_object_bi_and_bi_prime<T, Policy>(pol),
         guess_root,
         T(guess_root - tolerance),
         T(guess_root + tolerance),
         my_digits2,
...
inline typename tools::promote_args<T>::type airy_ai(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::airy_ai_imp<value_type>(static_cast<value_type>(x), forwarding_policy()), "pdalboost::math::airy<%1%>(%1%)");
}
...
inline typename tools::promote_args<T>::type airy_bi(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::airy_bi_imp<value_type>(static_cast<value_type>(x), forwarding_policy()), "pdalboost::math::airy<%1%>(%1%)");
}
...
inline typename tools::promote_args<T>::type airy_ai_prime(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::airy_ai_prime_imp<value_type>(static_cast<value_type>(x), forwarding_policy()), "pdalboost::math::airy<%1%>(%1%)");
}
...
inline typename tools::promote_args<T>::type airy_bi_prime(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::airy_bi_prime_imp<value_type>(static_cast<value_type>(x), forwarding_policy()), "pdalboost::math::airy<%1%>(%1%)");
}
...
inline T airy_ai_zero(unsigned m, const Policy& /*pol*/)
{
   ...
   return policies::checked_narrowing_cast<T, Policy>(detail::airy_ai_zero_imp<value_type>(m, forwarding_policy()), "pdalboost::math::airy_ai_zero<%1%>(unsigned)");
}
                         ...
                         const Policy& pol)
{
   ...
   {
      *out_it = pdalboost::math::airy_ai_zero<result_type>(start_index + i, pol);
      ++out_it;
   }
   return out_it;
}
...
inline T airy_bi_zero(unsigned m, const Policy& /*pol*/)
{
   ...
   typedef typename policies::normalise<
      ...
      policies::assert_undefined<> >::type forwarding_policy;
   BOOST_STATIC_ASSERT_MSG(false == std::numeric_limits<T>::is_integer, "Airy return type must be a floating-point type.");
   return policies::checked_narrowing_cast<T, Policy>(detail::airy_bi_zero_imp<value_type>(m, forwarding_policy()), "pdalboost::math::airy_bi_zero<%1%>(unsigned)");
}
                         ...
                         const Policy& pol)
{
   ...
   for(unsigned i = 0; i < number_of_zeros; ++i)
   {
      *out_it = pdalboost::math::airy_bi_zero<result_type>(start_index + i, pol);
      ++out_it;
   }
   return out_it;
}
