...
inline T airy_ai_zero(unsigned m, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<T, Policy>(detail::airy_ai_zero_imp<T>(m, pol), "pdalboost::math::airy_ai_zero<%1%>(unsigned)");
}
...
inline T airy_bi_zero(unsigned m, const Policy& pol)
{
   ...
   typedef typename policies::normalise<
      ...
      policies::assert_undefined<> >::type forwarding_policy;
   BOOST_STATIC_ASSERT_MSG(false == std::numeric_limits<T>::is_integer, "Airy return type must be a floating-point type.");
   return policies::checked_narrowing_cast<T, Policy>(detail::airy_bi_zero_imp<T>(m, pol), "pdalboost::math::airy_bi_zero<%1%>(unsigned)");
}
