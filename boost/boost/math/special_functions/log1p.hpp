...
{ // The function returns the natural logarithm of 1 + x.
   ...
   static const char* function = "pdalboost::math::log1p<%1%>(%1%)";
...
struct log1p_initializer
{
   struct init
   {
      ...
      {
         pdalboost::math::log1p(static_cast<T>(0.25), Policy());
      }
...
{ 
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::log1p_imp(static_cast<value_type>(x), forwarding_policy(), tag_type()), "pdalboost::math::log1p<%1%>(%1%)");
}
...
inline float log1p(float x, const Policy& pol)
{
   return static_cast<float>(pdalboost::math::log1p(static_cast<double>(x), pol));
}
#ifndef _WIN32_WCE
//
...
{
   return pdalboost::math::log1p(x, policies::policy<>());
}
//
// Compute log(1+x)-x:
//
   ...
   log1pmx(T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   BOOST_MATH_STD_USING
   static const char* function = "pdalboost::math::log1pmx<%1%>(%1%)";
   ...
   if(a < tools::epsilon<result_type>())
      return -x * x / 2;
   pdalboost::math::detail::log1p_series<T> s(x);
   s();
   pdalboost::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
   T zero = 0;
   T result = pdalboost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T result = pdalboost::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>(function, max_iter, pol);
   return result;
}
