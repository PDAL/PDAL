...
T digamma_imp(T x, const Tag* t, const Policy& pol)
{
   ...
   if(x < 0)
   {
      ...
      if(remainder == 0)
      {
         return policies::raise_pole_error<T>("pdalboost::math::digamma<%1%>(%1%)", 0, (1-x), pol);
      }
...
struct digamma_initializer
{
   struct init
   {
      init()
      {
         pdalboost::math::digamma(T(1.5), Policy());
         pdalboost::math::digamma(T(500), Policy());
      }
   ...
   digamma(T x, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::digamma_imp(
      ...
      static_cast<const tag_type*>(0), pol), "pdalboost::math::digamma<%1%>(%1%)");
}
