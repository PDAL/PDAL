...
inline T bessel_j_small_z_series(T v, T x, const Policy& pol)
{
   ...
   {
      prefix = pow(x / 2, v) / pdalboost::math::tgamma(v+1, pol);
   }
   else
   {
      prefix = v * log(x / 2) - pdalboost::math::lgamma(v+1, pol);
      prefix = exp(prefix);
   }
   if(0 == prefix)
      return prefix;
...
#endif
   policies::check_series_iterations<T>("pdalboost::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
inline T bessel_y_small_z_series(T v, T x, T* pscale, const Policy& pol)
{
   ...
   if(!need_logs)
   {
      gam = pdalboost::math::tgamma(v, pol);
      p = pow(x / 2, v);
   ...
   {
      gam = pdalboost::math::lgamma(v, pol);
      p = v * p;
   ...
   *pscale = scale;
   ...
   T result = pdalboost::math::tools::sum_series(s, pdalboost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>("pdalboost::math::bessel_y_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   if(!need_logs)
   {
      prefix = pdalboost::math::tgamma(-v, pol) * pdalboost::math::cos_pi(v) * p / constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = pdalboost::math::lgamma(-v, &sgn, pol) + p;
...
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
   T b = pdalboost::math::tools::sum_series(s2, pdalboost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T b = pdalboost::math::tools::sum_series(s2, pdalboost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
...
T bessel_yn_small_z(int n, T z, T* scale, const Policy& pol)
{
   ...
   {
      T p = pow(z / 2, n);
      T result = -((pdalboost::math::factorial<T>(n - 1) / constants::pi<T>()));
