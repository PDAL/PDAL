...
T lgamma_small_imp(T z, T zm1, T zm2, const mpl::int_<0>&, const Policy& pol, const Lanczos&)
{
   ...
   else if(z >= 1.5)
   {
      // special case near 2:
      T dz = zm2;
      result = dz * log((z + Lanczos::g() - T(0.5)) / pdalboost::math::constants::e<T>());
      result += pdalboost::math::log1p(dz / (Lanczos::g() + T(1.5)), pol) * T(1.5);
      result += pdalboost::math::log1p(Lanczos::lanczos_sum_near_2(dz), pol);
   }
   else
   {
      // special case near 1:
      T dz = zm1;
      result = dz * log((z + Lanczos::g() - T(0.5)) / pdalboost::math::constants::e<T>());
      result += pdalboost::math::log1p(dz / (Lanczos::g() + T(0.5)), pol) / 2;
      result += pdalboost::math::log1p(Lanczos::lanczos_sum_near_1(dz), pol);
   }
   return result;
}
