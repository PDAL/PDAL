...
T erf_inv_imp(const T& p, const T& q, const Policy&, const pdalboost::mpl::int_<64>*)
{
...
struct erf_roots
{
   pdalboost::math::tuple<T,T,T> operator()(const T& guess)
   {
      BOOST_MATH_STD_USING
      T derivative = sign * (2 / sqrt(constants::pi<T>())) * exp(-(guess * guess));
      T derivative2 = -2 * guess * derivative;
      return pdalboost::math::make_tuple(((sign > 0) ? static_cast<T>(pdalboost::math::erf(guess, Policy()) - target) : static_cast<T>(pdalboost::math::erfc(guess, Policy())) - target), derivative, derivative2);
   }
...
template <class T, class Policy>
T erf_inv_imp(const T& p, const T& q, const Policy& pol, const pdalboost::mpl::int_<0>*)
{
   //
   ...
   if(policies::digits<T, Policy>() > 64)
   {
      ...
      policies::check_root_iterations<T>("pdalboost::math::erf_inv<%1%>", max_iter, pol);
   }
   else
   {
      result = guess;
...
struct erf_inv_initializer
{
   struct init
   {
      ...
      static void do_init()
      {
         pdalboost::math::erf_inv(static_cast<T>(0.25), Policy());
         pdalboost::math::erf_inv(static_cast<T>(0.55), Policy());
         pdalboost::math::erf_inv(static_cast<T>(0.95), Policy());
         pdalboost::math::erfc_inv(static_cast<T>(1e-15), Policy());
         if(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-130)) != 0)
            pdalboost::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-130)), Policy());
         ...
         if(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-800)) != 0)
            pdalboost::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-800)), Policy());
         if(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-900)) != 0)
            pdalboost::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-900)), Policy());
#else
         if(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-800)) != 0)
            pdalboost::math::erfc_inv(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-800)), Policy());
         if(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-900)) != 0)
            pdalboost::math::erfc_inv(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-900)), Policy());
#endif
      }
...
typename tools::promote_args<T>::type erfc_inv(T z, const Policy& pol)
{
   ...
   //
   static const char* function = "pdalboost::math::erfc_inv<%1%>(%1%, %1%)";
   if((z < 0) || (z > 2))
...
typename tools::promote_args<T>::type erf_inv(T z, const Policy& pol)
{
   ...
   // Begin by testing for domain errors, and other special cases:
   //
   static const char* function = "pdalboost::math::erf_inv<%1%>(%1%, %1%)";
   if((z < -1) || (z > 1))
