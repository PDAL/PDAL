...
T inverse_students_t_hill(T ndf, T u, const Policy& pol)
{
   ...
   if (ndf > 1e20f)
      return -pdalboost::math::erfc_inv(2 * u, pol) * constants::root_two<T>();
   ...
   if (y > (0.05f + a))
   {
      ...
      //
      x = -pdalboost::math::erfc_inv(2 * u, pol) * constants::root_two<T>();
      y = x * x;
      ...
      y = pdalboost::math::expm1(a * y * y, pol);
   }
   else
   {
...
T inverse_students_t_tail_series(T df, T v, const Policy& pol)
{
   ...
   T w = pdalboost::math::tgamma_delta_ratio(df / 2, constants::half<T>(), pol)
...
T inverse_students_t_body_series(T df, T u, const Policy& pol)
{
   ...
   //
   T v = pdalboost::math::tgamma_delta_ratio(df / 2, constants::half<T>(), pol)
...
calculate_real:
      if(df > 0x10000000)
      {
         result = -pdalboost::math::erfc_inv(2 * u, pol) * constants::root_two<T>();
         if((pexact) && (df >= 1e20))
            *pexact = true;
      }
      else if(df < 3)
      {
         ...
         if(u > crossover)
         {
            result = pdalboost::math::detail::inverse_students_t_body_series(df, u, pol);
         }
         else
         {
            result = pdalboost::math::detail::inverse_students_t_tail_series(df, u, pol);
         }
      }
      else
      {
         ...
         if(u > crossover)
         {
            result = pdalboost::math::detail::inverse_students_t_hill(df, u, pol);
         }
         else
         {
            result = pdalboost::math::detail::inverse_students_t_tail_series(df, u, pol);
...
inline T find_ibeta_inv_from_t_dist(T a, T p, T /*q*/, T* py, const Policy& pol)
{
   ...
   T v = 1 - u;
   T df = a * 2;
   T t = pdalboost::math::detail::inverse_students_t(df, u, v, pol);
   *py = t * t / (df + t * t);
   return df / (df + t * t);
}
...
inline T fast_students_t_quantile_imp(T df, T p, const Policy& pol, const mpl::false_*)
{
   ...
   if(df * y > tools::max_value<T>() * x)
      t = policies::raise_overflow_error<T>("pdalboost::math::students_t_quantile<%1%>(%1%,%1%)", 0, pol);
   else
      t = sqrt(df * y / x);
   //
...
T fast_students_t_quantile_imp(T df, T p, const Policy& pol, const mpl::true_*)
{
   ...
   bool invert = false;
   if((df < 2) && (floor(df) != df))
      return pdalboost::math::detail::fast_students_t_quantile_imp(df, p, pol, static_cast<mpl::false_*>(0));
   if(p > 0.5)
   {
      p = 1 - p;
      invert = true;
...
inline T fast_students_t_quantile(T df, T p, const Policy& pol)
{
   ...
   > tag_type;
   return policies::checked_narrowing_cast<T, forwarding_policy>(fast_students_t_quantile_imp(static_cast<value_type>(df), static_cast<value_type>(p), pol, static_cast<tag_type*>(0)), "pdalboost::math::students_t_quantile<%1%>(%1%,%1%,%1%)");
}
