...
struct beta_inv_ab_t
{
   ...
   {
      return invert ? 
         p - pdalboost::math::ibetac(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) 
         : pdalboost::math::ibeta(swap_ab ? b : a, swap_ab ? a : b, z, Policy()) - p;
   }
private:
   T b, z, p;
   bool invert, swap_ab;
...
T inverse_negative_binomial_cornish_fisher(T n, T sf, T sfc, T p, T q, const Policy& pol)
{
   ...
   T x = pdalboost::math::erfc_inv(p > q ? 2 * q : 2 * p, pol) * constants::root_two<T>();
   // Set the sign:
   if(p < 0.5)
      x = -x;
   T x2 = x * x;
...
T ibeta_inv_ab_imp(const T& b, const T& z, const T& p, const T& q, bool swap_ab, const Policy& pol)
{
   ...
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      policies::raise_evaluation_error<T>("pdalboost::math::ibeta_invab_imp<%1%>(%1%,%1%,%1%)", "Unable to locate the root within a reasonable number of iterations, closest approximation so far was %1%", r.first, pol);
      ...
      ibeta_inva(RT1 b, RT2 x, RT3 p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_inv_ab_imp(
         ...
         false, pol), 
      "pdalboost::math::ibeta_inva<%1%>(%1%,%1%,%1%)");
}
      ...
      ibetac_inva(RT1 b, RT2 x, RT3 q, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_inv_ab_imp(
         ...
         false, pol),
      "pdalboost::math::ibetac_inva<%1%>(%1%,%1%,%1%)");
}
      ...
      ibeta_invb(RT1 a, RT2 x, RT3 p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_inv_ab_imp(
         ...
         static_cast<value_type>(1 - static_cast<value_type>(p)), 
         true, pol),
      "pdalboost::math::ibeta_invb<%1%>(%1%,%1%,%1%)");
}
      ...
      ibetac_invb(RT1 a, RT2 x, RT3 q, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_inv_ab_imp(
         ...
         static_cast<value_type>(q), 
         true, pol),
         "pdalboost::math::ibetac_invb<%1%>(%1%,%1%,%1%)");
}
...
{
   return pdalboost::math::ibeta_inva(b, x, p, policies::policy<>());
}
...
{
   return pdalboost::math::ibetac_inva(b, x, q, policies::policy<>());
}
...
{
   return pdalboost::math::ibeta_invb(a, x, p, policies::policy<>());
}
...
{
   return pdalboost::math::ibetac_invb(a, x, q, policies::policy<>());
}
