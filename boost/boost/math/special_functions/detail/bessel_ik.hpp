...
int temme_ik(T v, T x, T* K, T* K1, const Policy& pol)
{
    ...
    T gp = pdalboost::math::tgamma1pm1(v, pol);
    T gm = pdalboost::math::tgamma1pm1(-v, pol);

    a = log(x / 2);
    b = exp(v * a);
    sigma = -a * v;
    c = abs(v) < tools::epsilon<T>() ?
       T(1) : T(pdalboost::math::sin_pi(v) / (v * pi<T>()));
    ...
    policies::check_series_iterations<T>("pdalboost::math::bessel_ik<%1%>(%1%,%1%) in temme_ik", k, pol);
...
int CF1_ik(T v, T x, T* fv, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>("pdalboost::math::bessel_ik<%1%>(%1%,%1%) in CF1_ik", k, pol);
...
int CF2_ik(T v, T x, T* Kv, T* Kv1, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>("pdalboost::math::bessel_ik<%1%>(%1%,%1%) in CF2_ik", k, pol);
...
int bessel_ik(T v, T x, T* I, T* K, int kind, const Policy& pol)
{
    ...
    static const char* function = "pdalboost::math::bessel_ik<%1%>(%1%,%1%)";
    ...
    if (x == 0)
    {
       ...
       {
           T z = (u + n % 2);
           Iv = pdalboost::math::sin_pi(z, pol) == 0 ? 
               Iv : 
    ...
    {
        T z = (u + n % 2);
        T fact = (2 / pi<T>()) * (pdalboost::math::sin_pi(z) * Kv);
        if(fact == 0)
           *I = Iv;
