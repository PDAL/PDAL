...
struct gamma_inva_t
{
   ...
   T operator()(T a)
   {
      return invert ? p - pdalboost::math::gamma_q(a, z, Policy()) : pdalboost::math::gamma_p(a, z, Policy()) - p;
   }
private:
   T z, p;
   bool invert;
...
T gamma_inva_imp(const T& z, const T& p, const T& q, const Policy& pol)
{
   ...
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      policies::raise_evaluation_error<T>("pdalboost::math::gamma_p_inva<%1%>(%1%, %1%)", "Unable to locate the root within a reasonable number of iterations, closest approximation so far was %1%", r.first, pol);
   ...
   gamma_p_inva(T1 x, T2 p, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::gamma_inva_imp(
         ...
         pol), "pdalboost::math::gamma_p_inva<%1%>(%1%, %1%)");
}
   ...
   gamma_q_inva(T1 x, T2 q, const Policy& pol)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::gamma_inva_imp(
         ...
         pol), "pdalboost::math::gamma_q_inva<%1%>(%1%, %1%)");
}
...
{
   return pdalboost::math::gamma_p_inva(x, p, policies::policy<>());
}
...
{
   return pdalboost::math::gamma_q_inva(x, q, policies::policy<>());
}
