...
{
   T c = 1;
   T p = (v / pdalboost::math::sin_pi(v, pol)) * pow(x / 2, -v) / pdalboost::math::tgamma(1 - v, pol);
   T q = (v / pdalboost::math::sin_pi(v, pol)) * pow(x / 2, v) / pdalboost::math::tgamma(1 + v, pol);
   T f = (p - q) / v;
   T g_prefix = pdalboost::math::sin_pi(v / 2, pol);
...
T asymptotic_bessel_i_large_x(T v, T x, const Policy& pol)
{
   ...
   return (pdalboost::math::isfinite)(s) ? 
      s : policies::raise_overflow_error<T>("pdalboost::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)", 0, pol);
}
