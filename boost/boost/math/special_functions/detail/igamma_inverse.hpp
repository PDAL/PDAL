...
inline T didonato_FN(T p, T a, T x, unsigned N, T tolerance, const Policy& pol)
{
   ...
   //
   BOOST_MATH_STD_USING
   T u = log(p) + pdalboost::math::lgamma(a + 1, pol);
...
T find_inverse_gamma(T a, T p, T q, const Policy& pol, bool* p_has_10_digits)
{
   ...
   *p_has_10_digits = false;
   ...
   {
      T g = pdalboost::math::tgamma(a, pol);
      T b = q * g;
   ...
   else
   {
      ...
      else if (p > 0.5)
      {
         ...
         else
         {
            ...
            T lg = pdalboost::math::lgamma(a, pol);
            T lb = log(q) + lg;
            if(lb < -D * 2.3)
            {
      ...
      else
      {
         ...
         if(w < 0.15f * ap1)
         {
            ...
            T v = log(p) + pdalboost::math::lgamma(ap1, pol);
            z = exp((v + w) / a);
            s = pdalboost::math::log1p(z / ap1 * (1 + z / ap2));
            z = exp((v + z - s) / a);
            s = pdalboost::math::log1p(z / ap1 * (1 + z / ap2));
            z = exp((v + z - s) / a);
            s = pdalboost::math::log1p(z / ap1 * (1 + z / ap2 * (1 + z / (a + 3))));
            z = exp((v + z - s) / a);
...
struct gamma_p_inverse_func
{
   ...
   pdalboost::math::tuple<T, T, T> operator()(const T& x)const
   {
      ...
      T f, f1;
      value_type ft;
      f = static_cast<T>(pdalboost::math::detail::gamma_incomplete_imp(
      ...
      return pdalboost::math::make_tuple(static_cast<T>(f - p), f1, f2);
   }
private:
   T a, p;
   bool invert;
...
T gamma_p_inv_imp(T a, T p, const Policy& pol)
{
   ...
   static const char* function = "pdalboost::math::gamma_p_inv<%1%>(%1%, %1%)";
...
T gamma_q_inv_imp(T a, T q, const Policy& pol)
{
   ...
   static const char* function = "pdalboost::math::gamma_q_inv<%1%>(%1%, %1%)";
