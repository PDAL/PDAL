...
T bessel_yn(int n, T x, const Policy& pol)
{
    ...
    static const char* function = "pdalboost::math::bessel_yn<%1%>(%1%,%1%)";
    ...
    if(x < policies::get_epsilon<T, Policy>())
    {
       ...
       if(tools::max_value<T>() * fabs(scale) < fabs(value))
          return pdalboost::math::sign(scale) * pdalboost::math::sign(value) * policies::raise_overflow_error<T>(function, 0, pol);
       value /= scale;
    }
    else if (n == 0)
    {
    ...
    else
    {
       ...
       int k = 1;
       BOOST_ASSERT(k < n);
       policies::check_series_iterations<T>("pdalboost::math::bessel_y_n<%1%>(%1%,%1%)", n, pol);
       do
       {
           T fact = 2 * k / x;
