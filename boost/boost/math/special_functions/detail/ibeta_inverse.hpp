...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost{ namespace math{ namespace detail{
...
struct temme_root_finder
{
   ...
   pdalboost::math::tuple<T, T> operator()(T x)
   {
      ...
      if(y == 0)
      {
         ...
         return pdalboost::math::make_tuple(static_cast<T>(-big), static_cast<T>(-big));
      }
      if(x == 0)
      {
         T big = tools::max_value<T>() / 4;
         return pdalboost::math::make_tuple(static_cast<T>(-big), big);
      }
      T f = log(x) + a * log(y) + t;
      T f1 = (1 / x) - (a / (y));
      return pdalboost::math::make_tuple(f, f1);
   }
private:
   T t, a;
};
...
T temme_method_1_ibeta_inverse(T a, T b, T z, const Policy& pol)
{
   ...
   //
   T eta0 = pdalboost::math::erfc_inv(2 * z, pol);
   eta0 /= -sqrt(a / 2);
...
T temme_method_3_ibeta_inverse(T a, T b, T p, T q, const Policy& pol)
{
   ...
   T eta0;
   if(p < q)
      eta0 = pdalboost::math::gamma_q_inv(b, p, pol);
   else
      eta0 = pdalboost::math::gamma_p_inv(b, q, pol);
   eta0 /= a;
   //
...
struct ibeta_roots
{
   ...
   pdalboost::math::tuple<T, T, T> operator()(T x)
   {
      BOOST_MATH_STD_USING // ADL of std names
      ...
      return pdalboost::math::make_tuple(f, f1, f2);
   }
private:
   T a, b, target;
   bool invert;
...
T ibeta_inv_imp(T a, T b, T p, T q, const Policy& pol, T* py)
{
   ...
   else if(a + b > 5)
   {
      ...
      else
      {
         ...
         if((lambda >= 0.2) && (lambda <= 0.8) && (r >= 10))
         {
            ...
            {
               x = ppa * pow(a * pdalboost::math::beta(a, b, pol), 1/a);
            }
            else
         ...
         else
         {
            ...
            T bet = 0;
            if(b < 2)
               bet = pdalboost::math::beta(a, b, pol);
            if(bet != 0)
            {
               y = pow(b * q * bet, 1/b);
               x = 1 - y;
   ...
   else if((a < 1) && (b < 1))
   {
      ...
      //
      T fs = pdalboost::math::ibeta(a, b, xs, pol) - p;
      ...
      T xg = pow(a * p * pdalboost::math::beta(a, b, pol), 1/a);
      x = xg / (1 + xg);
      y = 1 / (1 + xg);
      //
   ...
   else if((a > 1) && (b > 1))
   {
      ...
      T xs = (a - 1) / (a + b - 2);
      T xs2 = (b - 1) / (a + b - 2);
      T ps = pdalboost::math::ibeta(a, b, xs, pol) - p;
      ...
      //
      T lx = log(p * a * pdalboost::math::beta(a, b, pol)) / a;
      x = exp(lx);
      y = x < 0.9 ? T(1 - x) : (T)(-pdalboost::math::expm1(lx, pol));
   ...
   else /*if((a <= 1) != (b <= 1))*/
   {
      ...
      if(pow(p, 1/a) < 0.5)
      {
         x = pow(p * a * pdalboost::math::beta(a, b, pol), 1 / a);
         if(x == 0)
            x = pdalboost::math::tools::min_value<T>();
         y = 1 - x;
      }
      else /*if(pow(q, 1/b) < 0.1)*/
      {
         // model a distorted quarter circle:
         y = pow(1 - pow(p, b * pdalboost::math::beta(a, b, pol)), 1/b);
         if(y == 0)
            y = pdalboost::math::tools::min_value<T>();
         x = 1 - y;
      }
   }
   ...
   if(lower == 0)
   {
      if(invert && (py == 0))
      {
         ...
         //
         lower = pdalboost::math::tools::epsilon<T>();
         if(x < lower)
            x = lower;
      }
      else
         lower = pdalboost::math::tools::min_value<T>();
      if(x < lower)
         x = lower;
   }
   //
   ...
   //
   int digits = pdalboost::math::policies::digits<T, Policy>() / 2;
   if((x < 1e-50) && ((a < 1) || (b < 1)))
   {
      //
   ...
   x = pdalboost::math::tools::halley_iterate(
      pdalboost::math::detail::ibeta_roots<T, Policy>(a, b, (p < q ? p : q), (p < q ? false : true)), x, lower, upper, digits, max_iter);
   policies::check_root_iterations<T>("pdalboost::math::ibeta<%1%>(%1%, %1%, %1%)", max_iter, pol);
   //
   ...
   //BOOST_ASSERT((x != lower) || (x == pdalboost::math::tools::min_value<T>()) || (x == pdalboost::math::tools::epsilon<T>()));
   //
...
{
   static const char* function = "pdalboost::math::ibeta_inv<%1%>(%1%,%1%,%1%)";
   BOOST_FPU_EXCEPTION_GUARD
...
{
   static const char* function = "pdalboost::math::ibetac_inv<%1%>(%1%,%1%,%1%)";
   BOOST_FPU_EXCEPTION_GUARD
   typedef typename tools::promote_args<T1, T2, T3, T4>::type result_type;
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   typedef typename policies::normalise<
