...
{
   if((prob < 0) || (prob > 1) || !(pdalboost::math::isfinite)(prob))
   {
...
{ //  df > 0 but NOT +infinity allowed.
   if((df <= 0) || !(pdalboost::math::isfinite)(df))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
{  // df > 0 or +infinity are allowed.
   if( (df <= 0) || (pdalboost::math::isnan)(df) )
...
inline bool check_scale(
      ...
      const Policy& pol)
{
   if((scale <= 0) || !(pdalboost::math::isfinite)(scale))
...
inline bool check_location(
      ...
      const Policy& pol)
{
   if(!(pdalboost::math::isfinite)(location))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
inline bool check_positive_x(
      ...
      const Policy& pol)
{
   if(!(pdalboost::math::isfinite)(x) || (x < 0))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
inline bool check_non_centrality(
      ...
      const Policy& pol)
{
   if((ncp < 0) || !(pdalboost::math::isfinite)(ncp))
   { // Assume scale == 0 is NOT valid for any distribution.
      *result = policies::raise_domain_error<RealType>(
         function,
