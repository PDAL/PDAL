...
public:
   ...
   { // Constructor.
      ...
      detail::check_df_gt0_to_inf( // Checks that df > 0 or df == inf.
         "pdalboost::math::students_t_distribution<%1%>::students_t_distribution", df_, &result, Policy());
...
inline RealType pdf(const students_t_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   if(false == detail::check_x(
      "pdalboost::math::pdf(const students_t_distribution<%1%>&, %1%)", x, &error_result, Policy()))
      return error_result;
   RealType df = dist.degrees_of_freedom();
   if(false == detail::check_df_gt0_to_inf( // Check that df > 0 or == +infinity.
      "pdalboost::math::pdf(const students_t_distribution<%1%>&, %1%)", df, &error_result, Policy()))
      return error_result;

   RealType result;
   if ((pdalboost::math::isinf)(x))
   { // +infinity.
   ...
   { // 
     ...
     if(basem1 < 0.125)
     {
        result = exp(-pdalboost::math::log1p(basem1, Policy()) * (1+df) / 2);
     }
     else
     {
        result = pow(1 / (1 + basem1), (df + 1) / 2);
     }
     result /= sqrt(df) * pdalboost::math::beta(df / 2, RealType(0.5f), Policy());
   }
   return result;
} // pdf
...
inline RealType cdf(const students_t_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   if(false == detail::check_df_gt0_to_inf(  // Check that df > 0 or == +infinity.
      "pdalboost::math::cdf(const students_t_distribution<%1%>&, %1%)", df, &error_result, Policy()))
      return error_result;
...
inline RealType quantile(const students_t_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType df = dist.degrees_of_freedom();
   static const char* function = "pdalboost::math::quantile(const students_t_distribution<%1%>&, %1%)";
   RealType error_result;
   if(false == (detail::check_df_gt0_to_inf( // Check that df > 0 or == +infinity.
   ...
   //
   return pdalboost::math::detail::fast_students_t_quantile(df, probability, Policy());
} // quantile
...
RealType students_t_distribution<RealType, Policy>::find_degrees_of_freedom(
      ...
      RealType hint)
{
   static const char* function = "pdalboost::math::students_t_distribution<%1%>::find_degrees_of_freedom";
   //
   // Check for domain errors:
   //
   RealType error_result;
...
{  // Revised for https://svn.boost.org/trac/boost/ticket/7177
   RealType df = dist.degrees_of_freedom();
   if(((pdalboost::math::isnan)(df)) || (df <= 1) ) 
   { // mean is undefined for moment <= 1!
      policies::raise_domain_error<RealType>(
      "pdalboost::math::mean(students_t_distribution<%1%> const&, %1%)",
...
{
    RealType df = dist.degrees_of_freedom();
   if( ((pdalboost::math::isnan)(df)) || (dist.degrees_of_freedom() <= 3))
   { // Undefined for moment k = 3.
      policies::raise_domain_error<RealType>(
         "pdalboost::math::skewness(students_t_distribution<%1%> const&, %1%)",
...
{
   RealType df = dist.degrees_of_freedom();
   if(((pdalboost::math::isnan)(df)) || (df <= 4))
   { // Undefined or infinity for moment k = 4.
      policies::raise_domain_error<RealType>(
       "pdalboost::math::kurtosis(students_t_distribution<%1%> const&, %1%)",
...
inline RealType kurtosis_excess(const students_t_distribution<RealType, Policy>& dist)
{
   ...
   { // Undefined or infinity for moment k = 4.
     policies::raise_domain_error<RealType>(
       "pdalboost::math::kurtosis_excess(students_t_distribution<%1%> const&, %1%)",
