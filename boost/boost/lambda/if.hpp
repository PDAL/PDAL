...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(pdalboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
      detail::select(pdalboost::tuples::get<1>(args), CALL_ACTUAL_ARGS); 
  }
};
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(pdalboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
      ...
      detail::select(pdalboost::tuples::get<2>(args), CALL_ACTUAL_ARGS);
  }
};
...
struct return_type_2_ifthenelsereturn<1, false, false, false, A, B> {
  ...
  typedef const typename pdalboost::remove_reference<A>::type plainA; 
  typedef const typename pdalboost::remove_reference<B>::type plainB; 
  // TODO: Add support for volatile ?

  typedef typename
       return_type_2_ifthenelsereturn<
         2,
         pdalboost::is_convertible<plainA,plainB>::value, 
         pdalboost::is_convertible<plainB,plainA>::value,
         pdalboost::is_same<plainA,plainB>::value,
         plainA, 
         plainB>::type type;
};
...
struct return_type_2<other_action<ifthenelsereturn_action>, A, B> { 
  ...
  typedef typename 
    pdalboost::add_const<typename detail::same_or_not<A1, B1>::type>::type type;
};
...
public:
  ...
  template<class RET, CALL_TEMPLATE_ARGS>
  RET call(CALL_FORMAL_ARGS) const {
    return (detail::select(pdalboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) ?
       detail::select(pdalboost::tuples::get<1>(args), CALL_ACTUAL_ARGS) 
    : 
       detail::select(pdalboost::tuples::get<2>(args), CALL_ACTUAL_ARGS);
  }
