...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename pdalboost::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    pdalboost::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<int N, class T> struct tuple_element_as_reference {   
  typedef typename
     pdalboost::tuples::access_traits<
       typename pdalboost::tuples::element<N, T>::type
     >::non_const_type type;
};
...
struct parameter_traits_<pdalboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const pdalboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<volatile pdalboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const volatile pdalboost::reference_wrapper<T>, Any >{
  typedef T& type;
};
...
struct const_copy_argument {
  typedef typename 
    detail::parameter_traits_<
      T,
      typename detail::IF<pdalboost::is_function<T>::value, T&, const T>::RET
    >::type type;
};
...
class bind_type_generator {
  ...
  BOOST_STATIC_CONSTANT(int, nof_elems = pdalboost::tuples::length<args_t>::value);
