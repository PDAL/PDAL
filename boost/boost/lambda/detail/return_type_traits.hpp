...
using ::pdalboost::type_traits::ice_and;
using ::pdalboost::type_traits::ice_or;
using ::pdalboost::type_traits::ice_not;
...
namespace detail {
  ...
  class protect_conversion {
      typedef typename pdalboost::remove_reference<T>::type non_ref_T;
    public:

  // add const to rvalues, so that all rvalues are stored as const in 
  // the args tuple
    typedef typename detail::IF_type<
//      pdalboost::is_reference<T>::value && !pdalboost::is_const<non_ref_T>::value,
      ice_and<pdalboost::is_reference<T>::value,
              ice_not<pdalboost::is_const<non_ref_T>::value>::value>::value,
...
// experimental feature
  ...
  typedef typename pdalboost::remove_reference<A>::type non_ref_A;
  typedef typename pdalboost::remove_reference<B>::type non_ref_B;
