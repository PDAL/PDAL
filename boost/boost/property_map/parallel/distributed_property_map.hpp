...
using pdalboost::graph::parallel::trigger_receive_context;
 ...
 public:
  ...
  { 
    send(data->process_group, p, property_map_put, 
         pdalboost::parallel::detail::make_untracked_pair(k, v)); 
  }
 ...
 private:
  ...
  struct handle_message
  {
    ...
    handle_put(int source, int tag, 
               const pdalboost::parallel::detail::untracked_pair<key_type, value_type>& data, 
...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost {
  ...
  struct hash<pdalboost::detail::edge_desc_impl<D, V> >
  {
    std::size_t operator()(const pdalboost::detail::edge_desc_impl<D, V> & x) const
