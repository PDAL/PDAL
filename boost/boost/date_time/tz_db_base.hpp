...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost {
  namespace date_time {
    ...
    public:
      ...
      typedef pdalboost::date_time::dst_adjustment_offsets<time_duration_type> dst_adjustment_offsets;
      ...
      void load_from_file(const std::string& pathspec)
      {
        ...
        if(!ifs){
          pdalboost::throw_exception(data_not_accessible(pathspec));
        }
      ...
      bool add_record(const string_type& region, 
                      pdalboost::shared_ptr<time_zone_base_type> tz)
      {
      ...
      pdalboost::shared_ptr<time_zone_base_type> 
      time_zone_from_region(const string_type& region) const 
      {
        ...
        if(record == m_zone_map.end()){
          return pdalboost::shared_ptr<time_zone_base_type>(); //null pointer
        }
    ...
    private:
      typedef std::map<string_type, pdalboost::shared_ptr<time_zone_base_type> > map_type;
      map_type m_zone_map;
      ...
      {
        typedef pdalboost::char_separator<char_type, std::char_traits<char_type> > char_separator_type;
      ...
      bool parse_string(string_type& s)
      {
        ...
        typedef pdalboost::token_iterator_generator<pdalboost::escaped_list_separator<char_type>, string_type::const_iterator, string_type >::type token_iter_type;

        token_iter_type i = pdalboost::make_token_iterator<string_type>(s.begin(), s.end(),pdalboost::escaped_list_separator<char_type>());
        ...
        if (result.size() != expected_fields) { 
          ...
          pdalboost::throw_exception(bad_field_count(msg.str()));
        ...
        pdalboost::shared_ptr<rule_type> rules;
        ...
        if(has_dst){
          ...
          rules = 
            pdalboost::shared_ptr<rule_type>(parse_rules(result[START_DATE_RULE],
                                                     result[END_DATE_RULE]));
        }
        string_type id(result[ID]);
        pdalboost::shared_ptr<time_zone_base_type> zone(new time_zone_type(names, utc_offset, adjust, rules));
