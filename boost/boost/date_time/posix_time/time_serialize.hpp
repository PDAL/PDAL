...
BOOST_SERIALIZATION_SPLIT_FREE(pdalboost::posix_time::ptime)
BOOST_SERIALIZATION_SPLIT_FREE(pdalboost::posix_time::time_duration)
BOOST_SERIALIZATION_SPLIT_FREE(pdalboost::posix_time::time_period)
          ...
          unsigned int /*version*/)
{
  ...
  if(!d.is_special()) {
    ar & make_nvp("ptime_time_duration", td);
    pt = pdalboost::posix_time::ptime(d,td);
  }
  else {
    pt = pdalboost::posix_time::ptime(d.as_special());
  }
                                ...
                                const unsigned int /*file_version*/)
{
  ...
  new(pt) pdalboost::posix_time::ptime(pdalboost::posix_time::not_a_date_time);
}
...
template<class Archive>
void load(Archive & ar, 
          pdalboost::posix_time::time_period & tp, 
          unsigned int /*version*/)
{
  ...
  ar & make_nvp("time_period_begin", beg);
  ar & make_nvp("time_period_end", end);
  tp = pdalboost::posix_time::time_period(beg, end);
}
                                ...
                                const unsigned int /*file_version*/)
{
  ...
  posix_time::ptime beg(d,td);
  posix_time::ptime end(d,td);
  new(tp) pdalboost::posix_time::time_period(beg,end);
}
