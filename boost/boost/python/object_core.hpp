...
// attribute/item access or addition.
  ...
  //
  BOOST_PYTHON_IS_XXX_DEF(proxy, pdalboost::python::api::proxy, 1)
...
# define BOOST_PYTHON_FORWARD_OBJECT_CONSTRUCTORS_(derived, base)              \
    inline explicit derived(::pdalboost::python::detail::borrowed_reference p)     \
        : base(p) {}                                                           \
    inline explicit derived(::pdalboost::python::detail::new_reference p)          \
        : base(p) {}                                                           \
    inline explicit derived(::pdalboost::python::detail::new_non_null_reference p) \
        : base(p) {}
...
# endif
  ...
  struct object_initializer_impl<false, true>
  {
      ...
      get(T const& x, U)
      {
          return python::incref(get_managed_object(x, pdalboost::python::tag));
      }
  };
...
{ 
  ...
  return pdalboost::python::detail::args_proxy(x); 
} 
...
{ 
  ...
  PyObject *result = PyObject_Call(get_managed_object(self, pdalboost::python::tag), 
                                   args.operator object().ptr(), 
                                   0); 
  return object(pdalboost::python::detail::new_reference(result)); 
