...
{
    if (void* wrapped = holds_wrapped(dst_t, pdalboost::addressof(m_held), pdalboost::addressof(m_held)))
        return wrapped;
    
    type_info src_t = python::type_id<Value>();
    return src_t == dst_t ? pdalboost::addressof(m_held)
        : find_static_type(pdalboost::addressof(m_held), src_t, dst_t);
}
...
# endif
    ...
    {
        python::detail::initialize_wrapper(self, pdalboost::addressof(this->m_held));
    }
