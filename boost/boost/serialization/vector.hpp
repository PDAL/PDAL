...
// namespace - pdalboost::serialization
...
inline void save(
    ...
    pdalboost::serialization::stl::save_collection<Archive, STD::vector<U, Allocator> >(
        ar, t
    );
}
...
inline void load(
    ...
    pdalboost::serialization::stl::load_collection<
        Archive,
        std::vector<U, Allocator>,
        pdalboost::serialization::stl::archive_input_seq<
            Archive, STD::vector<U, Allocator> 
        >,
        pdalboost::serialization::stl::reserve_imp<STD::vector<U, Allocator> >
    >(ar, t);
}
...
inline void load(
    ...
    if (ar.get_library_version()==pdalboost::archive::library_version_type(5))
    {
      load(ar,t,file_version, pdalboost::is_arithmetic<U>());
      return;
    }
#endif
    typedef BOOST_DEDUCED_TYPENAME 
...
inline void save(
    ...
    while(count-- > 0){
        bool tb = *it++;
        ar << pdalboost::serialization::make_nvp("item", tb);
    }
}
...
inline void load(
    ...
    while(count-- > 0){
        bool i;
        ar >> pdalboost::serialization::make_nvp("item", i);
        t.push_back(i);
    }
}
