...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost {
    ...
    public:
        ...
        const charT& at(size_t pos) const {
            if ( pos >= len_ )
                BOOST_THROW_EXCEPTION( std::out_of_range ( "pdalboost::string_ref::at" ) );
            return ptr_[pos];
            }
...
namespace std {
    // Hashing
    template<> struct hash<pdalboost::string_ref>;
    template<> struct hash<pdalboost::u16string_ref>;
    template<> struct hash<pdalboost::u32string_ref>;
    template<> struct hash<pdalboost::wstring_ref>;
}
#endif
