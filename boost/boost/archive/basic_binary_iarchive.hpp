...
public:
    ...
    void load_override(class_id_reference_type & t, int version){
        ...
        if(pdalboost::archive::library_version_type(6) < lvt){
            int_least16_t x=0;
            * this->This() >> x;
            t = pdalboost::archive::class_id_reference_type(
                pdalboost::archive::class_id_type(x)
            );
        }
        else{
            int x=0;
    ...
    void load_override(pdalboost::serialization::item_version_type & t, int version){
        library_version_type lvt = this->get_library_version();
//        if(pdalboost::archive::library_version_type(7) < lvt){
        if(pdalboost::archive::library_version_type(6) < lvt){
            this->detail_common_iarchive::load_override(t, version);
