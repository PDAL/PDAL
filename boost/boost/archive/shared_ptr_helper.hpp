...
class shared_ptr_helper {
    ...
    typedef std::set<
        pdalboost::shared_ptr<const void>,
    ...
    struct void_deleter {
        ...
        void_deleter(const pdalboost::serialization::extended_type_info *eti) :
            m_eti(eti)
        {}
...
#else
    template<class Archive, class T>
    friend inline void pdalboost::serialization::load(
        Archive & ar,
        pdalboost::shared_ptr< T > &t,
        const unsigned int file_version
    );
...
//  #ifdef BOOST_SERIALIZATION_SHARED_PTR_132_HPP
    ...
    std::list<pdalboost_132::shared_ptr<const void> > * m_pointers_132;
//  #endif
    ...
    get_od(
        ...
        const pdalboost::serialization::extended_type_info * this_type
    );

    BOOST_ARCHIVE_DECL(void)
    append(const pdalboost::shared_ptr<const void> &);

    template<class T>
    struct non_polymorphic {
        static const pdalboost::serialization::extended_type_info * 
...
//  #ifdef BOOST_SERIALIZATION_SHARED_PTR_132_HPP
    BOOST_ARCHIVE_DECL(void)
    append(const pdalboost_132::shared_ptr<const void> & t);
//  #endif
public:
