...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost {
    ...
    struct dijkstra_bfs_visitor
    {
      ...
      void examine_edge(Edge e, Graph& g) {
        ...
        if (m_compare(m_combine(m_zero, get(m_weight, e)), m_zero)) 
            pdalboost::throw_exception(negative_edge());
  ...
  namespace detail {
    ...
    struct vertex_property_map_generator_helper<Graph, IndexMap, Value, true> {
      typedef pdalboost::iterator_property_map<Value*, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index, pdalboost::scoped_array<Value>& array_holder) {
    ...
    struct vertex_property_map_generator_helper<Graph, IndexMap, Value, false> {
      typedef pdalboost::vector_property_map<Value, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index, pdalboost::scoped_array<Value>& array_holder) {
        return pdalboost::make_vector_property_map<Value>(index);
      }
    };

    template <class Graph, class IndexMap, class Value>
    struct vertex_property_map_generator {
      typedef pdalboost::is_base_and_derived<
                pdalboost::vertex_list_graph_tag,
                typename pdalboost::graph_traits<Graph>::traversal_category>
              known_num_vertices;
  ...
  namespace detail {
    ...
    struct default_color_map_generator_helper<Graph, IndexMap, true> {
      typedef pdalboost::two_bit_color_map<IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        size_t nv = num_vertices(g);
        return pdalboost::two_bit_color_map<IndexMap>(nv, index);
      }
    };

    template <class Graph, class IndexMap>
    struct default_color_map_generator_helper<Graph, IndexMap, false> {
      typedef pdalboost::vector_property_map<pdalboost::two_bit_color_type, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        return pdalboost::make_vector_property_map<pdalboost::two_bit_color_type>(index);
      }
    };
     ...
     DijkstraVisitor vis, ColorMap color)
  {
    ...
    MutableQueue Q(num_vertices(g), icmp, index_map);
      ...
      pdalboost::scoped_array<std::size_t> index_in_heap_map_holder;
      typedef
  ...
  {
    pdalboost::two_bit_color_map<IndexMap> color(num_vertices(g), index_map);
     ...
     DijkstraVisitor vis, ColorMap color)
  {
    ...
    for (pdalboost::tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui) {
