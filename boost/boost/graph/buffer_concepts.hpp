...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost {
  ...
  BOOST_concept(Buffer, (B))
  {
    ...
    BOOST_CONCEPT_USAGE(Buffer) {
      typedef typename pdalboost::add_reference<value_type>::type reference;
      ...
      buf.pop();
      reference t = buf.top();
      pdalboost::ignore_unused_variable_warning(t);
    }
    
    void const_constraints(const B& cbuf) {
      typedef typename pdalboost::add_const<typename pdalboost::remove_reference<value_type>::type>::type& const_reference;
    ...
    : UpdatableQueue<Q>
  {
    ...
    BOOST_CONCEPT_USAGE(KeyedUpdatableQueue) {
      BOOST_CONCEPT_ASSERT((pdalboost::ReadWritePropertyMapConcept<key_map, typename Buffer<Q>::value_type>));
    }
