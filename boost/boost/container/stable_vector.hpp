...
//   node();
   ...
   public:
   typename ::pdalboost::intrusive::pointer_traits<Pointer>::element_type value;
};

template<typename Pointer, bool IsConst>
class iterator
{
   typedef pdalboost::intrusive::pointer_traits<Pointer>                                non_const_ptr_traits;
   public:
   ...
   typedef typename ::pdalboost::container::container_detail::if_c
      < IsConst
   ...
   {
      typedef pdalboost::intrusive::pointer_traits<pointer> ptr_traits;
...
struct index_traits
{
   ...
   typedef ::pdalboost::container::vector
...
class stable_vector
{
   ...
   typedef ::pdalboost::container::container_detail::
      integral_constant<unsigned, 1>                  allocator_v1;
   typedef ::pdalboost::container::container_detail::
      integral_constant<unsigned, 2>                  allocator_v2;
   typedef ::pdalboost::container::container_detail::integral_constant
      <unsigned, pdalboost::container::container_detail::
   ...
   stable_vector(BOOST_RV_REF(stable_vector) x)
      : internal_data(pdalboost::move(x.priv_node_alloc())), index(pdalboost::move(x.index))
   {
   ...
   stable_vector& operator=(BOOST_RV_REF(stable_vector) x)
   {
      if (&x != this){
         ...
         if(this_alloc == x_alloc){
            ...
            this->clear();
            this->index = pdalboost::move(x.index);
            this->priv_swap_members(x);
         ...
         //If unequal allocators, then do a one by one move
            ...
            this->assign( pdalboost::make_move_iterator(x.begin())
                        , pdalboost::make_move_iterator(x.end()));
         }
      }
      return *this;
   }
   ...
   void emplace_back(Args &&...args)
   {
      ...
      EmplaceFunctor &&ef = EmplaceFunctor(pdalboost::forward<Args>(args)...);
   ...
   void priv_push_back(BOOST_MOVE_CATCH_FWD(U) x)
   {
      if(this->priv_capacity_bigger_than_size()){
         ...
         BOOST_ASSERT(!!p);
         {
            ...
            //This might throw
            this->priv_build_node_from_convertible(p, ::pdalboost::forward<U>(x));
            rollback.release();
         }
         ...
         this->insert(this->cend(), ::pdalboost::forward<U>(x));
      }
   }
   ...
   iterator priv_insert(const_iterator position, BOOST_RV_REF(T) x)
   {
      ...
      typedef pdalboost::move_iterator<repeat_it>      repeat_move_it;
      //Just call more general insert(pos, size, value) and return iterator
   ...
   {
      //This can throw
      pdalboost::container::construct_in_place
   ...
   {
      //This can throw
      pdalboost::container::allocator_traits<node_allocator_type>::construct
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , ::pdalboost::forward<ValueConvertible>(value_convertible));
      //This does not throw
   ...
   {
      pdalboost::container::swap_dispatch(this->internal_data.pool_size, x.internal_data.pool_size);
      ...
      : public node_allocator_type
   {
      ...
      explicit ebo_holder(BOOST_FWD_REF(AllocatorRLValue) a)
         : node_allocator_type(pdalboost::forward<AllocatorRLValue>(a))
         , pool_size(0)
         , end_node()
      {}
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<pdalboost::container::stable_vector<T, Allocator> >
