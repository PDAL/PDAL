...
template<class T, bool IsClass = pdalboost::is_class<T>::value >
struct has_member_swap
{
   static const bool value = pdalboost::container::container_detail::
...
template<class T> inline
typename container_detail::enable_if_c
      <!container_swap::has_member_swap<T>::value && pdalboost::has_move_emulation_enabled<T>::value, void>::type
   swap_dispatch(T &left, T &right)
{
   T temp(pdalboost::move(left)); // may throw
   left = pdalboost::move(right); // may throw
   right = pdalboost::move(temp); // may throw
}
...
{  return pdalboost::container::container_detail::to_raw_pointer(p.operator->());  }
...
{  pdalboost::container::swap_dispatch(l, r);   }
...
struct is_memcpy_copy_assignable
{
   static const bool value = are_contiguous_and_same<I, O>::value &&
      pdalboost::has_trivial_assign< typename ::std::iterator_traits<I>::value_type >::value;
};
...
{
   static const bool value = are_contiguous_and_same<I, O>::value &&
      pdalboost::has_trivial_copy< typename ::std::iterator_traits<I>::value_type >::value;
};
   ...
   uninitialized_move_alloc(A &a, I f, I l, F r)
{
   ...
   BOOST_TRY{
      while (f != l) {
         allocator_traits<A>::construct(a, container_detail::to_raw_pointer(&*r), pdalboost::move(*f));
         ++f; ++r;
      }
   }
   BOOST_CATCH(...){
...
inline void destroy_alloc_n(A &a, I f, typename std::iterator_traits<I>::difference_type n
   ,typename pdalboost::container::container_detail::enable_if_c
      < !pdalboost::has_trivial_destructor<typename std::iterator_traits<I>::value_type>::value >::type* = 0)
{
   while(n--){
                    ...
                    , G large_range_f, typename allocator_traits<A>::size_type n_j)
{
   ...
   for (; n != n_i ; ++short_range_f, ++large_range_f, ++n){
      pdalboost::container::swap_dispatch(*short_range_f, *large_range_f);
   }
