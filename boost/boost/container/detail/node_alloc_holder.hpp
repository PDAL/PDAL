...
struct node_alloc_holder
{
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x)
      : members_(pdalboost::move(x.node_alloc()))
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x, const ValPred &c)
      : members_(pdalboost::move(x.node_alloc()), c)
   {  this->icont().swap(x.icont());  }
   ...
   NodePtr create_node(Args &&...args)
   {
      ...
      allocator_traits<NodeAlloc>::construct
         ( this->node_alloc()
         , container_detail::addressof(p->m_data), pdalboost::forward<Args>(args)...);
   ...
   NodePtr create_node_from_it(const It &it)
   {
      NodePtr p = this->allocate_one();
      Deallocator node_deallocator(p, this->node_alloc());
      ::pdalboost::container::construct_in_place(this->node_alloc(), container_detail::addressof(p->m_data), it);
      node_deallocator.release();
      //This does not throw
      typedef typename Node::hook_type hook_type;
      ::new(static_cast<hook_type*>(container_detail::to_raw_pointer(p))) hook_type;
      ...
      if(n){
         ...
         BOOST_TRY{
            ...
            while(n--){
               ...
               ++itbeg;
               //This can throw
               pdalboost::container::construct_in_place(nalloc, container_detail::addressof(p->m_data), beg);
               sdestructor.set(p);
               ++beg;
               //This does not throw
               typedef typename Node::hook_type hook_type;
   ...
   void clear(allocator_v2)
   {
      ...
      //BOOST_STATIC_ASSERT((::pdalboost::has_move_emulation_enabled<typename NodeAlloc::multiallocation_chain>::value == true));
      if(!chain.empty())
      ...
      :  public NodeAlloc
   {
      ...
      explicit members_holder(BOOST_FWD_REF(ConvertibleToAlloc) c2alloc)
         :  NodeAlloc(pdalboost::forward<ConvertibleToAlloc>(c2alloc))
         , m_icont()
      {}
