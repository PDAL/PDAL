...
template<class Allocator, unsigned Version = pdalboost::container::container_detail::version<Allocator>::value>
...
struct allocator_version_traits<Allocator, 1>
{
   ...
   typedef typename pdalboost::intrusive::pointer_traits<pointer>::
   ...
   static void deallocate_individual(Allocator &a, multiallocation_chain &holder)
   {
      ...
      while(n--){
         pointer p = pdalboost::intrusive::pointer_traits<pointer>::pointer_to(*it);
         ++it;
         a.deallocate(p, 1);
      }
   }
