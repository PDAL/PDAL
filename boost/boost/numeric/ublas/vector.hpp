...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost { namespace numeric { namespace ublas {
    ...
    /// vector into a normal vector. It must first be assigned to another normal vector by any suitable means. Its memory footprint is constant.
    /// \tparam T type of the objects stored in the vector: it can be anything even if most of the time, scalar types will be used like \c double or \c int. Complex types can be used, or even classes like pdalboost::interval.
    template<class T, class ALLOC>
    class scalar_vector:
