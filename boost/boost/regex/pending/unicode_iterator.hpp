...
public:
   ...
   void increment()
   {
      ...
      if((static_cast<pdalboost::uint8_t>(*m_position) & 0xC0) == 0x80)
         invalid_sequence();
      // skip high surrogate first if there is one:
      ...
      if(m_value == pending_read)
      {
         ...
         for(unsigned i = 0; i < c; ++i)
         {
            ...
            if((i != c - 1) && ((static_cast<pdalboost::uint8_t>(*m_position) & 0xC0) != 0x80))
               invalid_sequence();
         }
      }
      else
...
private:
   ...
   void extract_current()const
   {
      ...
      for(unsigned c = 0; c < extra; ++c)
      {
         ...
         m_value <<= 6;
         // We must have a continuation byte:
         if((static_cast<pdalboost::uint8_t>(*next) & 0xC0) != 0x80)
            invalid_sequence();
