...
{
   std::size_t r = pdalboost::hash_range(i, j);
...
struct regex_data : public named_subexpressions
{
   ...
   regex_data(const ::pdalboost::shared_ptr<
      ::pdalboost::regex_traits_wrapper<traits> >& t) 
      : m_ptraits(t), m_expression(0), m_expression_len(0) {}
   regex_data() 
      : m_ptraits(new ::pdalboost::regex_traits_wrapper<traits>()), m_expression(0), m_expression_len(0) {}

   ::pdalboost::shared_ptr<
      ::pdalboost::regex_traits_wrapper<traits>
...
public:
   ...
   basic_regex_implementation(const ::pdalboost::shared_ptr<
      ::pdalboost::regex_traits_wrapper<traits> >& t)
   ...
   {
      if(n == 0)
         pdalboost::throw_exception(std::out_of_range("0 is not a valid subexpression index."));
   ...
   const ::pdalboost::regex_traits_wrapper<traits>& get_traits()const
   {
...
public:
   ...
   {
      if(!m_pimpl.get())
         pdalboost::throw_exception(std::logic_error("Can't access subexpressions in an invalid regex."));
   ...
   pdalboost::shared_ptr<re_detail::named_subexpressions > get_named_subs()const
   {
      return m_pimpl;
   }
