...
typedef pdalboost::tokenizer<pdalboost::char_separator<char> > tokenizer;
...
struct parsing_assigner
{
            ...
            Point& point, std::string const& wkt)
    {
        ...
        catch(pdalboost::bad_lexical_cast const& blc)
        {
...
struct container_appender
{
    typedef typename geometry::point_type
        <
            typename pdalboost::remove_reference<Geometry>::type
        >::type point_type;
...
{
    it = tokens.begin();
    if (it != tokens.end() && pdalboost::iequals(*it++, geometry_name))
    {
...
struct box_parser
{
    static inline void apply(std::string const& wkt, Box& box)
    {
        ...
        if (it != end && pdalboost::iequals(*it, "POLYGON"))
        {
            ++it;
            bool has_empty, has_z, has_m;
        ...
        else if (it != end && pdalboost::iequals(*it, "BOX"))
        {
            ++it;
        }
        else
...
struct segment_parser
{
    static inline void apply(std::string const& wkt, Segment& segment)
    {
        ...
        tokenizer::iterator end = tokens.end();
        if (it != end &&
            (pdalboost::iequals(*it, "SEGMENT")
            || pdalboost::iequals(*it, "LINESTRING") ))
        {
            ++it;
        }
        else
