...
struct svg_range
{
        ...
        Range const& range, std::string const& style, int )
    {
        ...
        for (iterator it = pdalboost::begin(range);
...
struct svg_poly
{
        ...
        Polygon const& polygon, std::string const& style, int )
    {
        ...
        typedef typename pdalboost::range_iterator<ring_type const>::type iterator_type;

        bool first = true;
        os << "<g fill-rule=\"evenodd\"><path d=\"";

        ring_type const& ring = geometry::exterior_ring(polygon);
        for (iterator_type it = pdalboost::begin(ring);
            it != pdalboost::end(ring);
            ++it, first = false)
        {
            os << (first ? "M" : " L") << " "
                << geometry::get<0>(*it)
        ...
        // Inner rings:
        {
            ...
            for (BOOST_AUTO_TPL(rit, pdalboost::begin(rings));
                rit != pdalboost::end(rings); ++rit)
            {
                first = true;
                for (BOOST_AUTO_TPL(it, pdalboost::begin(*rit)); it != pdalboost::end(*rit);
                    ++it, first = false)
                {
                    os << (first ? "M" : " L") << " "
                        << geometry::get<0>(*it)
