...
struct path_intersection<Indexable, Linestring, box_tag, linestring_tag>
{
    ...
    {
        typedef typename ::pdalboost::range_value<Linestring>::type point_type;
        typedef typename ::pdalboost::range_const_iterator<Linestring>::type const_iterator;        
        typedef typename ::pdalboost::range_size<Linestring>::type size_type;
        
        const size_type count = ::pdalboost::size(path);

        if ( count == 2 )
        {
            return index::detail::segment_intersection(b, *::pdalboost::begin(path), *(::pdalboost::begin(path)+1), comparable_distance);
        }
        else if ( 2 < count )
        {
            const_iterator it0 = ::pdalboost::begin(path);
            const_iterator it1 = ::pdalboost::begin(path) + 1;
            const_iterator last = ::pdalboost::end(path);
