...
{
    typedef pdalboost::container::vector<NewValue> type;
};
...
public:
    ...
    inline allocators(BOOST_FWD_REF(allocators) a)
        : internal_node_allocator_type(pdalboost::move(a.internal_node_allocator()))
        , leaf_allocator_type(pdalboost::move(a.leaf_allocator()))
    {}

    inline allocators & operator=(BOOST_FWD_REF(allocators) a)
    {
        internal_node_allocator() = ::pdalboost::move(a.internal_node_allocator());
        leaf_allocator() = ::pdalboost::move(a.leaf_allocator());
        return *this;
    }
    ...
    {
        pdalboost::swap(internal_node_allocator(), a.internal_node_allocator());
        pdalboost::swap(leaf_allocator(), a.leaf_allocator());
    }
...
struct create_dynamic_node
{
    ...
    {
        typedef pdalboost::container::allocator_traits<AllocNode> Al;
        typedef typename Al::pointer P;

        P p = Al::allocate(alloc_node, 1);

        if ( 0 == p )
            throw_runtime_error("pdalboost::geometry::index::rtree node creation failed");

        auto_deallocator<AllocNode> deallocator(alloc_node, p);

        Al::construct(alloc_node, pdalboost::addressof(*p), alloc_node);
...
struct destroy_dynamic_node
{
    ...
    static inline void apply(AllocNode & alloc_node, BaseNodePtr n)
    {
        ...
        Al::destroy(alloc_node, pdalboost::addressof(*p));
