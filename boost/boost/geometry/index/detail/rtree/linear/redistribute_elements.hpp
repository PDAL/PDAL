...
inline R difference_dispatch(T const& from, T const& to, ::pdalboost::mpl::bool_<false> const& /*is_unsigned*/)
{
    return to - from;
}

template <typename R, typename T>
inline R difference_dispatch(T const& from, T const& to, ::pdalboost::mpl::bool_<true> const& /*is_unsigned*/)
{
...
{
    BOOST_MPL_ASSERT_MSG(!pdalboost::is_unsigned<R>::value, RESULT_CANT_BE_UNSIGNED, (R));

    typedef ::pdalboost::mpl::bool_<
        pdalboost::is_unsigned<T>::value
    > is_unsigned;
...
struct find_greatest_normalized_separation<Elements, Parameters, Translator, box_tag, DimensionIndex>
{
    ...
    typedef typename pdalboost::mpl::if_c<
        pdalboost::is_integral<coordinate_type>::value,
        double,
        coordinate_type
