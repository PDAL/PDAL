...
struct get_extremes
{
    ...
    inline void apply(InputRange const& range)
    {
        ...
        RangeIterator left_it = pdalboost::begin(range);
        RangeIterator right_it = pdalboost::begin(range);

        for (RangeIterator it = pdalboost::begin(range) + 1;
...
struct assign_range
{
    ...
    inline void apply(InputRange const& range)
    {
        ...
        for (RangeIterator it = pdalboost::begin(range);
            it != pdalboost::end(range);
            ++it)
        {
...
{
    typedef typename pdalboost::range_value<Range>::type point_type;
    typedef geometry::less<point_type> comparator;

    std::sort(pdalboost::begin(range), pdalboost::end(range), comparator());
}
...
public:
    ...
    inline void apply(InputGeometry const& geometry, partitions& state) const
    {
        ...
        //std::cout << pdalboost::size(assigner.lower_points) << std::endl;
        //std::cout << pdalboost::size(assigner.upper_points) << std::endl;
...
private:
        ...
        OutputIterator out, bool skip_first)
    {
        ...
        bool first = true;
        for (typename pdalboost::range_iterator<view_type const>::type it = pdalboost::begin(view);
            it != pdalboost::end(view); ++it)
        {
            if (first && skip_first)
            {
                first = false;
