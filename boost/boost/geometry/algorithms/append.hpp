...
struct append_range
{
    ...
    static inline void apply(Geometry& geometry, Range const& range,
                int = 0, int = 0)
    {
        for (typename pdalboost::range_iterator<Range const>::type
            it = pdalboost::begin(range);
...
struct devarianted_append<pdalboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    template <typename RangeOrPoint>
    struct visitor: pdalboost::static_visitor<void>
    {
    ...
    template <typename RangeOrPoint>
    static inline void apply(pdalboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>& variant_geometry,
                             RangeOrPoint const& range_or_point,
                             int ring_index,
                             int multi_index)
    {
