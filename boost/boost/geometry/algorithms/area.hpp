...
struct ring_area
{
    ...
    apply(Ring const& ring, Strategy const& strategy)
    {
        ...
        typedef typename closeable_view
            ...
            >::type view_type;
        typedef typename pdalboost::range_iterator<view_type const>::type iterator_type;

        rview_type rview(ring);
        view_type view(rview);
        typename Strategy::state_type state;
        iterator_type it = pdalboost::begin(view);
        iterator_type end = pdalboost::end(view);
...
struct area<pdalboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>, void>
{
    template <typename Strategy>
    struct visitor: pdalboost::static_visitor<typename Strategy::return_type>
    {
    ...
    {
        return pdalboost::apply_visitor(visitor<Strategy>(strategy), variant_geometry);
    }
};
