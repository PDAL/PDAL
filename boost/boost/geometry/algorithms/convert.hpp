...
struct point_to_box
{
    static inline void apply(Point const& point, Box& box)
    {
        ...
        set<Index, Dimension>(box,
                pdalboost::numeric_cast<coordinate_type>(get<Dimension>(point)));
        point_to_box
            <
                Point, Box,
...
struct segment_to_range
{
    static inline void apply(Segment const& segment, Range& range)
    {
        ...
        typename pdalboost::range_iterator<Range>::type it = pdalboost::begin(range);
...
<
    ...
    bool UseAssignment = pdalboost::is_same<Geometry1, Geometry2>::value
                         && !pdalboost::is_array<Geometry1>::value
>
...
struct devarianted_convert<pdalboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>, Geometry2>
{
