...
inline bool has_only_turns(Turns const& turns)
{
    ...
    typedef typename pdalboost::range_iterator<Turns const>::type iterator_type;
    for (iterator_type it = pdalboost::begin(turns); it != pdalboost::end(turns); ++it)
    {
