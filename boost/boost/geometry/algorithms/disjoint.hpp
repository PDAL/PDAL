...
struct disjoint_linestring_box
{
    static inline bool apply(Linestring const& linestring, Box const& box)
    {
        ...
        const size_type count = ::pdalboost::size(linestring);

        if ( count == 0 )
            return false;
        else if ( count == 1 )
            return detail::disjoint::point_box<point_type, Box, 0, dimension<point_type>::value>
                   ::apply(*::pdalboost::begin(linestring), box);
        else
        {
            const_iterator it0 = ::pdalboost::begin(linestring);
            const_iterator it1 = ::pdalboost::begin(linestring) + 1;
            const_iterator last = ::pdalboost::end(linestring);
