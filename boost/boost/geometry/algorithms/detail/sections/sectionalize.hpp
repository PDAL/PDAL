...
struct sectionalize_part
{
    ...
    typedef typename pdalboost::range_value<Sections>::type section_type;

    typedef typename pdalboost::range_iterator<Range const>::type iterator_type;
    ...
    static inline void apply(Sections& sections, section_type& section,
                ...
                ring_identifier ring_id)
    {
        if (int(pdalboost::size(range)) <= index)
        {
            return;
        }
        ...
        iterator_type it = pdalboost::begin(range);
        it += index;
            ...
            ++previous, ++it, index++)
        {
            ...
            if (section.count == 0)
            {
                ...
                section.range_count = pdalboost::size(range);
...
struct sectionalize_range
{
                ...
                ring_identifier ring_id)
    {
        ...
        std::size_t const n = pdalboost::size(view);
        if (n == 0)
        {
...
inline void set_section_unique_ids(Sections& sections)
{
    // Set ID's.
    int index = 0;
    for (typename pdalboost::range_iterator<Sections>::type it = pdalboost::begin(sections);
        it != pdalboost::end(sections);
        ++it)
    {
        it->id = index++;
    }
