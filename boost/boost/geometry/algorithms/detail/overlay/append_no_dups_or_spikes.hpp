...
inline void append_no_dups_or_spikes(Range& range, Point const& point)
{
    ...
    while(pdalboost::size(range) >= 3 
            && point_is_spike_or_equal(point, *(pdalboost::end(range) - 3), *(pdalboost::end(range) - 2)))
    {
        // Use the Concept/traits, so resize and append again
        traits::resize<Range>::apply(range, pdalboost::size(range) - 2);
        traits::push_back<Range>::apply(range, point);
    }

    // There might still be one duplicate not catched by the condition above
    if (pdalboost::size(range) == 2
        && geometry::detail::equals::equals_point_point(*pdalboost::begin(range), point))
    {
