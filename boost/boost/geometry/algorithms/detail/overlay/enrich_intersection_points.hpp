...
inline void update_discarded(Turns& turn_points, Operations& operations)
{
    ...
    for (typename pdalboost::range_iterator<Operations>::type it = pdalboost::begin(operations);
         it != pdalboost::end(operations);
         ++it)
    {
            ...
            Strategy const& strategy)
{
    ...
    std::sort(pdalboost::begin(operations),
                pdalboost::end(operations),
    ...
    if (clustered)
    {
        typedef typename pdalboost::range_iterator<Container>::type nc_iterator;
        nc_iterator it = pdalboost::begin(operations);
        nc_iterator begin_cluster = pdalboost::end(operations);
            ...
            prev = it++)
        {
            ...
            {
                if (begin_cluster == pdalboost::end(operations))
                {
                    begin_cluster = prev;
                }
            }
            else if (begin_cluster != pdalboost::end(operations))
            {
                handle_cluster<IndexType, Reverse1, Reverse2>(begin_cluster, it, turn_points,
                        for_operation, geometry1, geometry2, strategy);
                begin_cluster = pdalboost::end(operations);
            }
        }
        if (begin_cluster != pdalboost::end(operations))
        {
...
inline void enrich_discard(Container& operations, TurnPoints& turn_points)
{
    ...
    operations.erase(
            std::remove_if(pdalboost::begin(operations),
                    pdalboost::end(operations),
                    predicate),
            pdalboost::end(operations));
}
...
{
    typedef typename IndexType::type operations_type;
    typedef typename pdalboost::range_iterator<Container const>::type iterator_type;
    ...
    if (operations.size() > 0)
    {
        ...
        bool first = true;
        iterator_type it = pdalboost::begin(operations);
        for (iterator_type prev = it + (pdalboost::size(operations) - 1);
             it != pdalboost::end(operations);
             prev = it++)
        {
...
#ifdef BOOST_GEOMETRY_DEBUG_ENRICH
    {
        for (iterator_type it = pdalboost::begin(operations);
             it != pdalboost::end(operations);
             ++it)
        {
            operations_type& op = turn_points[it->index]
            ...
            std::cout << it->index
                ...
                << " seg: " << op.seg_id
                << " dst: " << pdalboost::numeric_cast<double>(op.enriched.distance)
...
{
    typedef typename pdalboost::range_value<TurnPoints>::type turn_point_type;
    typedef typename turn_point_type::container_type container_type;

    int index = 0;
    for (typename pdalboost::range_iterator<TurnPoints const>::type
            it = pdalboost::begin(turn_points);
         it != pdalboost::end(turn_points);
         ++it, ++index)
    {
        ...
        {
            int op_index = 0;
            for (typename pdalboost::range_iterator<container_type const>::type
                    op_it = pdalboost::begin(it->operations);
                op_it != pdalboost::end(it->operations);
                ++op_it, ++op_index)
            {
    ...
    Strategy const& strategy)
{
    ...
    for (typename pdalboost::range_iterator<TurnPoints>::type
            it = pdalboost::begin(turn_points);
         it != pdalboost::end(turn_points);
         ++it)
    {
