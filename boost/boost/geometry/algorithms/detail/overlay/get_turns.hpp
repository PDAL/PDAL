...
class get_turns_in_sections
{
            ...
            int index1, int index2)
    {
        ...
        pdalboost::ignore_unused_variable_warning(index1);
        pdalboost::ignore_unused_variable_warning(index2);

        return pdalboost::is_same
                    <
                        typename tag_cast
                            <
...
public :
    ...
    {
        pdalboost::ignore_unused_variable_warning(interrupt_policy);
        ...
        range1_iterator begin_range_1 = pdalboost::begin(view1);
        range1_iterator end_range_1 = pdalboost::end(view1);

        range2_iterator begin_range_2 = pdalboost::begin(view2);
        range2_iterator end_range_2 = pdalboost::end(view2);
            ...
            ++prev1, ++it1, ++index1, ++next1, ++ndi1)
        {
                ...
                ++prev2, ++it2, ++index2, ++next2, ++ndi2)
            {
                ...
                if (! skip)
                {
                    ...
                    typedef typename pdalboost::range_value<Turns>::type turn_info;
                    ...
                    std::size_t const size_before = pdalboost::size(turns);
                    ...
                    if (InterruptPolicy::enabled)
                    {
                        if (interrupt_policy.apply(
                            std::make_pair(pdalboost::begin(turns) + size_before,
                                pdalboost::end(turns))))
                        {
                            return false;
                        }
...
private :
    ...
    static inline void get_start_point_iterator(Section & section,
            Range const& range,
            typename pdalboost::range_iterator<Range const>::type& it,
            typename pdalboost::range_iterator<Range const>::type& prev,
            typename pdalboost::range_iterator<Range const>::type& end,
            int& index, int& ndi,
            int dir, Box const& other_bounding_box)
    {
        it = pdalboost::begin(range) + section.begin_index;
        end = pdalboost::begin(range) + section.end_index + 1;
...
public:
            ...
            Turns& turns, InterruptPolicy& interrupt_policy)
    {
        ...
        typedef typename pdalboost::range_value<Turns>::type ip_type;
...
struct get_turns_cs
{
    ...
    {
        if (pdalboost::size(range) <= 1)
        {
            return;
        }

        pdalboost::array<box_point_type,4> bp;
        assign_box_corners_oriented<ReverseBox>(box, bp);

        cview_type cview(range);
        view_type view(cview);

        iterator_type it = pdalboost::begin(view);

        ever_circling_iterator<iterator_type> next(
                pdalboost::begin(view), pdalboost::end(view), it, true);
        next++;
        next++;
...
struct get_turns_polygon_cs
{
            ...
            int multi_index = -1)
    {
        ...
        for (BOOST_AUTO_TPL(it, pdalboost::begin(rings)); it != pdalboost::end(rings);
            ++it, ++i)
        {
            intersector_type::apply(
                    source_id1, *it,
            ...
            InterruptPolicy& interrupt_policy)
{
    ...
    //        Geometry1,
    //        Geometry2,
    //        typename pdalboost::range_value<Turns>::type
