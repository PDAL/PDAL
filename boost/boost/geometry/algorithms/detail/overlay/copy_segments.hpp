...
struct copy_segments_ring
{
    ...
    typedef typename pdalboost::range_iterator<rview_type const>::type iterator;
            ...
            RangeOut& current_output)
    {
        ...
        // Sanity check
        BOOST_ASSERT(from_index < int(pdalboost::size(view)));

        ec_iterator it(pdalboost::begin(view), pdalboost::end(view),
                    pdalboost::begin(view) + from_index);

        // [2..4] -> 4 - 2 + 1 = 3 -> {2,3,4} -> OK
        // [4..2],size=6 -> 6 - 4 + 2 + 1 = 5 -> {4,5,0,1,2} -> OK
        // [1..1], travel the whole ring round
        typedef typename pdalboost::range_difference<Ring>::type size_type;
        size_type const count = from_index <= to_index
            ? to_index - from_index + 1
            : int(pdalboost::size(view)) - from_index + to_index + 1;
...
struct copy_segments_linestring
{
    ...
    typedef typename pdalboost::range_iterator<LineString const>::type iterator;
            ...
            RangeOut& current_output)
    {
        ...
        // Sanity check
        if (from_index > to_index || from_index < 0 || to_index >= int(pdalboost::size(ls)))
        {
            return;
        }

        typedef typename pdalboost::range_difference<LineString>::type size_type;
        size_type const count = to_index - from_index + 1;

        typename pdalboost::range_iterator<LineString const>::type it = pdalboost::begin(ls) + from_index;
...
struct copy_segments_box
{
            ...
            RangeOut& current_output)
    {
        ...
        pdalboost::array<typename point_type<Box>::type, 5> bp;
