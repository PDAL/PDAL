                ...
                std::string const& header)
{
    ...
    if (pdalboost::contains(header, "Finished"))
    {
        std::cout << std::endl;
    }
}
...
inline void set_visited_for_continue(Info& info, Turn const& turn)
{
    ...
    if (turn.operation == detail::overlay::operation_continue)
    {
        for (typename pdalboost::range_iterator
            ...
            >::type it = pdalboost::begin(info.operations);
            it != pdalboost::end(info.operations);
            ++it)
        {
            if (it->visited.none())
            {
...
inline bool assign_next_ip(G1 const& g1, G2 const& g2,
            Turns& turns,
            typename pdalboost::range_iterator<Turns>::type& ip,
            ...
            segment_identifier& seg_id)
{
    ...
    if (info.enriched.next_ip_index < 0)
    {
        ...
        seg_id = info.seg_id;
        ip = pdalboost::begin(turns) + info.enriched.travels_to_ip_index;
    }
    else
    {
        ip = pdalboost::begin(turns) + info.enriched.next_ip_index;
        seg_id = info.seg_id;
    }
            ...
            Iterator& selected)
{
    ...
    bool has_tp = false;
    selected = pdalboost::end(turn.operations);
    for (Iterator it = pdalboost::begin(turn.operations);
        it != pdalboost::end(turn.operations);
        ++it)
    {
...
public :
    ...
    {
        typedef typename pdalboost::range_value<Rings>::type ring_type;
        typedef typename pdalboost::range_iterator<Turns>::type turn_iterator;
        typedef typename pdalboost::range_value<Turns>::type turn_type;
        ...
        std::size_t size_at_start = pdalboost::size(rings);
        ...
        do
        {
            ...
            for (turn_iterator it = pdalboost::begin(turns);
                state.good() && it != pdalboost::end(turns);
                ++it)
            {
                // Skip discarded ones
                if (! (it->is_discarded() || it->blocked()))
                {
                    for (turn_operation_iterator_type iit = pdalboost::begin(it->operations);
                        state.good() && iit != pdalboost::end(it->operations);
                        ++iit)
                    {
                        if (iit->visited.none()
