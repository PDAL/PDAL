...
public :
                ...
                Turns& turns, OutputIterator out)
    {
        ...
        std::sort(pdalboost::begin(turns), pdalboost::end(turns), sort_on_segment<turn_type>());
        ...
        bool entered = false;
        bool first = true;
        for (turn_iterator it = pdalboost::begin(turns); it != pdalboost::end(turns); ++it)
        {
            turn_operation_iterator_type iit = pdalboost::begin(it->operations);
