...
struct intersection_linestring_linestring_point
{
            ...
            Strategy const& )
    {
        ...
        for (typename pdalboost::range_iterator<std::deque<turn_info> const>::type
            it = pdalboost::begin(turns); it != pdalboost::end(turns); ++it)
        {
            PointOut p;
...
struct intersection_of_linestring_with_areal
{
    ...
    static inline OutputIterator apply(LineString const& linestring, Areal const& areal,
            ...
            Strategy const& )
    {
        if (pdalboost::size(linestring) == 0)
        {
            return out;
        }
