...
{
    typedef typename pdalboost::range_value<Collection>::type item_type;
    typedef typename item_type::type calculation_type;

    static inline void apply(Collection& collection, Range const& range)
    {
        if (pdalboost::size(range) < 2)
        {
            return;
        }

        typedef typename pdalboost::range_iterator<Range const>::type iterator;

        bool first = true;
        iterator it = pdalboost::begin(range);

        for (iterator prev = it++;
            it != pdalboost::end(range);
            prev = it++)
        {
            typename pdalboost::range_value<Collection>::type v;
            ...
            calculation_type magnitude = sqrt(
                pdalboost::numeric_cast<calculation_type>(v.dx * v.dx + v.dy * v.dy));
        ...
        if (pdalboost::size(collection) > 1 
...
struct box_collect_vectors
{
    ...
    static inline void apply(Collection& collection, Box const& box)
    {
        ...
        typedef typename pdalboost::range_value<Collection>::type item;
...
struct polygon_collect_vectors
{
    static inline void apply(Collection& collection, Polygon const& polygon)
    {
        ...
        for (BOOST_AUTO_TPL(it, pdalboost::begin(rings)); it != pdalboost::end(rings); ++it)
        {
...
struct multi_collect_vectors
{
    ...
    {
        for (typename pdalboost::range_iterator<MultiGeometry const>::type
                it = pdalboost::begin(multi);
            it != pdalboost::end(multi);
            ++it)
        {
...
struct collect_vectors<multi_polygon_tag, Collection, MultiPolygon>
    : detail::collect_vectors::multi_collect_vectors
        <
            ...
            <
                typename pdalboost::range_value<MultiPolygon>::type,
                Collection
            >
        >
{};
