...
{
    typedef pdalboost::range_iterator
        <
            index_vector_type const
        >::type index_iterator_type;

    for(index_iterator_type it = pdalboost::begin(input);
        it != pdalboost::end(input);
        ++it)
    {
...
static inline void handle_one(InputCollection const& collection,
        ...
        Policy& policy)
{
    typedef pdalboost::range_iterator<index_vector_type const>::type
                index_iterator_type;
    // Quadratic behaviour at lowest level (lowest quad, or all exceeding)
    for(index_iterator_type it1 = pdalboost::begin(input);
        it1 != pdalboost::end(input);
        ++it1)
    {
        index_iterator_type it2 = it1;
        for(++it2; it2 != pdalboost::end(input); ++it2)
        {
        ...
        Policy& policy)
{
    ...
    for(index_iterator_type it1 = pdalboost::begin(input1);
        it1 != pdalboost::end(input1);
        ++it1)
    {
        for(index_iterator_type it2 = pdalboost::begin(input2);
            it2 != pdalboost::end(input2);
            ++it2)
        {
...
class partition_one_collection
{
    ...
    {
        if (pdalboost::size(input) > 0)
        {
            if (std::size_t(pdalboost::size(input)) > min_elements && level < 100)
            {
...
class partition_two_collections
{
    ...
    static inline void next_level(Box const& box,
            ...
            Policy& policy, VisitBoxPolicy& box_policy)
    {
        if (pdalboost::size(input1) > 0 && pdalboost::size(input2) > 0)
        {
            if (std::size_t(pdalboost::size(input1)) > min_elements
                && std::size_t(pdalboost::size(input2)) > min_elements
                && level < 100)
            {
...
public :
            ...
            Policy& policy, VisitBoxPolicy& box_policy)
    {
        ...
        if (pdalboost::size(exceeding1) > 0)
        {
        ...
        if (pdalboost::size(exceeding2) > 0)
        {
...
class partition
{
    ...
    {
        std::size_t index = 0;
        for(typename pdalboost::range_iterator<InputCollection const>::type it
            = pdalboost::begin(collection);
            it != pdalboost::end(collection);
            ++it, ++index)
        {
...
public :
    ...
    {
        if (std::size_t(pdalboost::size(collection)) > min_elements)
        {
        ...
        else
        {
            typedef typename pdalboost::range_iterator
                ...
                >::type iterator_type;
            for(iterator_type it1 = pdalboost::begin(collection);
                it1 != pdalboost::end(collection);
                ++it1)
            {
                iterator_type it2 = it1;
                for(++it2; it2 != pdalboost::end(collection); ++it2)
                {
    ...
    {
        if (std::size_t(pdalboost::size(collection1)) > min_elements
            && std::size_t(pdalboost::size(collection2)) > min_elements)
        {
        ...
        else
        {
            typedef typename pdalboost::range_iterator
                <
                    InputCollection const
                >::type iterator_type;
            for(iterator_type it1 = pdalboost::begin(collection1);
                it1 != pdalboost::end(collection1);
                ++it1)
            {
                for(iterator_type it2 = pdalboost::begin(collection2);
                    it2 != pdalboost::end(collection2);
                    ++it2)
                {
                    visitor.apply(*it1, *it2);
                }
