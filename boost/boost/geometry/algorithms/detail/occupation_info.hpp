...
inline Iterator advance_circular(Iterator it, Vector const& vector, segment_identifier& seg_id, bool forward = true)
{
	...
	if (it == pdalboost::begin(vector) && increment < 0)
	{
		it = pdalboost::end(vector);
        seg_id.segment_index = pdalboost::size(vector);
	}
	it += increment;
    seg_id.segment_index += increment;
	if (it == pdalboost::end(vector))
	{
        seg_id.segment_index = 0;
		it = pdalboost::begin(vector);
	}
	return it;
}
                ...
                Info& info)
{
	...
	int const n = pdalboost::size(ring);
	...
	iterator_type it = pdalboost::begin(ring) + seg_id.segment_index;
