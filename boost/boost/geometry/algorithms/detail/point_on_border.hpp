...
struct point_on_range
{
    ...
    {
        const std::size_t n = pdalboost::size(range);
        if (midpoint && n > 1)
        {
            ...
            iterator prev = it++;
            while (it != pdalboost::end(range)
                && detail::equals::equals_point_point(*it, *prev))
            {
                prev = it++;
            }
            if (it != pdalboost::end(range))
            {
        ...
        if (n > 0)
        {
            geometry::detail::conversion::convert_point_to_point(*pdalboost::begin(range), point);
            return true;
        }
        return false;
    }
