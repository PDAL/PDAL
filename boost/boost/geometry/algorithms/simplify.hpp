...
struct simplify_copy
{
                             ...
                             Distance const& , Strategy const& )
    {
        std::copy
            (
                pdalboost::begin(range), pdalboost::end(range), std::back_inserter(out)
            );
    }
};
...
struct simplify_range
{
                    ...
                    Distance const& max_distance, Strategy const& strategy)
    {
        ...
        */
        ...
        if (pdalboost::size(range) <= int(Minimum) || max_distance < 0.0)
        {
...
struct simplify_polygon
{
                    ...
                    Distance const& max_distance, Strategy const& strategy)
    {
        ...
        BOOST_AUTO_TPL(it_out, pdalboost::begin(rings_out));
        for (BOOST_AUTO_TPL(it_in,  pdalboost::begin(rings_in));
            it_in != pdalboost::end(rings_in);
            ++it_in, ++it_out)
        {
