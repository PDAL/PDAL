...
struct closing_iterator
    ...
    <
        ...
        typename pdalboost::range_value<Range>::type const,
    ...
    >
{
    ...
    explicit inline closing_iterator(Range& range)
        : m_range(&range)
        , m_iterator(pdalboost::begin(range))
        , m_end(pdalboost::end(range))
        , m_size(pdalboost::size(range))
        , m_index(0)
    {}
    ...
    explicit inline closing_iterator(Range& range, bool)
        : m_range(&range)
        , m_iterator(pdalboost::end(range))
        , m_end(pdalboost::end(range))
        , m_size(pdalboost::size(range))
        , m_index(m_size + 1)
    {}
...
private:
    ...
    inline typename pdalboost::range_value<Range>::type const& dereference() const
    {
        return *m_iterator;
    }
    ...
    inline void update_iterator()
    {
        this->m_iterator = m_index <= m_size
            ? pdalboost::begin(*m_range) + (m_index % m_size)
            : pdalboost::end(*m_range)
            ;
    }

    Range* m_range;
    typename pdalboost::range_iterator<Range>::type m_iterator;
    typename pdalboost::range_iterator<Range>::type m_end;
