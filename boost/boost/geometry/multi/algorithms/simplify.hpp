...
struct simplify_multi
{
    ...
    {
        traits::resize<MultiGeometry>::apply(out, pdalboost::size(multi));

        typename pdalboost::range_iterator<MultiGeometry>::type it_out
                = pdalboost::begin(out);
        for (typename pdalboost::range_iterator<MultiGeometry const>::type it_in
                    = pdalboost::begin(multi);
            it_in != pdalboost::end(multi);
            ++it_in, ++it_out)
        {
