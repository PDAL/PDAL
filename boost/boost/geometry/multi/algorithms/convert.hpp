...
struct single_to_multi: private Policy
{
    static inline void apply(Single const& single, Multi& multi)
    {
        ...
        Policy::apply(single, *pdalboost::begin(multi));
    }
};
...
struct multi_to_multi: private Policy
{
    ...
    {
        traits::resize<Multi2>::apply(multi2, pdalboost::size(multi1));

        typename pdalboost::range_iterator<Multi1 const>::type it1
                = pdalboost::begin(multi1);
        typename pdalboost::range_iterator<Multi2>::type it2
                = pdalboost::begin(multi2);

        for (; it1 != pdalboost::end(multi1); ++it1, ++it2)
        {
...
// version types might be the same and therefore we call pdalboost::is_same again
...
struct convert<Multi1, Multi2, multi_tag, multi_tag, DimensionCount, false>
    : detail::conversion::multi_to_multi
        <
            ...
            convert
                <
                    typename pdalboost::range_value<Multi1>::type,
                    typename pdalboost::range_value<Multi2>::type,
    ...
    : detail::conversion::single_to_multi
        <
            ...
            convert
                <
                    Single,
                    typename pdalboost::range_value<Multi>::type,
