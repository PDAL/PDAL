...
struct intersection_multi_linestring_multi_linestring_point
{
            ...
            Strategy const& strategy)
    {
        ...
        for (typename pdalboost::range_iterator
                ...
                >::type it1 = pdalboost::begin(ml1);
            it1 != pdalboost::end(ml1);
            ++it1)
        {
            for (typename pdalboost::range_iterator
                    <
                        MultiLinestring2 const
                    >::type it2 = pdalboost::begin(ml2);
                it2 != pdalboost::end(ml2);
                ++it2)
            {
...
struct clip_multi_linestring
{
            ...
            Box const& box, OutputIterator out, Strategy const& )
    {
        ...
        for (typename pdalboost::range_iterator<MultiLinestring const>::type it
            = pdalboost::begin(multi_linestring);
            it != pdalboost::end(multi_linestring); ++it)
        {
