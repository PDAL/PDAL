...
namespace dispatch
{
    ...
    struct select_rings<multi_polygon_tag, Multi>
    {
                    ...
                    ring_identifier id, Map& map, bool midpoint)
        {
            ...
            typedef select_rings<polygon_tag, typename pdalboost::range_value<Multi>::type> per_polygon;

            id.multi_index = 0;
            for (iterator_type it = pdalboost::begin(multi); it != pdalboost::end(multi); ++it)
            {
                id.ring_index = -1;
