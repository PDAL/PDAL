...
struct is_variant_move_noexcept {
    typedef typename pdalboost::mpl::find_if<
        Types, mpl::not_<pdalboost::is_nothrow_move_constructible<pdalboost::mpl::_1> >
    >::type iterator_t;

    typedef typename pdalboost::mpl::end<Types>::type end_t;
...
public: // internal visitor interface
    ...
    internal_visit(pdalboost::detail::variant::backup_holder<T>& operand, long) const
    {
        new(storage_) T( ::pdalboost::detail::variant::move(operand.get()) );
        BOOST_VARIANT_AUX_RETURN_VOID;
    }

    template <typename T>
        BOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(T& operand, int) const BOOST_NOEXCEPT_IF(BOOST_NOEXCEPT_EXPR(T(pdalboost::declval<T>())))
    {
        new(storage_) T(::pdalboost::detail::variant::move(operand));
        BOOST_VARIANT_AUX_RETURN_VOID;
    }
};
#endif
...
public: // internal visitor interfaces
    ...
    {
        lhs_content.get()
            = ::pdalboost::detail::variant::move(static_cast<backup_holder<T>* >(rhs_storage_)->get());
        BOOST_VARIANT_AUX_RETURN_VOID;
    }
    ...
    internal_visit(T& lhs_content, int) const
    {
        ...
        // Hint: Are any of the bounded types const-qualified or references?
        //
        lhs_content = ::pdalboost::detail::variant::move(*static_cast<T* >(rhs_storage_));
        BOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // visitor interface
    ...
    bool operator()(T& lhs)
    {
        lhs = ::pdalboost::detail::variant::move(rhs_);
        return true;
    }
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename T>
    typename pdalboost::enable_if<pdalboost::is_rvalue_reference<T&&> >::type convert_construct(
          T&& operand
        , int
        , mpl::false_ = mpl::false_() // is_foreign_variant
        )
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename Variant>
    typename pdalboost::enable_if<pdalboost::is_rvalue_reference<Variant&&> >::type convert_construct(
          Variant&& operand
        , long
        , mpl::true_// is_foreign_variant
        )
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename Variant>
    typename pdalboost::enable_if<pdalboost::is_rvalue_reference<Variant&&> >::type convert_construct_variant(Variant&& operand)
    {
        // [Determine if the given variant is itself a bounded type, or if its
        //  content needs to be converted (i.e., it is a 'foreign' variant):]
        //
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <class T>
    variant(T&& operand, typename pdalboost::enable_if<pdalboost::is_rvalue_reference<T&&> >::type* = 0, 
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <class T>
    typename pdalboost::enable_if_c<pdalboost::is_rvalue_reference<T&&>::value && !pdalboost::is_const<T>::value, variant& >::type 
        operator=(T&& rhs)
    {
