...
class BOOST_MPI_DECL communicator
{
 public:
   ...
   */
  communicator(const communicator& comm, const pdalboost::mpi::group& subgroup);
   ...
   */
  pdalboost::mpi::group group() const;
...
{
  packed_oarchive oa(*this);
  oa << n << pdalboost::serialization::make_array(values, n);
  send(dest, tag, oa);
}
                              ...
                              mpl::false_) const
{
  ...
  // Deserialize the data in the message
  pdalboost::serialization::array<T> arr(values, count > n? n : count);
  ia >> arr;
...
{
  shared_ptr<packed_oarchive> archive(new packed_oarchive(*this));
  *archive << n << pdalboost::serialization::make_array(values, n);
  request result = isend(dest, tag, *archive);
  result.m_data = archive;
  return result;
}
