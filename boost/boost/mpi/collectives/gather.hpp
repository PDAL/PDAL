       ...
       int root)
{
  if (comm.rank() == root) {
    ...
    ::pdalboost::mpi::gather(comm, in_value, &out_values[0], root);
  } else {
    ::pdalboost::mpi::gather(comm, in_value, root);
  }
}
       ...
       std::vector<T>& out_values, int root)
{
  if (comm.rank() == root) {
    ...
    ::pdalboost::mpi::gather(comm, in_values, n, &out_values[0], root);
  } 
  else
    ::pdalboost::mpi::gather(comm, in_values, n, root);
}
