...
{
  if (comm.rank() == root)
    ::pdalboost::mpi::scatter<T>(comm, &in_values[0], out_value, root);
  else
    ::pdalboost::mpi::scatter<T>(comm, static_cast<const T*>(0), out_value, 
                             root);
}
...
{
  if (comm.rank() == root)
    ::pdalboost::mpi::scatter(comm, &in_values[0], out_values, n, root);
  else
    ::pdalboost::mpi::scatter(comm, static_cast<const T*>(0), out_values, 
                                    n, root);
}
