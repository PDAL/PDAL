...
public:
  ...
  template <class T>
  MPI_Datatype datatype(const T& x = T(), typename pdalboost::enable_if<is_mpi_builtin_datatype<T> >::type* =0)
  {
    return get_mpi_datatype<T>(x);
  }

  template <class T>
  MPI_Datatype datatype(const T& x =T(), typename pdalboost::disable_if<is_mpi_builtin_datatype<T> >::type* =0 )
  {
