...
public:
    ...
    {
      if (x.count())
        save_impl(x.address(), pdalboost::mpi::get_mpi_datatype(*x.address()), x.count());
    }
    ...
    MPI_Datatype get_mpi_datatype()
    {
      if (!is_committed)
      {
        BOOST_MPI_CHECK_RESULT(MPI_Type_struct,
                    (
                      addresses.size(),
                      pdalboost::serialization::detail::get_data(lengths),
                      pdalboost::serialization::detail::get_data(addresses),
                      pdalboost::serialization::detail::get_data(types),
                      &datatype_
                    ));
    ...
    void save(const T & t)
    {
        save_impl(&t, pdalboost::mpi::get_mpi_datatype(t), 1);
    }
