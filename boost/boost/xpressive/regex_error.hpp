    ...
    )
    {
        if(!cond)
        {
            ...
            pdalboost::throw_exception(
                pdalboost::xpressive::regex_error(code, msg)
                    << pdalboost::throw_function(fun)
                    << pdalboost::throw_file(file)
                    << pdalboost::throw_line((int)line)
            );
            #else
            pdalboost::throw_exception(pdalboost::xpressive::regex_error(code, msg));
            #endif
        }
        return true;
    }
...
#define BOOST_XPR_ENSURE_(pred, code, msg)                                                          \
    pdalboost::xpressive::detail::ensure_(!!(pred), code, msg, BOOST_CURRENT_FUNCTION, __FILE__, __LINE__)  \
    /**/
