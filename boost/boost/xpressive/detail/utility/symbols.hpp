...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost { namespace xpressive { namespace detail
{
    ...
    struct symbols
    {
        ...
        {
            iterator begin = pdalboost::begin(map);
            iterator end = pdalboost::end(map);
            node* root_p = this->root.get();
            for(; begin != end; ++begin)
            {
                key_iterator kbegin = pdalboost::begin(begin->first);
                key_iterator kend = pdalboost::end(begin->first);
    ...
    private:
        ...
        pdalboost::shared_ptr<node> root;
    };
