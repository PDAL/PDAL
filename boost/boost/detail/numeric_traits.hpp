...
//      great as pdalboost::intmax_t can cause some differences to be
...
# ifndef BOOST_NO_LIMITS_COMPILE_TIME_CONSTANTS
  ...
  // This should probably be rolled into pdalboost::integer_traits one day, but I
      ...
      if_true<(int(x::is_signed)
              && (!int(x::is_bounded)
                 // digits is the number of no-sign bits
                  || (int(x::digits) + 1 >= digit_traits<pdalboost::intmax_t>::digits)))>::template then<
        Integer,
...
#else
      BOOST_STATIC_ASSERT(pdalboost::is_integral<Integer>::value);
