...
public:
    ...
    :m_bits(alloc),
     m_num_bits(0)
    {
        using pdalboost::detail::dynamic_bitset_impl::value_to_type;
        using pdalboost::detail::dynamic_bitset_impl::is_numeric;
    ...
    void m_append(BlockInputIterator first, BlockInputIterator last, std::forward_iterator_tag)
    {
        ...
        std::size_t d = pdalboost::detail::distance(first, last);
...
private:
                                 ...
                                 const Allocator& alloc*/)
    {
        ...
        typedef pdalboost::detail::dynamic_bitset_impl
...
to_ulong() const
{
  ...
  if (find_next(ulong_width - 1) != npos)
    throw std::overflow_error("pdalboost::dynamic_bitset::to_ulong overflow");
...
dynamic_bitset<Block, Allocator>::m_do_find_from(size_type first_block) const
{
    ...
    return i * bits_per_block + pdalboost::lowest_bit(m_bits[i]);
