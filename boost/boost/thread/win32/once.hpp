...
#define BOOST_THREAD_INVOKE_RET_VOID_CALL
#else
#define BOOST_THREAD_INVOKE_RET_VOID pdalboost::bind
#define BOOST_THREAD_INVOKE_RET_VOID_CALL ()
#endif
    ...
    namespace detail
    {
        ...
        inline bool enter_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
          ...
          if(!status)
          {
            ...
            if(ctx.event_handle)
            {
                ::pdalboost::detail::win32::ResetEvent(ctx.event_handle);
            }
            return true;
          }
          return false;
        ...
        inline void commit_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
          ...
          if(ctx.event_handle)
          {
              ::pdalboost::detail::win32::SetEvent(ctx.event_handle);
          }
        }
        inline void rollback_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
...
//#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR)
    inline void call_once(once_flag& flag, void (*f)())
    {
        ...
        while(::pdalboost::detail::interlocked_read_acquire(&flag.status)
              !=ctx.function_complete_flag_value)
        {
            ...
            if(!ctx.counted)
            {
                BOOST_INTERLOCKED_INCREMENT(&flag.count);
                ctx.counted=true;
                long status=::pdalboost::detail::interlocked_read_acquire(&flag.status);
            ...
            BOOST_VERIFY(!::pdalboost::detail::win32::WaitForSingleObject(
                             ctx.event_handle,::pdalboost::detail::win32::infinite));
        }
    }
//#endif
            ...
            if(detail::enter_once_region(flag, ctx))
            {
                BOOST_TRY
                {
                  BOOST_THREAD_INVOKE_RET_VOID(
                        thread_detail::decay_copy(pdalboost::forward<Function>(f)),
                        thread_detail::decay_copy(pdalboost::forward<A>(a)),
        ...
        void call_once(once_flag& flag,void (*f)(BOOST_THREAD_RV_REF(T1)), BOOST_THREAD_RV_REF(T1) p1)
        {
                  ...
                  !=ctx.function_complete_flag_value)
            {
                if(detail::enter_once_region(flag, ctx))
                {
                    ...
                    {
                       f(
                           thread_detail::decay_copy(pdalboost::forward<T1>(p1))
                       );
                    }
                    BOOST_CATCH(...)
                    {
                  ...
                  !=ctx.function_complete_flag_value)
            {
                if(detail::enter_once_region(flag, ctx))
                {
                    BOOST_TRY
                    {
                      f(
                          thread_detail::decay_copy(pdalboost::forward<T1>(p1)),
                          thread_detail::decay_copy(pdalboost::forward<T2>(p2))
                      );
                    }
                    BOOST_CATCH(...)
                    {
                ...
                if(detail::enter_once_region(flag, ctx))
                {
                    BOOST_TRY
                    {
                      f(
                          thread_detail::decay_copy(pdalboost::forward<T1>(p1)),
                          thread_detail::decay_copy(pdalboost::forward<T2>(p2)),
                          thread_detail::decay_copy(pdalboost::forward<T3>(p3))
                      );
                    }
                    BOOST_CATCH(...)
                    {
