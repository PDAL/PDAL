...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost
{
  ...
  namespace system
  {
    ...
    {
        return error_code(underlying_cast<int>(e), pdalboost::future_category());
    }
...
#else
            typedef T& source_reference_type;
            typedef typename pdalboost::mpl::if_<pdalboost::is_convertible<T&,BOOST_THREAD_RV_REF(T) >,BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
            typedef typename pdalboost::mpl::if_<pdalboost::is_convertible<T&,BOOST_THREAD_RV_REF(T) >,BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#endif
...
//        };
    ...
    public:
        ...
        ~promise()
        {
            if(future_)
            {
                pdalboost::lock_guard<pdalboost::mutex> lock(future_->mutex);

                if(!future_->done)
                {
                    future_->mark_exceptional_finish_internal(pdalboost::copy_exception(broken_promise()));
                }
            }
        }
    ...
    public:
        ...
        explicit packaged_task(BOOST_THREAD_RV_REF(F) f):
            task(new detail::task_object<R,
                ...
                >(pdalboost::forward<F>(f))),future_obtained(false)
        {}
#else
        template <class F>
        ...
        template <class F>
        explicit packaged_task(BOOST_THREAD_RV_REF(F) f):
            task(new detail::task_object<R,F>(pdalboost::move(f))),future_obtained(false)
        {}
#endif
        ...
        packaged_task(pdalboost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
          ...
          task = task_ptr(::new(a2.allocate(1)) detail::task_object<R,FR>(pdalboost::forward<F>(f)), D(a2, 1) );
          future_obtained = false;
        }
#else
        template <class F, class Allocator>
        ...
        packaged_task(pdalboost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
          ...
          task = task_ptr(::new(a2.allocate(1)) detail::task_object<R,F>(pdalboost::move(f)), D(a2, 1) );
          future_obtained = false;
        }
...
//    {
//        typedef typename pdalboost::result_of<F()>::type R;
