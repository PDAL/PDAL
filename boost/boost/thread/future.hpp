...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost
{
    ...
    namespace detail
    {
        ...
        struct future_continuation_base
        {
          ...
          virtual void do_continuation(pdalboost::unique_lock<pdalboost::mutex>& ) {};
        private:
...
//#endif
...
#endif
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            void set_continuation_ptr(future_continuation_base* continuation, pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
        ...
        struct future_traits
        {
            ...
            typedef typename pdalboost::mpl::if_<pdalboost::has_move_emulation_enabled<T>,BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
            typedef typename pdalboost::mpl::if_<pdalboost::has_move_emulation_enabled<T>,BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#else
            typedef T& source_reference_type;
            ...
            detail::future_object_base
        {
            ...
            void mark_finished_with_result_internal(T& result_, pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
                //future_traits<T>::init(result,result_);
                result= &result_;
                mark_finished_internal(lock);
...
//         void set_value(R && r);
    ...
    namespace detail
    {
      ...
      struct task_base:
        ...
        public:
            ...
            {}
            task_object(BOOST_THREAD_RV_REF(F) f_):
                f(pdalboost::move(f_)) // TODO forward
            {}
#endif
    ...
    ////////////////////////////////
...
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
              pdalboost::thread( pdalboost::move(pt),  pdalboost::forward<ArgTypes>(args)... ).detach();
#else
        ...
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          return pdalboost::detail::make_future_async_object<Rp>(
              BF(
...
//          );
    ...
    {
        return async(launch(launch::any), pdalboost::forward<F>(f));
    }
  ...
  namespace detail
  {
      ...
      struct future_continuation : future_continuation_base
      {
        ...
        void do_continuation(pdalboost::unique_lock<pdalboost::mutex>& lk)
        {
          try
          {
            lk.unlock();
...
//            }
  ...
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_RV_REF(F) func)
  {
    ...
    if (this->future_)
    {
      ...
      detail::future_continuation<BOOST_THREAD_FUTURE<R>, future_type, F > *ptr =
          new detail::future_continuation<BOOST_THREAD_FUTURE<R>, future_type, F>(*this, pdalboost::forward<F>(func), policy);
      if (ptr==0)
      {
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_RV_REF(F) func)
  {
    ...
    if (this->future_)
    {
      pdalboost::unique_lock<pdalboost::mutex> lock(this->future_->mutex);
      detail::future_continuation<BOOST_THREAD_FUTURE<R>, future_type, F > *ptr =
          new detail::future_continuation<BOOST_THREAD_FUTURE<R>, future_type, F>(*this, pdalboost::forward<F>(func));
      if (ptr==0)
      {
        return BOOST_THREAD_FUTURE<future_type>();
      }
