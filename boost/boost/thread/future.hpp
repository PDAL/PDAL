...
//#endif
            ...
            void wait_internal(pdalboost::unique_lock<pdalboost::mutex> &lock, bool rethrow=true)
            {
              do_callback(lock);
            ...
            void set_interrupted_pdalboostat_thread_exit()
            {
              ...
              get_current_thread_data()->make_ready_pdalboostat_thread_exit(shared_from_this());
            }
#endif
            ...
            bool has_value(unique_lock<pdalboost::mutex>& )
            {
                return done && !(exception
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                    || thread_was_interrupted
            ...
            bool has_exception(unique_lock<pdalboost::mutex>&)
            {
                return done && (exception
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                    || thread_was_interrupted
...
//            }
        ...
        public:
          explicit future_async_object(BOOST_THREAD_FWD_REF(Fp) f) :
            thr_(&future_async_object::run, this, pdalboost::forward<Fp>(f))
          {
            this->set_async();
          }
          ...
          move_dest_type get()
          {
              ...
              //this->wait();
              pdalboost::unique_lock<pdalboost::mutex> lock(this->mutex);
              this->wait_internal(lock);

              //return static_cast<move_dest_type>(*(this->result));
              //return pdalboost::move<Rp>(*(this->result));
              return pdalboost::move(*(this->result));
          }
...
//        };
        class future_waiter
        {
            ...
            struct registered_waiter
            {
                pdalboost::shared_ptr<detail::future_object_base> future_;
                detail::future_object_base::waiter_list::iterator wait_iterator;
                count_type index;

                registered_waiter(pdalboost::shared_ptr<detail::future_object_base> const& a_future,
...
#else
                        locks[i]=pdalboost::unique_lock<pdalboost::mutex>(futures[i].future_->mutex); // TODO shouldn't be moved explicitly
#endif
                    }
                }
    ...
    namespace detail
    {
        ...
        BOOST_THREAD_FUTURE<Rp>
        make_future_async_continuation(pdalboost::unique_lock<pdalboost::mutex> &lock, F& f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_deferred_continuation(pdalboost::unique_lock<pdalboost::mutex> &lock, F& f, BOOST_THREAD_FWD_REF(Fp) c);
#endif
    ...
    private:
        ...
        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_async_continuation(pdalboost::unique_lock<pdalboost::mutex> &lock, F& f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_deferred_continuation(pdalboost::unique_lock<pdalboost::mutex> &lock, F& f, BOOST_THREAD_FWD_REF(Fp) c);
#endif
...
//#endif
    ...
    public:
        ...
        {
            base_type::operator=(pdalboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            //shared_future(pdalboost::move(other)).swap(*this);
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    {
      typedef pdalboost::shared_ptr<detail::task_base<R(ArgTypes...)> > task_ptr;
      pdalboost::shared_ptr<detail::task_base<R(ArgTypes...)> > task;
  #else
    template<typename R>
    class packaged_task<R()>
    {
      typedef pdalboost::shared_ptr<detail::task_base<R()> > task_ptr;
    ...
    public:
        ...
        explicit packaged_task(R(*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {
            ...
            task= task_ptr(new task_object_type(f, pdalboost::forward<ArgTypes>(args)...));
            future_obtained=false;
        }
  #else
            ...
            )
        {
          typedef typename remove_cv<typename remove_reference<F>::type>::type FR;
            ...
            typedef detail::task_object<FR,R> task_object_type;
#endif
            task = task_ptr(new task_object_type(pdalboost::forward<F>(f)));
            future_obtained = false;
...
#else
            typedef detail::task_object<F,R()> task_object_type;
            task = task_ptr(new task_object_type(pdalboost::move<F>(f))); // TODO forward
#endif
#else
            typedef detail::task_object<F,R> task_object_type;
            task = task_ptr(new task_object_type(pdalboost::forward<F>(f)));
        ...
        packaged_task(pdalboost::allocator_arg_t, Allocator a, BOOST_THREAD_FWD_REF(F) f)
        {
          ...
          task = task_ptr(::new(a2.allocate(1)) task_object_type(pdalboost::forward<F>(f)), D(a2, 1) );
          future_obtained = false;
        }
        ...
        packaged_task(pdalboost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
          ...
          task = task_ptr(::new(a2.allocate(1)) task_object_type(pdalboost::forward<F>(f)), D(a2, 1) );
#else
          task = task_ptr(::new(a2.allocate(1)) task_object_type(pdalboost::move(f)), D(a2, 1) );  // TODO forward
#endif
          future_obtained = false;
        }
    ...
    make_future_deferred_object(BOOST_THREAD_FWD_REF(Fp) f)
    {
      shared_ptr<future_deferred_object<Rp, Fp> >
          h(new future_deferred_object<Rp, Fp>(pdalboost::forward<Fp>(f)));
    ...
    make_future_async_object(BOOST_THREAD_FWD_REF(Fp) f)
    {
      shared_ptr<future_async_object<Rp, Fp> >
          h(new future_async_object<Rp, Fp>(pdalboost::forward<Fp>(f)));
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
    ...
    ////////////////////////////////
        ...
        async(launch policy, R(*f)())
        {
          ...
          if (int(policy) & int(launch::async))
            {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          return BOOST_THREAD_MAKE_RV_REF(pdalboost::detail::make_future_async_object<Rp>(
              BF(
            ...
            {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          return BOOST_THREAD_MAKE_RV_REF(pdalboost::detail::make_future_deferred_object<Rp>(
              BF(
                  thread_detail::decay_copy(pdalboost::forward<F>(f))
                  , thread_detail::decay_copy(pdalboost::forward<ArgTypes>(args))...
              )
        ...
        if (int(policy) & int(launch::async))
        {
          ...
          ret.set_async();
//#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
//          pdalboost::thread( pdalboost::move(pt), pdalboost::forward<ArgTypes>(args)... ).detach(); // todo forward
//#else
          pdalboost::thread( pdalboost::move(pt) ).detach();
//#endif
          return ::pdalboost::move(ret);
        ...
        else if (int(policy) & int(launch::deferred))
        {
          ...
          return BOOST_THREAD_MAKE_RV_REF(pdalboost::detail::make_future_deferred_object<Rp>(
              ...
              BOOST_THREAD_FUTURE<R> ret;
              return ::pdalboost::move(ret);
//          return pdalboost::detail::make_future_deferred_object<Rp>(
//              BF(
//                  thread_detail::decay_copy(pdalboost::forward<F>(f))
//              )
//          );
  ...
  namespace detail
  {
        ...
        )
    {
      shared_ptr<future_deferred_continuation<F, Rp, Fp> >
          h(new future_deferred_continuation<F, Rp, Fp>(f, pdalboost::forward<Fp>(c)));
    ...
    BOOST_THREAD_FUTURE<Rp>
    make_future_async_continuation(
        pdalboost::unique_lock<pdalboost::mutex> &lock, F& f, BOOST_THREAD_FWD_REF(Fp) c
        )
    {
      shared_ptr<future_async_continuation<F,Rp, Fp> >
          h(new future_async_continuation<F,Rp, Fp>(f, pdalboost::forward<Fp>(c)));
      f.future_->set_continuation_ptr(h, lock);
...
//        launch policy_;
//
//        future_continuation(pdalboost::unique_lock<pdalboost::mutex>& lk, F& f, BOOST_THREAD_FWD_REF(C) c) :
//          parent(f),
//          continuation(pdalboost::forward<C>(c)),
//          policy_(f.launch_policy())
//        {
//          init_continuation(lk);
//        }
//        future_continuation(pdalboost::unique_lock<pdalboost::mutex>& lk, F& f, BOOST_THREAD_FWD_REF(C) c, launch policy) :
//          parent(f),
//          continuation(pdalboost::forward<C>(c)),
//          policy_(policy)
//        {
...
//        {}
//
//        void init_continuation(pdalboost::unique_lock<pdalboost::mutex>& lk)
//        {
//          try
//          {
//            lk.unlock();
...
//              next.set_value(pdalboost::move(val));
//            }
//            else
//            {
//              BOOST_THREAD_FUTURE<R> f = async(policy_, continuation, pdalboost::ref(parent));
//              R val = f.get();
//              next.set_value(pdalboost::move(val));
//            }
//          }
//          catch (...)
//          {
//            next.set_exception(pdalboost::current_exception());
//          }
//        }
//      private:
//
...
//        {}
//
//        void start_continuation(pdalboost::unique_lock<pdalboost::mutex>& lk)
//        {
//          try
//          {
//            lk.unlock();
...
//#endif
  ...
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)
  {
      ...
      if (int(policy) & int(launch::async))
    {
      return BOOST_THREAD_MAKE_RV_REF((pdalboost::detail::make_future_async_continuation<BOOST_THREAD_FUTURE<R>, future_type, F>(
          lock, *this, pdalboost::forward<F>(func)
      )));
    }
      else
        if (int(policy) & int(launch::deferred))
        {
          return BOOST_THREAD_MAKE_RV_REF((pdalboost::detail::make_future_deferred_continuation<BOOST_THREAD_FUTURE<R>, future_type, F>(
              lock, *this, pdalboost::forward<F>(func)
          )));
        }
      else
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_FWD_REF(F) func)
  {
    ...
    {
      return pdalboost::detail::make_future_async_continuation<BOOST_THREAD_FUTURE<R>, future_type, F>(
          lock, *this, pdalboost::forward<F>(func)
      );
    }
    else if (int(this->launch_policy()) & int(launch::deferred))
    {
      ...
      return pdalboost::detail::make_future_deferred_continuation<BOOST_THREAD_FUTURE<R>, future_type, F>(
          lock, *this, pdalboost::forward<F>(func)
      );
    }
    else
