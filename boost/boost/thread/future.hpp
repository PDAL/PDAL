...
// pdalboost::thread::future requires exception handling
// due to pdalboost::exception::exception_ptr dependency
...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost
{
  ...
  public:
    ...
    namespace detail
    {
        struct relocker
        {
            pdalboost::unique_lock<pdalboost::mutex>& lock_;
            bool  unlocked_;

            relocker(pdalboost::unique_lock<pdalboost::mutex>& lk):
                lock_(lk)
            {
                lock_.unlock();
                unlocked_=true;
        ...
        struct shared_state_base : enable_shared_from_this<shared_state_base>
        {
            ...
            // This declaration should be only included conditionally, but is included to maintain the same layout.
            virtual void launch_continuation(pdalboost::unique_lock<pdalboost::mutex>&)
            {
            }
            ...
            void do_continuation(pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
                if (continuation_ptr) {
...
#else
            void do_continuation(pdalboost::unique_lock<pdalboost::mutex>&)
            {
            }
#endif
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            void set_continuation_ptr(continuation_ptr_type continuation, pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
...
#endif
            void mark_finished_internal(pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
                done=true;
                waiters.notify_all();
            ...
            void wait_internal(pdalboost::unique_lock<pdalboost::mutex> &lk, bool rethrow=true)
            {
              do_callback(lk);
              //if (!done) // fixme why this doesn't work?
              {
                ...
                else
                {
                  ...
                  {
                      throw pdalboost::thread_interrupted();
                  }
#endif
...
#endif
            void mark_exceptional_finish_internal(pdalboost::exception_ptr const& e, pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
                exception=e;
                mark_finished_internal(lock);
            }
            ...
            {
                pdalboost::unique_lock<pdalboost::mutex> lock(mutex);
                mark_exceptional_finish_internal(pdalboost::current_exception(), lock);
            }
            ...
            void set_interrupted_pdalboostat_thread_exit()
            {
              unique_lock<pdalboost::mutex> lk(mutex);
              thread_was_interrupted=true;
              if (has_value(lk))
              {
                  throw_exception(promise_already_satisfied());
              }
              detail::make_ready_pdalboostat_thread_exit(shared_from_this());
            }
#endif

            void set_exception_pdalboostat_thread_exit(exception_ptr e)
            {
              unique_lock<pdalboost::mutex> lk(mutex);
              if (has_value(lk))
              {
            ...
            bool has_value(unique_lock<pdalboost::mutex>& )  const
            {
                return done && !(exception
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                    || thread_was_interrupted
            ...
            bool has_exception(unique_lock<pdalboost::mutex>&) const
            {
                return done && (exception
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                    || thread_was_interrupted
            ...
            bool is_deferred(pdalboost::lock_guard<pdalboost::mutex>&)  const {
                return is_deferred_;
            }

            launch launch_policy(pdalboost::unique_lock<pdalboost::mutex>&) const
            {
                return policy_;
            }
            ...
            exception_ptr get_exception_ptr(pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
                wait_internal(lock, false);
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                if(thread_was_interrupted)
                {
                    return copy_exception(pdalboost::thread_interrupted());
                }
#endif
                return exception;
            }
            ...
            virtual void execute(pdalboost::unique_lock<pdalboost::mutex>&) {}
        ...
        struct future_traits
        {
          ...
          //typedef typename pdalboost::mpl::if_<pdalboost::is_fundamental<T>,dummy&,BOOST_THREAD_RV_REF(T)>::type rvalue_source_type;
          typedef BOOST_THREAD_RV_REF(T) rvalue_source_type;
          //typedef typename pdalboost::mpl::if_<pdalboost::is_fundamental<T>,T,BOOST_THREAD_RV_REF(T)>::type move_dest_type;
          typedef T move_dest_type;
#elif defined BOOST_THREAD_USES_MOVE
          typedef typename pdalboost::mpl::if_c<pdalboost::is_fundamental<T>::value,T,T&>::type source_reference_type;
          //typedef typename pdalboost::mpl::if_c<pdalboost::is_fundamental<T>::value,T,BOOST_THREAD_RV_REF(T)>::type rvalue_source_type;
          //typedef typename pdalboost::mpl::if_c<pdalboost::enable_move_utility_emulation<T>::value,BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
          typedef BOOST_THREAD_RV_REF(T) rvalue_source_type;
          typedef T move_dest_type;
#else
          typedef T& source_reference_type;
          typedef typename pdalboost::mpl::if_<pdalboost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >,BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
          typedef typename pdalboost::mpl::if_<pdalboost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >,BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#endif
            ...
            static void init(storage_type& storage,rvalue_source_type t)
            {
              ...
              storage.reset(new T(pdalboost::forward<T>(t)));
#else
            ...
            detail::shared_state_base
        {
            ...
            void mark_finished_with_result_internal(source_reference_type result_, pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
                future_traits<T>::init(result,result_);
                this->mark_finished_internal(lock);
            }

            void mark_finished_with_result_internal(rvalue_source_type result_, pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                future_traits<T>::init(result,pdalboost::forward<T>(result_));
#else
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                mark_finished_with_result_internal(pdalboost::forward<T>(result_), lock);
#else
            ...
            //void set_value_pdalboostat_thread_exit(const T & result_)
            void set_value_pdalboostat_thread_exit(source_reference_type result_)
            {
              unique_lock<pdalboost::mutex> lk(this->mutex);
            ...
            //void set_value_pdalboostat_thread_exit(BOOST_THREAD_RV_REF(T) result_)
            void set_value_pdalboostat_thread_exit(rvalue_source_type result_)
            {
              unique_lock<pdalboost::mutex> lk(this->mutex);
              if (this->has_value(lk))
                  throw_exception(promise_already_satisfied());
              result.reset(new T(pdalboost::move(result_)));
              //future_traits<T>::init(result,static_cast<rvalue_source_type>(result_));
              this->is_constructed = true;
              detail::make_ready_pdalboostat_thread_exit(shared_from_this());
            }
            ...
            detail::shared_state_base
        {
            ...
            void set_value_pdalboostat_thread_exit(T& result_)
            {
              unique_lock<pdalboost::mutex> lk(this->mutex);
              if (this->has_value(lk))
                  throw_exception(promise_already_satisfied());
            ...
            detail::shared_state_base
        {
          typedef void shared_future_get_result_type;
            ...
            void mark_finished_with_result_internal(pdalboost::unique_lock<pdalboost::mutex>& lock)
            {
                mark_finished_internal(lock);
            }
            ...
            void set_value_pdalboostat_thread_exit()
            {
              unique_lock<pdalboost::mutex> lk(this->mutex);
              if (this->has_value(lk))
              {
        ...
        public:
          ...
          explicit future_async_shared_state_base(BOOST_THREAD_RV_REF(pdalboost::thread) th) :
            thr_(pdalboost::move(th))
          {
            this->set_async();
          }
        ...
        public:
          ...
          base_type(thread(&future_async_shared_state::run, this, pdalboost::forward<Fp>(f)))
          {
          }
        ...
        public:
          ...
          : func_(pdalboost::forward<Fp>(f))
          {
            this->set_deferred();
          }

          virtual void execute(pdalboost::unique_lock<pdalboost::mutex>& lck) {
            try
            {
              Fp local_fuct=pdalboost::move(func_);
              relocker relock(lck);
              Rp res = local_fuct();
              relock.lock();
              this->mark_finished_with_result_internal(pdalboost::move(res), lck);
            }
            catch (...)
            {
...
//        };
        class future_waiter
        {
            ...
            struct registered_waiter
            {
                pdalboost::shared_ptr<detail::shared_state_base> future_;
                detail::shared_state_base::waiter_list::iterator wait_iterator;
                count_type index;

                registered_waiter(pdalboost::shared_ptr<detail::shared_state_base> const& a_future,
            ...
            struct all_futures_lock
            {
                    ...
                    count(futures.size()),locks(new pdalboost::unique_lock<pdalboost::mutex>[count])
                {
                    for(count_type_portable i=0;i<count;++i)
                    {
                        ...
                        locks[i]=pdalboost::unique_lock<pdalboost::mutex>(futures[i].future_->mutex).move();
#else
                        locks[i]=pdalboost::unique_lock<pdalboost::mutex>(futures[i].future_->mutex);
#endif
                    }
                }
    ...
    namespace detail
    {
      ...
      public:
        ...
        typedef pdalboost::shared_ptr<detail::shared_state<R> > future_ptr;
      ...
      public:
        ...
        launch launch_policy(pdalboost::unique_lock<pdalboost::mutex>& lk) const
        {
    ...
    namespace detail
    {
        ...
        BOOST_THREAD_FUTURE<Rp>
        make_future_async_continuation_shared_state(pdalboost::unique_lock<pdalboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_deferred_continuation_shared_state(pdalboost::unique_lock<pdalboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);
#endif
        ...
        make_future_unwrap_shared_state(pdalboost::unique_lock<pdalboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
    }
    ...
    private:
        ...
        detail::make_future_async_continuation_shared_state(pdalboost::unique_lock<pdalboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_deferred_continuation_shared_state(pdalboost::unique_lock<pdalboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);
#endif
#if defined BOOST_THREAD_PROVIDES_FUTURE_UNWRAP
        template<typename F, typename Rp>
        ...
        template <class F, class Rp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_unwrap_shared_state(pdalboost::unique_lock<pdalboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
    ...
    public:
        ...
        base_type(pdalboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))))
        {
        }
        ...
        {
            this->base_type::operator=(pdalboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }

        shared_future<R> share()
        {
          return shared_future<R>(::pdalboost::move(*this));
        }
        ...
        typename pdalboost::disable_if< is_void<R2>, move_dest_type>::type
...
//#endif
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename pdalboost::result_of<F(BOOST_THREAD_FUTURE)>::type>
        ...
        template <typename R2>
        inline typename pdalboost::disable_if< is_void<R2>, BOOST_THREAD_FUTURE<R> >::type
...
//#endif
    ...
    public:
        ...
        {
            base_type::operator=(pdalboost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }
        ...
        template <typename R2>
        typename pdalboost::disable_if< is_void<R2>, typename detail::shared_state<R>::shared_future_get_result_type>::type
...
//#endif
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename pdalboost::result_of<F(shared_future)>::type>
        then(BOOST_THREAD_FWD_REF(F) func); // EXTENSION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename pdalboost::result_of<F(shared_future)>::type>
        then(launch policy, BOOST_THREAD_FWD_REF(F) func); // EXTENSION
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    public:
        ...
        promise(pdalboost::allocator_arg_t, Allocator a)
        {
        ...
        ~promise()
        {
            if(future_)
            {
                pdalboost::unique_lock<pdalboost::mutex> lock(future_->mutex);

                if(!future_->done && !future_->is_constructed)
                {
                    future_->mark_exceptional_finish_internal(pdalboost::copy_exception(broken_promise()), lock);
                }
            }
        }
        ...
        BOOST_THREAD_FUTURE<R> get_future()
        {
            ...
            if (future_.get()==0)
            {
                pdalboost::throw_exception(promise_moved());
            }
            if (future_obtained)
            {
...
//         void set_value(R && r);
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            future_->mark_finished_with_result_internal(pdalboost::forward<R>(r), lock);
#else
        ...
        void set_value_pdalboostat_thread_exit(const R& r)
        {
          if (future_.get()==0)
          {
              pdalboost::throw_exception(promise_moved());
          }
          future_->set_value_pdalboostat_thread_exit(r);
        }

        void set_value_pdalboostat_thread_exit(BOOST_THREAD_RV_REF(R) r)
        {
          if (future_.get()==0)
          {
              pdalboost::throw_exception(promise_moved());
          }
          future_->set_value_pdalboostat_thread_exit(pdalboost::move(r));
        }
        void set_exception_pdalboostat_thread_exit(exception_ptr e)
        {
          if (future_.get()==0)
          ...
          future_->set_exception_pdalboostat_thread_exit(e);
        }
        template <typename E>
        void set_exception_pdalboostat_thread_exit(E ex)
        {
          set_exception_pdalboostat_thread_exit(copy_exception(ex));
        }
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    public:
        ...
        // setting the result with deferred notification
        void set_value_pdalboostat_thread_exit(R& r)
        {
          if (future_.get()==0)
          {
              pdalboost::throw_exception(promise_moved());
    ...
    class promise<void>
    {
        typedef pdalboost::shared_ptr<detail::shared_state<void> > future_ptr;
    ...
    public:
        ...
        void set_value_pdalboostat_thread_exit()
        {
          ...
          future_->set_value_pdalboostat_thread_exit();
        }
    ...
    namespace detail
    {
      ...
      struct task_base_shared_state:
            ...
            detail::shared_state<R>
        {
...
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_run(pdalboost::forward<ArgTypes>(args)...);
#else
                do_run();
#endif
            }
...
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_apply(pdalboost::forward<ArgTypes>(args)...);
#else
                do_apply();
#endif
            }
            ...
            {
                pdalboost::unique_lock<pdalboost::mutex> lk(this->mutex);
                if(!started)
                {
                    started=true;
                    this->mark_exceptional_finish_internal(pdalboost::copy_exception(pdalboost::broken_promise()), lk);
                }
            }
        ...
        public:
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->set_value_pdalboostat_thread_exit(f(pdalboost::forward<ArgTypes>(args)...));
                }
#else
            void do_apply()
            {
                try
                {
                    this->set_value_pdalboostat_thread_exit(f());
                }
#endif
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                catch(thread_interrupted& )
                {
                    this->set_interrupted_pdalboostat_thread_exit();
                }
#endif
                catch(...)
                {
                    this->set_exception_pdalboostat_thread_exit(current_exception());
                }
            }
            ...
            void do_run(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->mark_finished_with_result(f(pdalboost::forward<ArgTypes>(args)...));
                }
#else
            void do_run()
            {
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                  R res((f()));
                  this->mark_finished_with_result(pdalboost::move(res));
#else
           ...
           task_base_shared_state<R>
            ...
            public:
                ...
                void do_apply()
                {
                    ...
                    {
                        R r((f()));
                        this->set_value_pdalboostat_thread_exit(pdalboost::move(r));
                    }
#endif
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                    catch(thread_interrupted& )
        ...
        public:
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
              try
              {
                f(pdalboost::forward<ArgTypes>(args)...);
#else
            void do_apply()
            {
                try
                {
                    f();
#endif
                  this->set_value_pdalboostat_thread_exit();
                }
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                catch(thread_interrupted& )
                {
    ...
    {
      typedef pdalboost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task_ptr;
      pdalboost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task;
  #else
    template<typename R>
    class packaged_task<R()>
    {
      typedef pdalboost::shared_ptr<detail::task_base_shared_state<R()> > task_ptr;
      pdalboost::shared_ptr<detail::task_base_shared_state<R()> > task;
  #endif
#else
    template<typename R>
    class packaged_task
    ...
    public:
        ...
        explicit packaged_task(R(*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {
            ...
            task= task_ptr(new task_shared_state_type(f, pdalboost::forward<ArgTypes>(args)...));
            future_obtained=false;
        }
  #else
        ...
        explicit packaged_task(BOOST_THREAD_FWD_REF(F) f
            , typename pdalboost::disable_if<is_same<typename decay<F>::type, packaged_task>, dummy* >::type=0
            )
        {
          typedef typename remove_cv<typename remove_reference<F>::type>::type FR;
            ...
            typedef detail::task_shared_state<FR,R> task_shared_state_type;
#endif
            task = task_ptr(new task_shared_state_type(pdalboost::forward<F>(f)));
            future_obtained = false;
...
#else
            typedef detail::task_shared_state<F,R()> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(pdalboost::move(f))); // TODO forward
#endif
#else
            typedef detail::task_shared_state<F,R> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(pdalboost::forward<F>(f)));
...
#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR)
        template <class Allocator>
        packaged_task(pdalboost::allocator_arg_t, Allocator a, R(*f)())
        {
          typedef R(*FR)();
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
        ...
        packaged_task(pdalboost::allocator_arg_t, Allocator a, BOOST_THREAD_FWD_REF(F) f)
        {
          typedef typename remove_cv<typename remove_reference<F>::type>::type FR;
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(pdalboost::forward<F>(f)), D(a2, 1) );
          future_obtained = false;
        }
#else // ! defined BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class F, class Allocator>
        packaged_task(pdalboost::allocator_arg_t, Allocator a, const F& f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
        ...
        template <class F, class Allocator>
        packaged_task(pdalboost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(pdalboost::forward<F>(f)), D(a2, 1) );
#else
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(pdalboost::move(f)), D(a2, 1) );  // TODO forward
#endif
          future_obtained = false;
        }
        ...
        packaged_task& operator=(BOOST_THREAD_RV_REF(packaged_task) other) BOOST_NOEXCEPT
        {
            ...
            // todo use forward
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            packaged_task temp(pdalboost::move(other));
#else
        ...
        void operator()(BOOST_THREAD_RV_REF(ArgTypes)... args)
        {
            ...
            task->run(pdalboost::forward<ArgTypes>(args)...);
        }
        void make_ready_pdalboostat_thread_exit(ArgTypes... args)
        {
          if(!task)
          {
              pdalboost::throw_exception(task_moved());
          ...
          task->apply(pdalboost::forward<ArgTypes>(args)...);
        }
#else
        void operator()()
        {
        ...
        void make_ready_pdalboostat_thread_exit()
        {
          if(!task)
          {
              pdalboost::throw_exception(task_moved());
    ...
    make_future_deferred_shared_state(BOOST_THREAD_FWD_REF(Fp) f)
    {
      shared_ptr<future_deferred_shared_state<Rp, Fp> >
          h(new future_deferred_shared_state<Rp, Fp>(pdalboost::forward<Fp>(f)));
    ...
    make_future_async_shared_state(BOOST_THREAD_FWD_REF(Fp) f)
    {
      shared_ptr<future_async_shared_state<Rp, Fp> >
          h(new future_async_shared_state<Rp, Fp>(pdalboost::forward<Fp>(f)));
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
    ...
    ////////////////////////////////
        ...
        async(launch policy, R(*f)())
        {
          ...
          if (int(policy) & int(launch::async))
            {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          return BOOST_THREAD_MAKE_RV_REF(pdalboost::detail::make_future_async_shared_state<Rp>(
              BF(
                  thread_detail::decay_copy(pdalboost::forward<F>(f))
                  , thread_detail::decay_copy(pdalboost::forward<ArgTypes>(args))...
              )
          ));
#else
              ...
              ret.set_async();
              pdalboost::thread( pdalboost::move(pt) ).detach();
              return ::pdalboost::move(ret);
#endif
            }
            else if (int(policy) & int(launch::deferred))
            {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          return BOOST_THREAD_MAKE_RV_REF(pdalboost::detail::make_future_deferred_shared_state<Rp>(
              BF(
                  thread_detail::decay_copy(pdalboost::forward<F>(f))
                  , thread_detail::decay_copy(pdalboost::forward<ArgTypes>(args))...
              )
        ...
        template <class F, class ...ArgTypes>
        BOOST_THREAD_FUTURE<typename pdalboost::result_of<typename decay<F>::type(
            typename decay<ArgTypes>::type...
        )>::type>
        async(launch policy, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {

          typedef typename pdalboost::result_of<typename decay<F>::type(
              typename decay<ArgTypes>::type...
          )>::type R;
  ...
  #else
        template <class F>
        BOOST_THREAD_FUTURE<typename pdalboost::result_of<typename decay<F>::type()>::type>
        async(launch policy, BOOST_THREAD_FWD_REF(F)  f)
        {
          typedef typename pdalboost::result_of<typename decay<F>::type()>::type R;
          typedef packaged_task<R()> packaged_task_type;
...
#else
          packaged_task_type pt( pdalboost::forward<F>(f) );
        ...
        else if (int(policy) & int(launch::deferred))
        {
          ...
          return BOOST_THREAD_MAKE_RV_REF(pdalboost::detail::make_future_deferred_shared_state<Rp>(
              ...
              BOOST_THREAD_FUTURE<R> ret;
              return ::pdalboost::move(ret);
//          return pdalboost::detail::make_future_deferred_shared_state<Rp>(
//              BF(
//                  thread_detail::decay_copy(pdalboost::forward<F>(f))
//              )
//          );
        ...
        ////////////////////////////////
            ...
            {
              return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), f, pdalboost::forward<ArgTypes>(args)...));
            }
    #else
            template <class R>
            BOOST_THREAD_FUTURE<R>
            ...
            {
                return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), pdalboost::forward<F>(f), pdalboost::forward<ArgTypes>(args)...));
            }
    #else
    template <class F>
    BOOST_THREAD_FUTURE<typename pdalboost::result_of<F()>::type>
    async(BOOST_THREAD_RV_REF(F) f)
    {
        return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), pdalboost::forward<F>(f)));
    }
#endif
  ...
  BOOST_THREAD_FUTURE<typename decay<T>::type> make_future(BOOST_THREAD_FWD_REF(T) value)
  {
    ...
    p.set_value(pdalboost::forward<future_value_type>(value));
  ...
  {
    promise<T> p;
    p.set_exception(pdalboost::copy_exception(ex));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  shared_future<typename decay<T>::type> make_shared_future(BOOST_THREAD_FWD_REF(T) value)
  {
    ...
    p.set_value(pdalboost::forward<T>(value));
...
//  }
  ...
  namespace detail
  {
    ...
    public:
      ...
      parent(pdalboost::move(f)),
      continuation(pdalboost::move(c))
      {
      }

      void launch_continuation(pdalboost::unique_lock<pdalboost::mutex>& lock)
      {
        lock.unlock();
      ...
      static void run(future_async_continuation_shared_state* that)
      {
        try
        {
          that->mark_finished_with_result(that->continuation(pdalboost::move(that->parent)));
        }
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
        catch(thread_interrupted& )
        {
    ...
    public:
      ...
      void launch_continuation(pdalboost::unique_lock<pdalboost::mutex>& lk)
      {
        lk.unlock();
        this->thr_ = thread(&future_async_continuation_shared_state::run, this);
      }
      ...
      static void run(future_async_continuation_shared_state* that)
      {
        try
        {
          that->continuation(pdalboost::move(that->parent));
          that->mark_finished_with_result();
        }
#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
        catch(thread_interrupted& )
    ...
    public:
      ...
      virtual void launch_continuation(pdalboost::unique_lock<pdalboost::mutex>& lk)
      {
        execute(lk);
      }
      ...
      virtual void execute(pdalboost::unique_lock<pdalboost::mutex>& lck) {
        try
        {
          Fp local_fuct=pdalboost::move(continuation);
          F ftmp = pdalboost::move(parent);
          relocker relock(lck);
          Rp res = local_fuct(pdalboost::move(ftmp));
          relock.lock();
          this->mark_finished_with_result_internal(pdalboost::move(res), lck);
        }
        catch (...)
    ...
    public:
      ...
      virtual void execute(pdalboost::unique_lock<pdalboost::mutex>& lck) {
        try
        {
          ...
          F ftmp = pdalboost::move(parent);
          relocker relock(lck);
          local_fuct(pdalboost::move(ftmp));
          relock.lock();
          this->mark_finished_with_result_internal(lck);
        }
        catch (...)
        ...
        )
    {
      shared_ptr<future_deferred_continuation_shared_state<F, Rp, Fp> >
          h(new future_deferred_continuation_shared_state<F, Rp, Fp>(pdalboost::move(f), pdalboost::forward<Fp>(c)));
    ...
    BOOST_THREAD_FUTURE<Rp>
    make_future_async_continuation_shared_state(
        pdalboost::unique_lock<pdalboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c
        )
    {
      shared_ptr<future_async_continuation_shared_state<F,Rp, Fp> >
          h(new future_async_continuation_shared_state<F,Rp, Fp>(pdalboost::move(f), pdalboost::forward<Fp>(c)));
      h->parent.future_->set_continuation_ptr(h, lock);
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename pdalboost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type>
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)
  {

    typedef typename pdalboost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_!=0, future_uninitialized());

    pdalboost::unique_lock<pdalboost::mutex> lock(this->future_->mutex);
    if (int(policy) & int(launch::async))
    {
      return BOOST_THREAD_MAKE_RV_REF((pdalboost::detail::make_future_async_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type, F>(
                  lock, pdalboost::move(*this), pdalboost::forward<F>(func)
              )));
    }
    else if (int(policy) & int(launch::deferred))
    {
      return BOOST_THREAD_MAKE_RV_REF((pdalboost::detail::make_future_deferred_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type, F>(
                  lock, pdalboost::move(*this), pdalboost::forward<F>(func)
              )));
    }
    else
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_FWD_REF(F) func)
  {
    ...
    {
      return pdalboost::detail::make_future_async_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type, F>(
          lock, pdalboost::move(*this), pdalboost::forward<F>(func)
      );
    }
    else if (int(this->launch_policy(lock)) & int(launch::deferred))
    {
      ...
      return pdalboost::detail::make_future_deferred_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type, F>(
          lock, pdalboost::move(*this), pdalboost::forward<F>(func)
      );
    }
    else
...
//#endif
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename pdalboost::result_of<F(shared_future<R>)>::type>
  shared_future<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)
  {

    typedef typename pdalboost::result_of<F(shared_future<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_!=0, future_uninitialized());

    pdalboost::unique_lock<pdalboost::mutex> lock(this->future_->mutex);
    if (int(policy) & int(launch::async))
    {
      return BOOST_THREAD_MAKE_RV_REF((pdalboost::detail::make_future_async_continuation_shared_state<shared_future<R>, future_type, F>(
                  lock, pdalboost::move(*this), pdalboost::forward<F>(func)
              )));
    }
    else if (int(policy) & int(launch::deferred))
    {
      return BOOST_THREAD_MAKE_RV_REF((pdalboost::detail::make_future_deferred_continuation_shared_state<shared_future<R>, future_type, F>(
                  lock, pdalboost::move(*this), pdalboost::forward<F>(func)
              )));
    }
    else
  ...
  shared_future<R>::then(BOOST_THREAD_FWD_REF(F) func)
  {
    ...
    if (int(this->launch_policy(lock)) & int(launch::async))
    {
      return pdalboost::detail::make_future_async_continuation_shared_state<shared_future<R>, future_type, F>(
          lock, pdalboost::move(*this), pdalboost::forward<F>(func)
      );
    }
    else if (int(this->launch_policy(lock)) & int(launch::deferred))
    {
      this->future_->wait_internal(lock);
      return pdalboost::detail::make_future_deferred_continuation_shared_state<shared_future<R>, future_type, F>(
          lock, pdalboost::move(*this), pdalboost::forward<F>(func)
      );
    }
    else
  ...
  namespace detail
  {
    ...
    struct mfallbacker_to
    {
      ...
      : value_(pdalboost::move(v))
      {}

      T operator()(BOOST_THREAD_FUTURE<T> fut)
      {
        return fut.get_or(pdalboost::move(value_));
  ...
  {
    return then(detail::mfallbacker_to<R>(pdalboost::move(v)));
  }
  ...
  namespace detail
  {
    ...
    make_future_unwrap_shared_state(pdalboost::unique_lock<pdalboost::mutex> &lock, BOOST_THREAD_RV_REF(F) f)
    {
      shared_ptr<future_unwrap_shared_state<F, Rp> >
          h(new future_unwrap_shared_state<F, Rp>(pdalboost::move(f)));
      h->parent.future_->set_continuation_ptr(h, lock);
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
  }
  ...
  BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >::unwrap()
  {
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_!=0, future_uninitialized());
    pdalboost::unique_lock<pdalboost::mutex> lock(this->future_->mutex);
    return pdalboost::detail::make_future_unwrap_shared_state<BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >, R2>(lock, pdalboost::move(*this));
  }
#endif
}
