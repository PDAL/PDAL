...
#define BOOST_DEFINE_HAS_MEMBER_CALLED(member_name)                     \
    ...
    public:
        ...
        void unlock()
        {
          if(m==0)
          {
            ...
            if(!owns_lock())
            {
                pdalboost::throw_exception(pdalboost::lock_error(system::errc::operation_not_permitted, "boost unique_lock doesn't own the mutex"));
            }
            m->unlock();
            is_locked=false;
        }
    ...
    public:
        ...
        void lock()
        {
          if(m==0)
          {
            ...
            if(owns_lock())
            {
              pdalboost::throw_exception(pdalboost::lock_error(system::errc::resource_deadlock_would_occur, "boost upgrade_lock owns already the mutex"));
            }
            m->lock_upgrade();
            is_locked=true;
        }
        ...
        void unlock()
        {
          if(m==0)
          {
            ...
            if(!owns_lock())
            {
              pdalboost::throw_exception(pdalboost::lock_error(system::errc::operation_not_permitted, "boost upgrade_lock doesn't own the mutex"));
            }
            m->unlock_upgrade();
            is_locked=false;
        }
    ...
    namespace detail
    {
...

        {
            ...
            Iterator const failed=pdalboost::try_lock(++begin,end);
            if(failed==end)
            {
                guard.release();
            }
    ...
    namespace detail
    {
        ...
        void lock_impl(Iterator begin,Iterator end,is_mutex_type_wrapper<false>)
        {
            ...
            for(;;)
            {
                ...
                {
                    begin_lock.lock();
                    Iterator const failed_lock=pdalboost::try_lock(next,end);
                    if(failed_lock==end)
                    {
                        begin_lock.release();
                        return;
                ...
                else
                {
                    ...
                    {
                        Iterator const failed_lock=pdalboost::try_lock(second,next);
                        if(failed_lock==next)
                        {
                            begin_lock.release();
                            guard.release();
