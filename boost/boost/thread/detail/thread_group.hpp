    ...
    public:
        ...
        void add_thread(thread* thrd)
        {
            if(thrd)
            {
                BOOST_THREAD_ASSERT_PRECONDITION( ! is_thread_in(thrd) ,
                    thread_resource_error(system::errc::resource_deadlock_would_occur, "pdalboost::thread_group: trying to add a duplicated thread")
                );
        ...
        void join_all()
        {
            BOOST_THREAD_ASSERT_PRECONDITION( ! is_this_thread_in() ,
                thread_resource_error(system::errc::resource_deadlock_would_occur, "pdalboost::thread_group: trying joining itself")
            );
            pdalboost::shared_lock<shared_mutex> guard(m);
