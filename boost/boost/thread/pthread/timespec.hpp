...
namespace pdalboost {} namespace boost = pdalboost; namespace pdalboost
{
  ...
  {
#if defined BOOST_THREAD_USES_DATETIME
    inline struct timespec to_timespec(pdalboost::system_time const& abs_time)
    {
    ...
    inline timespec to_timespec(pdalboost::intmax_t const& ns)
    {
      pdalboost::intmax_t s = ns / 1000000000l;
      struct timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline pdalboost::intmax_t to_nanoseconds_int_max(timespec const& ts)
    {
      return static_cast<pdalboost::intmax_t>(ts.tv_sec) * 1000000000l + ts.tv_nsec;
    }
