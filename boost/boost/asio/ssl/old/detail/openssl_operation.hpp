...
typedef pdalboost::function<int (::SSL*)> ssl_primitive_func; 
typedef pdalboost::function<void (const pdalboost::system::error_code&, int)>
  user_handler_func;
...
public:
  ...
  openssl_operation(ssl_primitive_func primitive,
                    ...
                    pdalboost::asio::io_service::strand& strand
                    )
  ...
  int start()
  {
    ...
    if (error_code == SSL_ERROR_SSL)
      return handler_(pdalboost::system::error_code(
            sys_error_code, pdalboost::asio::error::get_ssl_category()), rc);
      ...
      && !is_shut_down_sent)
    {
      ...
      {
        return handler_(pdalboost::system::error_code(
              sys_error_code, pdalboost::asio::error::system_category), rc); 
      }
      else
      {
        return handler_(pdalboost::system::error_code(
      ...
      if (recv_buf_.get_data_len() > 0)
      {
        ...
        else if (written < 0)
        {
          if (!BIO_should_retry(ssl_bio_))
          {
            ...
            return handler_(pdalboost::asio::error::no_recovery, 0);
          }
        }
...
private:
  typedef pdalboost::function<int (const pdalboost::system::error_code&, int)>
    int_handler_func;
  typedef pdalboost::function<int (bool, int)> write_func;
  typedef pdalboost::function<int ()> read_func;
  ...
  //
  int sync_user_handler(const pdalboost::system::error_code& error, int rc)
  {
    if (!error)
      return rc;

    throw pdalboost::system::system_error(error);
  }
    
  int async_user_handler(pdalboost::system::error_code error, int rc)
  {
    if (rc < 0)
    {
      if (!error)
        error = pdalboost::asio::error::no_recovery;
      rc = 0;
    }
  ...
  int  do_async_write(bool is_operation_done, int rc) 
  {
    ...
    { 
      ...
      if (len > 0)
      {
        ...
        ( 
          socket_, 
          pdalboost::asio::buffer(data_start, len),
          strand_->wrap
          (
            pdalboost::bind
            (
              ...
              is_operation_done,
              rc, 
              pdalboost::asio::placeholders::error, 
              pdalboost::asio::placeholders::bytes_transferred
            )
          )
        );
      ...
      else if (!BIO_should_retry(ssl_bio_))
      {
        ...
        handler_(pdalboost::asio::error::no_recovery, 0);
        return 0;
      }
    }
    ...
    if (is_operation_done)
    {
      ...
      handler_(pdalboost::system::error_code(), rc);
      return 0;
    }
  ...
  void async_write_handler(bool is_operation_done, int rc, 
    const pdalboost::system::error_code& error, size_t bytes_sent)
  {
    if (!error)
    {
  ...
  int do_async_read()
  {
    ...
    ( 
      pdalboost::asio::buffer(recv_buf_.get_unused_start(),
  ...
  void async_read_handler(const pdalboost::system::error_code& error,
      size_t bytes_recvd)
  {
    if (!error)
    {
  ...
  int do_sync_write(bool is_operation_done, int rc)
  {
    ...
    { 
      ...
      if (len > 0)
      {
        size_t sent_len = pdalboost::asio::write( 
                  socket_, 
                  pdalboost::asio::buffer(send_buf_.get_unused_start(), len)
                  );
      ...
      else if (!BIO_should_retry(ssl_bio_))
      {
        // Seems like fatal error
        // reading from SSL BIO has failed...
        throw pdalboost::system::system_error(pdalboost::asio::error::no_recovery);
      }
    }
