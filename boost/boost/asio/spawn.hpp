...
public:
  ...
  typedef pdalboost::coroutines::push_coroutine<void> callee_type;
#else
  typedef pdalboost::coroutines::coroutine<void()> callee_type;
#endif
...
#elif defined(BOOST_COROUTINES_UNIDRECT) || defined(BOOST_COROUTINES_V2)
  typedef pdalboost::coroutines::pull_coroutine<void> caller_type;
#else
  typedef pdalboost::coroutines::coroutine<void()>::caller_type caller_type;
#endif
   ...
   * } @endcode
   */
  basic_yield_context operator[](pdalboost::system::error_code& ec) const
  {
...
/**
 * @defgroup spawn pdalboost::asio::spawn
 *
 ...
 *
 * @code pdalboost::asio::spawn(my_strand, do_echo);
 *
 * // ...
 *
 * void do_echo(pdalboost::asio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           pdalboost::asio::buffer(data), yield);
 *
 *       pdalboost::asio::async_write(my_socket,
 *           pdalboost::asio::buffer(data, length), yield);
 *     }
 *   }
...
void spawn(pdalboost::asio::io_service::strand strand,
    BOOST_ASIO_MOVE_ARG(Function) function,
    const pdalboost::coroutines::attributes& attributes
      = pdalboost::coroutines::attributes());
 ...
 */
template <typename Function>
void spawn(pdalboost::asio::io_service& io_service,
    BOOST_ASIO_MOVE_ARG(Function) function,
    const pdalboost::coroutines::attributes& attributes
      = pdalboost::coroutines::attributes());
