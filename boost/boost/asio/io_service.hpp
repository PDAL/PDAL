 ...
 *
 * @li pdalboost::asio::ip::tcp::socket
 * @li pdalboost::asio::ip::tcp::acceptor
 * @li pdalboost::asio::ip::udp::socket
 * @li pdalboost::asio::deadline_timer.
 *
 ...
 * creating an object of type pdalboost::asio::io_service::work:
 *
 * @code pdalboost::asio::io_service io_service;
 * pdalboost::asio::io_service::work work(io_service);
 * ... @endcode
 *
 ...
 *
 * @code pdalboost::asio::io_service io_service;
 * auto_ptr<pdalboost::asio::io_service::work> work(
 *     new pdalboost::asio::io_service::work(io_service));
 * ...
...
public:
   ...
   * without blocking.
   */
  BOOST_ASIO_DECL std::size_t run(pdalboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t run_one(pdalboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t poll(pdalboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t poll_one(pdalboost::system::error_code& ec);
   ...
   * @code io_service.dispatch(pdalboost::bind(f, a1, ... an)); @endcode
   */
   ...
   *
   * @throws pdalboost::system::system_error Thrown on failure. If the notification
   ...
   * function:
   * @code my_io_service.notify_fork(pdalboost::asio::io_service::fork_prepare);
   * if (fork() == 0)
   * {
   *   // This is the child process.
   *   my_io_service.notify_fork(pdalboost::asio::io_service::fork_child);
   * }
   * else
   * {
   *   // This is the parent process.
   *   my_io_service.notify_fork(pdalboost::asio::io_service::fork_parent);
   * } @endcode
   *
   ...
   */
  BOOST_ASIO_DECL void notify_fork(pdalboost::asio::io_service::fork_event event);
   ...
   *
   * @throws pdalboost::asio::service_already_exists Thrown if a service of the
   * given type is already present in the io_service.
   *
   * @throws pdalboost::asio::invalid_service_owner Thrown if the service's owning
...
private:
  ...
  pdalboost::asio::detail::service_registry* service_registry_;
...
public:
   ...
   */
  explicit work(pdalboost::asio::io_service& io_service);
...
protected:
   ...
   */
  BOOST_ASIO_DECL service(pdalboost::asio::io_service& owner);
...
private:
  ...
  BOOST_ASIO_DECL virtual void fork_service(
      pdalboost::asio::io_service::fork_event event);

  friend class pdalboost::asio::detail::service_registry;
  struct key
  {
    key() : type_info_(0), id_(0) {}
    const std::type_info* type_info_;
    const pdalboost::asio::io_service::id* id_;
  } key_;
...
class service_id
  : public pdalboost::asio::io_service::id
{
};
...
{
public:
  static pdalboost::asio::detail::service_id<Type> id;

  // Constructor.
  service_base(pdalboost::asio::io_service& io_service)
    : pdalboost::asio::io_service::service(io_service)
  {
  }
};

template <typename Type>
pdalboost::asio::detail::service_id<Type> service_base<Type>::id;
