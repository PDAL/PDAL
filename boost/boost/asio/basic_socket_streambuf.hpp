...
template <typename Protocol,
  ...
  || defined(GENERATING_DOCUMENTATION)
    typename Time = pdalboost::posix_time::ptime,
...
public:
      ...
      const endpoint_type& endpoint)
  {
    ...
    {
      ec_ = pdalboost::asio::error::operation_aborted;
      return 0;
    }
   ...
   * buffer.
   */
  const pdalboost::system::error_code& puberror() const
  {
    return error();
  }
   ...
   * pdalboost::asio::error::operation_aborted.
   *
...
protected:
  int_type underflow()
  {
    if (gptr() == egptr())
    {
      ...
      this->get_service().async_receive(this->get_implementation(),
          pdalboost::asio::buffer(pdalboost::asio::buffer(get_buffer_) + putback_max),
          0, handler);
  ...
  int_type overflow(int_type c)
  {
    if (unbuffered_)
    {
      ...
      else
      {
        ...
        this->get_service().async_send(this->get_implementation(),
            pdalboost::asio::buffer(&ch, sizeof(char_type)), 0, handler);
    ...
    else
    {
      ...
      pdalboost::asio::const_buffer buffer =
        pdalboost::asio::buffer(pbase(), pptr() - pbase());
      while (pdalboost::asio::buffer_size(buffer) > 0)
      {
        if (timer_state_ == timer_has_expired)
        {
          ec_ = pdalboost::asio::error::operation_aborted;
        ...
        io_handler handler = { this };
        this->get_service().async_send(this->get_implementation(),
            pdalboost::asio::buffer(buffer), 0, handler);
   ...
   * buffer.
   */
  virtual const pdalboost::system::error_code& error() const
  {
    return ec_;
  }
...
private:
  ...
  void resolve_and_connect(const ResolverQuery& query)
  {
    ...
    {
      iterator_type end;
      ec_ = pdalboost::asio::error::host_not_found;
      while (ec_ && i != end)
      {
        this->basic_socket<Protocol, StreamSocketService>::close(ec_);
  ...
  struct timer_handler
  {
    ...
    void operator()(const pdalboost::system::error_code&)
    {
  ...
  void start_timer()
  {
    if (timer_state_ != timer_is_pending)
    {
      ...
      handler(pdalboost::system::error_code());
    }
  }

  enum { putback_max = 8 };
  enum { buffer_size = 512 };
  pdalboost::asio::detail::array<char, buffer_size> get_buffer_;
  pdalboost::asio::detail::array<char, buffer_size> put_buffer_;
  bool unbuffered_;
  pdalboost::system::error_code ec_;
