...
template <typename Stream>
std::size_t buffered_write_stream<Stream>::flush(pdalboost::system::error_code& ec)
{
  std::size_t bytes_written = write(next_layer_,
      buffer(storage_.data(), storage_.size()),
      transfer_all(), ec);
...
{
  if (pdalboost::asio::buffer_size(buffers) == 0)
    return 0;
...
namespace detail
{
  ...
  public:
    ...
    void operator()(const pdalboost::system::error_code& ec, std::size_t)
    {
      if (ec)
      {
      ...
      else
      {
        ...
        std::size_t bytes_avail = pdalboost::asio::buffer_size(buffers_);
        std::size_t length = bytes_avail < space_avail
          ? bytes_avail : space_avail;
        storage_.resize(orig_size + length);
        const std::size_t bytes_copied = pdalboost::asio::buffer_copy(
    ...
    BOOST_ASIO_MOVE_ARG(WriteHandler) handler)
{
  ...
  {
    next_layer_.async_write_some(pdalboost::asio::const_buffers_1(0, 0),
    ...
    const ConstBufferSequence& buffers)
{
  std::size_t orig_size = storage_.size();
  std::size_t space_avail = storage_.capacity() - orig_size;
  std::size_t bytes_avail = pdalboost::asio::buffer_size(buffers);
  std::size_t length = bytes_avail < space_avail ? bytes_avail : space_avail;
  storage_.resize(orig_size + length);
  return pdalboost::asio::buffer_copy(
