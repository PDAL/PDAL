...
template <typename Stream>
std::size_t buffered_read_stream<Stream>::fill(pdalboost::system::error_code& ec)
{
  detail::buffer_resize_guard<detail::buffered_stream_storage>
    resize_guard(storage_);
  std::size_t previous_size = storage_.size();
    ...
    BOOST_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  next_layer_.async_read_some(
      ...
      detail::buffered_fill_handler<BOOST_ASIO_HANDLER_TYPE(
        ReadHandler, void (pdalboost::system::error_code, std::size_t))>(
    ...
    BOOST_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  if (pdalboost::asio::buffer_size(buffers) == 0 || !storage_.empty())
  {
    next_layer_.async_read_some(pdalboost::asio::mutable_buffers_1(0, 0),
