...
namespace detail
{
  struct default_connect_condition
  {
    ...
    Iterator operator()(const pdalboost::system::error_code&, Iterator next)
    {
      return next;
    }
  };
    ...
    pdalboost::system::error_code& ec)
{
  ...
  if (!ec)
    ec = pdalboost::asio::error::not_found;
...
namespace detail
{
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const pdalboost::system::error_code& ec,
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const pdalboost::system::error_code&, Iterator&, Iterator&)
    {
    }
  };
  ...
  public:
    ...
    void operator()(pdalboost::system::error_code ec, int start = 0)
    {
      switch (start_ = start)
      {
        case 1:
        ...
        handler_(static_cast<const pdalboost::system::error_code&>(ec),
    ...
    Iterator begin, BOOST_ASIO_MOVE_ARG(ComposedConnectHandler) handler)
{
  ...
  detail::async_result_init<ComposedConnectHandler,
    void (pdalboost::system::error_code, Iterator)> init(
  ...
  detail::connect_op<Protocol, SocketService, Iterator,
    detail::default_connect_condition, BOOST_ASIO_HANDLER_TYPE(
      ComposedConnectHandler, void (pdalboost::system::error_code, Iterator))>(s,
        begin, Iterator(), detail::default_connect_condition(), init.handler)(
          pdalboost::system::error_code(), 1);
