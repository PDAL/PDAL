...
signal_set_service::signal_set_service(
  ...
  : io_service_(pdalboost::asio::use_service<io_service_impl>(io_service)),
#if !defined(BOOST_ASIO_WINDOWS) \
  && !defined(BOOST_ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
    reactor_(pdalboost::asio::use_service<reactor>(io_service)),
#endif // !defined(BOOST_ASIO_WINDOWS)
       //   && !defined(BOOST_ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
    next_(0),
    ...
    pdalboost::asio::io_service::fork_event fork_ev)
{
  ...
  switch (fork_ev)
  {
  case pdalboost::asio::io_service::fork_prepare:
    {
    ...
    break;
  case pdalboost::asio::io_service::fork_parent:
    ...
    break;
  case pdalboost::asio::io_service::fork_child:
    if (state->fork_prepared_)
    {
      pdalboost::asio::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
pdalboost::system::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl,
    int signal_number, pdalboost::system::error_code& ec)
{
...
pdalboost::system::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
    int signal_number, pdalboost::system::error_code& ec)
{
  // Check that the signal number is valid.
...
pdalboost::system::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
...
pdalboost::system::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    pdalboost::system::error_code& ec)
{
  ...
  op_queue<operation> ops;
  {
    ...
    {
      op->ec_ = pdalboost::asio::error::operation_aborted;
      impl.queue_.pop();
      ops.push(op);
    }
  }
...
void signal_set_service::open_descriptors()
{
  ...
  {
    pdalboost::system::error_code ec(errno,
        pdalboost::asio::error::get_system_category());
    pdalboost::asio::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(BOOST_ASIO_WINDOWS)
       //   && !defined(BOOST_ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
