...
{
public:
  explicit overlapped_wrapper(pdalboost::system::error_code& ec)
  {
    Internal = 0;
    InternalHigh = 0;
    Offset = 0;
    ...
    else
    {
      ...
      ec = pdalboost::system::error_code(last_error,
...
win_iocp_handle_service::win_iocp_handle_service(
  ...
  : iocp_service_(pdalboost::asio::use_service<win_iocp_io_service>(io_service)),
    mutex_(),
    impl_list_(0)
{
}
...
pdalboost::system::error_code win_iocp_handle_service::assign(
    win_iocp_handle_service::implementation_type& impl,
    const native_handle_type& handle, pdalboost::system::error_code& ec)
{
  if (is_open(impl))
  {
    ec = pdalboost::asio::error::already_open;
    return ec;
  }
...
pdalboost::system::error_code win_iocp_handle_service::close(
    win_iocp_handle_service::implementation_type& impl,
...
pdalboost::system::error_code win_iocp_handle_service::cancel(
    win_iocp_handle_service::implementation_type& impl,
    pdalboost::system::error_code& ec)
{
  if (!is_open(impl))
...
size_t win_iocp_handle_service::do_write(
    ...
    const pdalboost::asio::const_buffer& buffer, pdalboost::system::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = pdalboost::asio::error::bad_descriptor;
  ...
  BOOL ok = ::WriteFile(impl.handle_,
      pdalboost::asio::buffer_cast<LPCVOID>(buffer),
      static_cast<DWORD>(pdalboost::asio::buffer_size(buffer)), 0, &overlapped);
  if (!ok) 
  {
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_write_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const pdalboost::asio::const_buffer& buffer, operation* op)
{
  ...
  if (!is_open(impl))
  {
    iocp_service_.on_completion(op, pdalboost::asio::error::bad_descriptor);
  }
  else if (pdalboost::asio::buffer_size(buffer) == 0)
  {
    // A request to write 0 bytes on a handle is a no-op.
  ...
  else
  {
    ...
    BOOL ok = ::WriteFile(impl.handle_,
        pdalboost::asio::buffer_cast<LPCVOID>(buffer),
        static_cast<DWORD>(pdalboost::asio::buffer_size(buffer)),
...
size_t win_iocp_handle_service::do_read(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const pdalboost::asio::mutable_buffer& buffer, pdalboost::system::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = pdalboost::asio::error::bad_descriptor;
  ...
  BOOL ok = ::ReadFile(impl.handle_,
      pdalboost::asio::buffer_cast<LPVOID>(buffer),
      static_cast<DWORD>(pdalboost::asio::buffer_size(buffer)), 0, &overlapped);
  if (!ok) 
  {
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_read_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const pdalboost::asio::mutable_buffer& buffer, operation* op)
{
  update_cancellation_thread_id(impl);
  iocp_service_.work_started();
