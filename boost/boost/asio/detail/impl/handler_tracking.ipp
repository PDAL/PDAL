...
struct handler_tracking_timestamp
{
  ...
  {
#if defined(BOOST_ASIO_HAS_BOOST_DATE_TIME)
    pdalboost::posix_time::ptime epoch(pdalboost::gregorian::date(1970, 1, 1));
    pdalboost::posix_time::time_duration now =
      pdalboost::posix_time::microsec_clock::universal_time() - epoch;
#elif defined(BOOST_ASIO_HAS_STD_CHRONO)
    typedef chrono_time_traits<std::chrono::system_clock,
        pdalboost::asio::wait_traits<std::chrono::system_clock> > traits_helper;
    traits_helper::posix_time_duration now(
        std::chrono::system_clock::now().time_since_epoch());
#elif defined(BOOST_ASIO_HAS_BOOST_CHRONO)
    typedef chrono_time_traits<pdalboost::chrono::system_clock,
        pdalboost::asio::wait_traits<pdalboost::chrono::system_clock> > traits_helper;
    traits_helper::posix_time_duration now(
        pdalboost::chrono::system_clock::now().time_since_epoch());
#endif
...
void handler_tracking::completion::invocation_begin(
    const pdalboost::system::error_code& ec, int signal_number)
{
  handler_tracking_timestamp timestamp;
...
void handler_tracking::completion::invocation_begin(
    const pdalboost::system::error_code& ec, const char* arg)
{
  handler_tracking_timestamp timestamp;
