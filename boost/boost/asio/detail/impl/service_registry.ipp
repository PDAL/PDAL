...
service_registry::~service_registry()
{
  ...
  pdalboost::asio::io_service::service* service = first_service_;
  while (service)
  {
  ...
  {
    pdalboost::asio::io_service::service* next_service = first_service_->next_;
    destroy(first_service_);
    first_service_ = next_service;
  }
}

void service_registry::notify_fork(pdalboost::asio::io_service::fork_event fork_ev)
{
  // Make a copy of all of the services while holding the lock. We don't want
  // to hold the lock while calling into each service, as it may try to call
  // back into this class.
  std::vector<pdalboost::asio::io_service::service*> services;
  {
...
void service_registry::init_key(pdalboost::asio::io_service::service::key& key,
...
void service_registry::destroy(pdalboost::asio::io_service::service* service)
{
  delete service;
}

pdalboost::asio::io_service::service* service_registry::do_use_service(
    ...
    pdalboost::asio::io_service::service* new_service)
{
  if (&owner_ != &new_service->get_io_service())
    pdalboost::asio::detail::throw_exception(invalid_service_owner());
  ...
  {
    if (keys_match(service->key_, key))
      pdalboost::asio::detail::throw_exception(service_already_exists());
    service = service->next_;
  }
