...
class win_iocp_io_service
  : public pdalboost::asio::detail::service_base<win_iocp_io_service>
{
public:

  // Constructor. Specifies a concurrency hint that is passed through to the
  // underlying I/O completion port.
  BOOST_ASIO_DECL win_iocp_io_service(pdalboost::asio::io_service& io_service,
  ...
  BOOST_ASIO_DECL pdalboost::system::error_code register_handle(
      HANDLE handle, pdalboost::system::error_code& ec);

  // Run the event loop until stopped or no more work.
  BOOST_ASIO_DECL size_t run(pdalboost::system::error_code& ec);

  // Run until stopped or one operation is performed.
  BOOST_ASIO_DECL size_t run_one(pdalboost::system::error_code& ec);

  // Poll for operations without blocking.
  BOOST_ASIO_DECL size_t poll(pdalboost::system::error_code& ec);

  // Poll for one operation without blocking.
  BOOST_ASIO_DECL size_t poll_one(pdalboost::system::error_code& ec);
  ...
  // starting the operation.
  BOOST_ASIO_DECL void on_completion(win_iocp_operation* op,
      const pdalboost::system::error_code& ec, DWORD bytes_transferred = 0);
...
private:
  ...
  // either 0 or 1).
  BOOST_ASIO_DECL size_t do_one(bool block, pdalboost::system::error_code& ec);
