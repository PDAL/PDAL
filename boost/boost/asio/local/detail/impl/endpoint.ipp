...
{
  if (new_size > sizeof(pdalboost::asio::detail::sockaddr_un_type))
  {
  ...
  else
  {
    path_length_ = new_size
      - offsetof(pdalboost::asio::detail::sockaddr_un_type, sun_path);
...
void endpoint::init(const char* path_name, std::size_t path_length)
{
  if (path_length > sizeof(data_.local.sun_path) - 1)
  {
    ...
    pdalboost::system::error_code ec(pdalboost::asio::error::name_too_long);
    pdalboost::asio::detail::throw_error(ec);
  }

  using namespace std; // For memcpy.
  data_.local = pdalboost::asio::detail::sockaddr_un_type();
