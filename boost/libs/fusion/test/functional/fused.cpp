...
template <class Base = pdalboost::blank>
struct test_func
    : Base
{
...
int main()
{
    ...
    BOOST_MPL_ASSERT((pdalboost::is_same<pdalboost::result_of<ff(vec)>::type, int>));
    BOOST_TEST(fused_func(lv_vec) == 1);
    BOOST_MPL_ASSERT((pdalboost::is_same<pdalboost::result_of<ffr(vec)>::type, int>));
    BOOST_TEST(fused_func_c(lv_vec) == 0);
    BOOST_MPL_ASSERT((pdalboost::is_same<pdalboost::result_of<ffc(vec)>::type, int>));
    BOOST_TEST(fused_func_c2(lv_vec) == 0);
    BOOST_TEST(fused_func_ref(lv_vec) == 1);
    BOOST_MPL_ASSERT((pdalboost::is_same<pdalboost::result_of<ffcr(vec)>::type, int>));
