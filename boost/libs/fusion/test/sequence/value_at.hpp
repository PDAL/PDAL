...
#define FUSION_VALUE_AT(S, N) pdalboost::fusion::result_of::value_at_c<S, N>
#endif
...
test()
{
    ...
    BOOST_STATIC_ASSERT(!(
        pdalboost::is_const<FUSION_VALUE_AT(seq_type, 0)::type>::value));

    // constness should not affect
    BOOST_STATIC_ASSERT(!(
        pdalboost::is_const<FUSION_VALUE_AT(const seq_type, 0)::type>::value));

    BOOST_STATIC_ASSERT(!(
        pdalboost::is_const<FUSION_VALUE_AT(seq_type, 1)::type>::value));

    // constness should not affect
    BOOST_STATIC_ASSERT(!(
        pdalboost::is_const<FUSION_VALUE_AT(const seq_type, 1)::type>::value));
