...
void test_set(Set const& set)
{
    ...
    BOOST_STATIC_ASSERT(pdalboost::fusion::result_of::size<Set>::value == 3);
    BOOST_TEST((*find<int>(set) == 1));
    BOOST_TEST((*find<double>(set) == 1.5));
    BOOST_TEST((*find<std::string>(set) == "hello"));
}

typedef pdalboost::mpl::int_<1> _1;
typedef pdalboost::mpl::int_<2> _2;
typedef pdalboost::mpl::int_<3> _3;
typedef pdalboost::mpl::int_<4> _4;
...
void test_map(Map const& map)
{
    ...
    BOOST_STATIC_ASSERT(pdalboost::fusion::result_of::size<Map>::value == 3);
