...
namespace
{
    ...
    double time_for_fusion_accumulate(int& j)
    {
        ...
        do
        {
            ...
            for(counter = 0; counter < iter; ++counter)
            {
                i = pdalboost::fusion::accumulate(arr, 0, poly_add());
                static_cast<void>(i);
            }
            runtime = tim.elapsed();
            iter *= 2;
    ...
    double time_for_std_inner_product(int& j)
    {
        ...
        double runtime = 0;
        double run;
        pdalboost::array<int, N> arr1;
        pdalboost::array<int, N> arr2;
    ...
    double time_for_fusion_inner_product(int& j)
    {
        ...
        do
        {
            ...
            for(counter = 0; counter < iter; ++counter)
            {
                i = pdalboost::fusion::accumulate(
                    pdalboost::fusion::transform(arr1, arr2, poly_mult()), 0, poly_add());
                static_cast<void>(i);
            }
            runtime = tim.elapsed();
            iter *= 2;
    ...
    struct poly_combine
    {
        ...
        {
            return lhs + pdalboost::fusion::at_c<0>(rhs) * pdalboost::fusion::at_c<1>(rhs);
        }
    };
    ...
    double time_for_fusion_inner_product2(int& j)
    {
        ...
        do
        {
            ...
            {
                i = pdalboost::fusion::accumulate(
                    pdalboost::fusion::zip(arr1, arr2), 0, poly_combine());
                static_cast<void>(i);
            }
            runtime = tim.elapsed();
            iter *= 2;
