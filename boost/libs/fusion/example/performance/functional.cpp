...
namespace 
{
    struct fused_sum
    {
        ...
        {
            int state = 0;
            return pdalboost::fusion::fold(seq, state, sum_op());
        }
    ...
    double call_fused_ra(F const & func, int & j) 
    {
        ...
        do
        {
            pdalboost::fusion::vector<> v0;
            pdalboost::fusion::vector<int> v1(0);
            pdalboost::fusion::vector<int,int> v2(0,1);
            pdalboost::fusion::vector<int,int,int> v3(0,1,2);
            pdalboost::fusion::vector<int,int,int,int> v4(0,1,2,3);
            tim.restart();
            for(counter = 0; counter < iter; ++counter)
            {
                i += func(v0);
    ...
    double call_fused(F const & func, int & j) 
    {
        ...
        do
        {
            pdalboost::fusion::list<> l0;
            pdalboost::fusion::list<int> l1(0);
            pdalboost::fusion::list<int,int> l2(0,1);
            pdalboost::fusion::list<int,int,int> l3(0,1,2);
            pdalboost::fusion::list<int,int,int,int> l4(0,1,2,3);
            tim.restart();
            for(counter = 0; counter < iter; ++counter)
            {
                i += func(l0);
...
int main()
{
    ...
    {
        pdalboost::lightweight_forward_adapter< pdalboost::fusion::unfused<F> > f;
        std::cout << "lightweight_forward_adapter< unfused<F> >      " << call_unfused(f,res) << std::endl;
        total += res;
    }
    {
        pdalboost::forward_adapter< pdalboost::fusion::unfused<F> > f;
    ...
    {
        pdalboost::fusion::fused_function_object<U> f;
    ...
    {
        pdalboost::lightweight_forward_adapter< pdalboost::fusion::unfused< pdalboost::fusion::fused_function_object<U> > > f;
        std::cout << "lightweight_forward_adapter< unfused<fused_function_object<U> > >" << call_unfused(f,res) << std::endl;
        total += res;
    }
    {
        pdalboost::forward_adapter< pdalboost::fusion::unfused< pdalboost::fusion::fused_function_object<U> > > f;
