...
//  parsed by pdalboost::lexical_cast to make sure that the compiler is not 
...
//  fusion::accumulate:
//  
//      this->sum += pdalboost::fusion::accumulate(seq, 0, poly_add());
//
...
namespace
{
    struct poly_add
    {
        ...
        struct result<poly_add(Lhs, Rhs)>
            : pdalboost::remove_reference<Lhs>
        {};
    ...
    {
        test::live_code = pdalboost::fusion::accumulate(seq, 0, poly_add());
    }
}

// We'll initialize the sequences from numeric strings that
// pass through pdalboost::lexical_cast to make sure that the
// compiler is not optimizing by replacing the computation 
// with constant results computed at compile time.
#define INIT(z, n, text) pdalboost::lexical_cast<int>(BOOST_PP_STRINGIZE(n))
