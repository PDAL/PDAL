...
# endif 
  ...
  //
  using pdalboost::python::objects::dynamic_id_function;
  ...
  inline type_index_t::iterator type_position(class_id type)
  {
      ...
      return std::lower_bound(
          ...
          , pdalboost::make_tuple(type, vertex_t(), dynamic_id_function(0))
          , pdalboost::bind<bool>(std::less<class_id>()
               , pdalboost::bind<class_id>(select1st<entry>(), _1)
               , pdalboost::bind<class_id>(select1st<entry>(), _2)));
  }
  ...
  inline type_index_t::iterator demand_type(class_id type)
  {
      ...
      unused_variable(v2);
      assert(v == v2);
      return type_index().insert(p, pdalboost::make_tuple(type, v, dynamic_id_function(0)));
  }
  ...
  inline void* convert_type(void* const p, class_id src_t, class_id dst_t, bool polymorphic)
  {
      ...
      // info
      pdalboost::python::objects::dynamic_id_t dynamic_id = polymorphic
      ...
      cache_element seek(pdalboost::make_tuple(src_t, dst_t, offset, dynamic_id.second));
      cache_t& c = cache();
