...
{
#if PY_VERSION_HEX >= 0x03000000
    return x ? PyUnicode_FromString(x) : pdalboost::python::detail::none();
#else
    return x ? PyString_FromString(x) : pdalboost::python::detail::none();
#endif
}
  
BOOST_PYTHON_DECL PyObject* do_return_to_python(PyObject* x)
{
    return x ? x : pdalboost::python::detail::none();
}
  
BOOST_PYTHON_DECL PyObject* do_arg_to_python(PyObject* x)
{
    if (x == 0)
        return pdalboost::python::detail::none();
