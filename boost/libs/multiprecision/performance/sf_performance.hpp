...
#  include "libs/math/test/bessel_i_data.ipp"
      ...
      {
         r += pdalboost::math::cyl_bessel_i(Real(bessel_i_int_data[i][0]), Real(bessel_i_int_data[i][1]));
      }
      for(unsigned i = 0; i < bessel_i_data.size(); ++i)
      {
         r += pdalboost::math::cyl_bessel_i(Real(bessel_i_data[i][0]), Real(bessel_i_data[i][1]));
      }

#include "libs/math/test/bessel_j_int_data.ipp"
      for(unsigned i = 0; i < bessel_j_int_data.size(); ++i)
      {
         r += pdalboost::math::cyl_bessel_j(Real(bessel_j_int_data[i][0]), Real(bessel_j_int_data[i][1]));
      }

#include "libs/math/test/bessel_j_data.ipp"
      for(unsigned i = 0; i < bessel_j_data.size(); ++i)
      {
         r += pdalboost::math::cyl_bessel_j(Real(bessel_j_data[i][0]), Real(bessel_j_data[i][1]));
      }

#include "libs/math/test/bessel_j_large_data.ipp"
      for(unsigned i = 0; i < bessel_j_large_data.size(); ++i)
      {
         r += pdalboost::math::cyl_bessel_j(Real(bessel_j_large_data[i][0]), Real(bessel_j_large_data[i][1]));
      }

#include "libs/math/test/sph_bessel_data.ipp"
      for(unsigned i = 0; i < sph_bessel_data.size(); ++i)
      {
         r += pdalboost::math::sph_bessel(static_cast<unsigned>(sph_bessel_data[i][0]), Real(sph_bessel_data[i][1]));
      }
...
Real test_polynomial()
{
   ...
   for(Real k = 2; k < 1000; ++k)
      result += pdalboost::math::tools::evaluate_polynomial(t, k);
...
#include "libs/math/test/nct.ipp"
   ...
   {
      try{
         result += quantile(pdalboost::math::non_central_t_distribution<Real>(nct[i][0], nct[i][1]), nct[i][3]);
         result += cdf(pdalboost::math::non_central_t_distribution<Real>(nct[i][0], nct[i][1]), nct[i][2]);
      }
...
void poly_allocation_test(const char* name, Real x)
{
   static const unsigned a[] = { 2, 3, 4, 5, 6, 7, 8 };
   allocation_count = 0;
   Real result = pdalboost::math::tools::evaluate_polynomial(a, x);
   std::cout << "Allocation count for type " << name << " = " << allocation_count << std::endl;
}
...
void time_proc(const char* name, Real (*proc)(), unsigned threads = 1)
{
   try{
      static Real total = 0;
      allocation_count = 0;
      pdalboost::chrono::duration<double> time;
      stopwatch<pdalboost::chrono::high_resolution_clock> c;
      total += proc();
      time = c.elapsed();
