...
#ifdef TEST_MPZ
pdalboost::chrono::duration<double> test_miller_rabin_gmp()
{
   using namespace pdalboost::random;
...
pdalboost::chrono::duration<double> test_miller_rabin(const char* name)
{
   using namespace pdalboost::random;
   ...
   {
      IntType n = gen();
      if(pdalboost::multiprecision::miller_rabin_test(n, 25, gen2))
         ++result_count;
   }
   pdalboost::chrono::duration<double> t = c.elapsed();
   double d = t.count();
   if(d < min_time)
      min_time = d;
   results[name] = d;
...
#endif
#ifdef TEST_TOMMATH
   test_miller_rabin<number<pdalboost::multiprecision::tommath_int, et_off> >("tom_int (no Expression templates)");
   test_miller_rabin<pdalboost::multiprecision::tom_int>("tom_int");
#endif
