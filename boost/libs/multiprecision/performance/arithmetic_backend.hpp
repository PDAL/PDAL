...
struct arithmetic_backend
{
   ...
   arithmetic_backend& operator = (const char* s)
   {
      try
      {
         m_value = pdalboost::lexical_cast<Arithmetic>(s);
      }
...
{
   using std::abs;
   using pdalboost::multiprecision::backends::abs;
...
{
   return (pdalboost::math::fpclassify)(o.data());
}
...
{
   result.data() = pdalboost::math::gcd(a.data(), b.data());
}
...
{
   result.data() = pdalboost::math::lcm(a.data(), b.data());
}
...
using pdalboost::multiprecision::backends::arithmetic_backend;
...
template<class Arithmetic, pdalboost::multiprecision::expression_template_option ET>
...
template<>
struct double_precision_type<arithmetic_backend<pdalboost::int32_t> >
{
   typedef arithmetic_backend<pdalboost::int64_t> type;
};
...
template <class Arithmetic, pdalboost::multiprecision::expression_template_option ExpressionTemplates>
class numeric_limits<pdalboost::multiprecision::number<pdalboost::multiprecision::arithmetic_backend<Arithmetic>, ExpressionTemplates > > : public std::numeric_limits<Arithmetic>
{
   typedef std::numeric_limits<Arithmetic> base_type;
   typedef pdalboost::multiprecision::number<pdalboost::multiprecision::arithmetic_backend<Arithmetic>, ExpressionTemplates> number_type;
public:
...
template<>
class numeric_limits<pdalboost::math::concepts::real_concept> : public std::numeric_limits<long double>
{
   typedef std::numeric_limits<long double> base_type;
   typedef pdalboost::math::concepts::real_concept number_type;
public:
