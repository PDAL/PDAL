...
template <class B, pdalboost::multiprecision::expression_template_option ET, class T>
BOOST_FORCEINLINE void mul_2n(pdalboost::multiprecision::number<B, ET>& r, const T& a, const T& b)
{
   multiply(r, a, b);
}

BOOST_FORCEINLINE void mul_2n(int128_t& r, const pdalboost::int64_t& a, const pdalboost::int64_t& b)
{
...
void do_calc(const char* name)
{
   ...
   pdalboost::uint64_t flips = 0;
   pdalboost::uint64_t calcs = 0;
   ...
   double t = pdalboost::chrono::duration_cast<pdalboost::chrono::duration<double> >(w.elapsed()).count();
...
dt_dat generate_quadrilateral()
{
   ...
   static pdalboost::random::uniform_int_distribution<> dist(INT_MIN/2, INT_MAX/2);

   dt_dat result;

   result.ax = dist(gen);
   result.ay = dist(gen);
   result.bx = pdalboost::random::uniform_int_distribution<>(result.ax, INT_MAX/2)(gen);  // bx is to the right of ax.
   result.by = dist(gen);
   result.cx = dist(gen);
   result.cy = pdalboost::random::uniform_int_distribution<>(result.cx > result.bx ? result.by : result.ay, INT_MAX/2)(gen);  // cy is below at least one of ay and by.
   result.dx = pdalboost::random::uniform_int_distribution<>(result.cx, INT_MAX/2)(gen);  // dx is to the right of cx.
   result.dy = pdalboost::random::uniform_int_distribution<>(result.cx > result.bx ? result.by : result.ay, INT_MAX/2)(gen);  // cy is below at least one of ay and by.
...
int main()
{
   using namespace pdalboost::multiprecision;
   std::cout << "loading data...\n";
   load_data();

   std::cout << "calculating...\n";

   do_calc<test_traits<pdalboost::int64_t, pdalboost::int64_t> >("int64_t, int64_t");
   do_calc<test_traits<number<arithmetic_backend<pdalboost::int64_t>, et_off>, number<arithmetic_backend<pdalboost::int64_t>, et_off> > >("arithmetic_backend<int64_t>, arithmetic_backend<int64_t>");
   do_calc<test_traits<pdalboost::int64_t, number<arithmetic_backend<pdalboost::int64_t>, et_off> > >("int64_t, arithmetic_backend<int64_t>");
   do_calc<test_traits<number<cpp_int_backend<64, 64, pdalboost::multiprecision::signed_magnitude, pdalboost::multiprecision::unchecked, void>, et_off>, number<cpp_int_backend<64, 64, pdalboost::multiprecision::signed_magnitude, pdalboost::multiprecision::unchecked, void>, et_off> > >("multiprecision::int64_t, multiprecision::int64_t");

   do_calc<test_traits<pdalboost::int64_t, ::int128_t> >("int64_t, int128_t");
   do_calc<test_traits<pdalboost::int64_t, pdalboost::multiprecision::int128_t> >("int64_t, pdalboost::multiprecision::int128_t");
   do_calc<test_traits<pdalboost::int64_t, number<cpp_int_backend<128, 128, pdalboost::multiprecision::signed_magnitude, pdalboost::multiprecision::unchecked, void>, et_on> > >("int64_t, int128_t (ET)");
   do_calc<test_traits<number<cpp_int_backend<64, 64, pdalboost::multiprecision::signed_magnitude, pdalboost::multiprecision::unchecked, void>, et_off>, pdalboost::multiprecision::int128_t > >("multiprecision::int64_t, multiprecision::int128_t");

   do_calc<test_traits<pdalboost::int64_t, cpp_int> >("int64_t, cpp_int");
   do_calc<test_traits<pdalboost::int64_t, number<cpp_int_backend<>, et_off> > >("int64_t, cpp_int (no ET's)");
   do_calc<test_traits<pdalboost::int64_t, number<cpp_int_backend<128> > > >("int64_t, cpp_int(128-bit cache)");
   do_calc<test_traits<pdalboost::int64_t, number<cpp_int_backend<128>, et_off> > >("int64_t, cpp_int (128-bit Cache no ET's)");
