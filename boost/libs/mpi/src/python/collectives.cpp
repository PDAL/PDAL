...
object all_gather(const communicator& comm, object value)
{
  ...
  pdalboost::mpi::all_gather(comm, value, values);

  pdalboost::python::list l;
  for (int i = 0; i < comm.size(); ++i)
    l.append(values[i]);
  return pdalboost::python::tuple(l);
}
...
object all_to_all(const communicator& comm, object in_values)
{
  ...
  pdalboost::mpi::all_to_all(comm, in_values_vec, out_values_vec);
...
{
  pdalboost::mpi::broadcast(comm, value, root);
  return value;
}
...
object gather(const communicator& comm, object value, int root)
{
  if (comm.rank() == root) {
    std::vector<object> values;
    pdalboost::mpi::gather(comm, value, values, root);
    ...
    pdalboost::mpi::gather(comm, value, root);
    return object();
  }
}
...
object reduce(const communicator& comm, object value, object op, int root)
{
  if (comm.rank() == root) {
    object out_value;
    pdalboost::mpi::reduce(comm, value, out_value, op, root);
    return out_value;
  } else {
    pdalboost::mpi::reduce(comm, value, op, root);
    return object();
  }
}
...
object scatter(const communicator& comm, object values, int root)
{
  ...
  if (comm.rank() == root) {
    ...
    pdalboost::mpi::scatter(comm, values_vec, result, root);
  } else {
    pdalboost::mpi::scatter(comm, result, root);
  }
  return result;
}
