  ...
  if (m_handler) {
    ...
    if (error_code == MPI_ERR_IN_STATUS) {
      ...
      if (stats[0].MPI_ERROR == MPI_SUCCESS 
        ...
        pdalboost::throw_exception(exception("MPI_Waitall", stats[1].MPI_ERROR));
      else
        pdalboost::throw_exception(exception("MPI_Waitall", stats[0].MPI_ERROR));
    } else if (error_code != MPI_SUCCESS) {
      // There was an error somewhere in the MPI_Waitall call; throw
      // an exception for it.
      pdalboost::throw_exception(exception("MPI_Waitall", error_code));
    } 
...
optional<status> request::test()
{
  if (m_handler) {
    ...
    if (error_code == MPI_ERR_IN_STATUS) {
      ...
      if (stats[0].MPI_ERROR == MPI_SUCCESS 
          || stats[0].MPI_ERROR == MPI_ERR_PENDING)
        pdalboost::throw_exception(exception("MPI_Testall", stats[1].MPI_ERROR));
      else
        pdalboost::throw_exception(exception("MPI_Testall", stats[0].MPI_ERROR));
    } else if (error_code != MPI_SUCCESS) {
      // There was an error somewhere in the MPI_Testall call; throw
      // an exception for it.
      pdalboost::throw_exception(exception("MPI_Testall", error_code));
    }
