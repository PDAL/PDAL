...
struct paraguay_dst_traits { 
  typedef pdalboost::gregorian::date           date_type;
  typedef pdalboost::gregorian::date::day_type day_type;
  typedef pdalboost::gregorian::date::month_type month_type;
  typedef pdalboost::gregorian::date::year_type year_type;
  typedef pdalboost::date_time::partial_date<pdalboost::gregorian::date> start_rule_functor;
  typedef pdalboost::date_time::partial_date<pdalboost::gregorian::date> end_rule_functor;
  static day_type start_day(year_type) {return 1;}
  static month_type start_month(year_type) {return pdalboost::date_time::Oct;}
  static day_type end_day(year_type) {return 1;}
  static month_type end_month(year_type) {return pdalboost::date_time::Mar;}
...
main() 
{
  ...
  typedef pdalboost::date_time::us_dst_rules<date, time_duration, 120, 60> us_dst;
  ...
  check("check if time is dst -- not",   
        us_dst::local_is_dst(t.date(), t.time_of_day())==pdalboost::date_time::is_not_in_dst);
  check("label on dst boundary invalid", 
        us_dst::local_is_dst(t3a.date(),t3a.time_of_day())==pdalboost::date_time::invalid_time_label);
  check("label on dst boundary invalid",   
        us_dst::local_is_dst(t3b.date(),t3b.time_of_day())==pdalboost::date_time::invalid_time_label);
   check("check if time is dst -- not",   
         us_dst::local_is_dst(t4.date(),t4.time_of_day())==pdalboost::date_time::is_not_in_dst);
   check("check if time is dst -- yes",   
         us_dst::local_is_dst(t5.date(),t5.time_of_day())==pdalboost::date_time::is_in_dst);

   check("check if time is dst -- not",   
         us_dst::local_is_dst(t6.date(),t6.time_of_day())==pdalboost::date_time::is_in_dst);
   check("check if time is dst -- ambig", 
         us_dst::local_is_dst(t7.date(),t7.time_of_day())==pdalboost::date_time::ambiguous);
   check("check if time is dst -- ambig", 
         us_dst::local_is_dst(t8.date(),t8.time_of_day())==pdalboost::date_time::ambiguous);
   check("check if time is dst -- not",   
         us_dst::local_is_dst(t9.date(),t9.time_of_day())==pdalboost::date_time::is_not_in_dst);


  //Now try a local without dst
  typedef pdalboost::date_time::null_dst_rules<date, time_duration> no_dst_adj;

  check("check null dst rules",   
        no_dst_adj::local_is_dst(t4.date(),t4.time_of_day())==pdalboost::date_time::is_not_in_dst);
  check("check null dst rules",   
        no_dst_adj::local_is_dst(t5.date(),t5.time_of_day())==pdalboost::date_time::is_not_in_dst);
  check("check null dst rules",   
        no_dst_adj::utc_is_dst(t4.date(),t4.time_of_day())==pdalboost::date_time::is_not_in_dst);
  check("check null dst rules",   
        no_dst_adj::utc_is_dst(t5.date(),t5.time_of_day())==pdalboost::date_time::is_not_in_dst);
  ...
  typedef pdalboost::date_time::dst_calculator<date,time_duration> dstcalc;
  //clearly not in dst
  pdalboost::date_time::time_is_dst_result a1 =
    dstcalc::local_is_dst(date(2002,May,1),hours(3),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);

  check("check southern not dst",  a1==pdalboost::date_time::is_not_in_dst);

  pdalboost::date_time::time_is_dst_result a2 =
    dstcalc::local_is_dst(date(2002,Jan,1),hours(3),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);

  check("check southern is dst",  a2==pdalboost::date_time::is_in_dst);

  pdalboost::date_time::time_is_dst_result a3 =
    dstcalc::local_is_dst(date(2002,Oct,28),hours(3),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);

  check("check southern is dst",  a3==pdalboost::date_time::is_in_dst);
  pdalboost::date_time::time_is_dst_result a4 =
    dstcalc::local_is_dst(date(2002,Oct,27),time_duration(1,59,59),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);
  check("check southern boundary-not dst",  a4==pdalboost::date_time::is_not_in_dst);
  pdalboost::date_time::time_is_dst_result a5 =
    dstcalc::local_is_dst(date(2002,Oct,27),hours(3),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);
  check("check southern boundary-is dst",  a5==pdalboost::date_time::is_in_dst);
  pdalboost::date_time::time_is_dst_result a6 =
    dstcalc::local_is_dst(date(2002,Oct,27),hours(2),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);
  check("check southern boundary-invalid time",  a6==pdalboost::date_time::invalid_time_label);
  pdalboost::date_time::time_is_dst_result a7 =
    dstcalc::local_is_dst(date(2002,Mar,31),time_duration(1,59,59),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);
  check("check southern boundary-is dst",  a7==pdalboost::date_time::is_in_dst);
  pdalboost::date_time::time_is_dst_result a8 =
    dstcalc::local_is_dst(date(2002,Mar,31),time_duration(2,0,0),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);
  check("check southern boundary-ambiguous",  a8==pdalboost::date_time::ambiguous);
  pdalboost::date_time::time_is_dst_result a9 =
    dstcalc::local_is_dst(date(2002,Mar,31),time_duration(2,59,59),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);
  check("check southern boundary-ambiguous",  a9==pdalboost::date_time::ambiguous);
  pdalboost::date_time::time_is_dst_result a10 =
    dstcalc::local_is_dst(date(2002,Mar,31),time_duration(3,0,0),
                          dst_start2, 120,
                          dst_end2, 180,
                          60);
  check("check southern boundary-not",  a10==pdalboost::date_time::is_not_in_dst);

  /******************** post release 1 -- new dst calc engine ********/
  
  typedef pdalboost::date_time::us_dst_trait<date> us_dst_traits;
  typedef pdalboost::date_time::dst_calc_engine<date, time_duration, us_dst_traits>
    us_dst_calc2;
    
  {
    ...
    check("check if time is dst -- not",   
          us_dst_calc2::local_is_dst(t.date(), t.time_of_day())==pdalboost::date_time::is_not_in_dst);
    check("label on dst boundary invalid", 
          us_dst_calc2::local_is_dst(t3a.date(),t3a.time_of_day())==pdalboost::date_time::invalid_time_label);
    check("label on dst boundary invalid",   
          us_dst_calc2::local_is_dst(t3b.date(),t3b.time_of_day())==pdalboost::date_time::invalid_time_label);
    check("check if time is dst -- not",   
          us_dst_calc2::local_is_dst(t4.date(),t4.time_of_day())==pdalboost::date_time::is_not_in_dst);
    check("check if time is dst -- yes",   
          us_dst_calc2::local_is_dst(t5.date(),t5.time_of_day())==pdalboost::date_time::is_in_dst);

    check("check if time is dst -- not",   
          us_dst_calc2::local_is_dst(t6.date(),t6.time_of_day())==pdalboost::date_time::is_in_dst);
    check("check if time is dst -- ambig", 
          us_dst_calc2::local_is_dst(t7.date(),t7.time_of_day())==pdalboost::date_time::ambiguous);
    check("check if time is dst -- ambig", 
          us_dst_calc2::local_is_dst(t8.date(),t8.time_of_day())==pdalboost::date_time::ambiguous);
    check("check if time is dst -- not",   
          us_dst_calc2::local_is_dst(t9.date(),t9.time_of_day())==pdalboost::date_time::is_not_in_dst);
  }
  {
  ...
  /******************** post release 1 -- new dst calc engine - eu dst ********/
   ...
   typedef pdalboost::date_time::eu_dst_trait<date> eu_dst_traits;
   typedef pdalboost::date_time::dst_calc_engine<date, time_duration, eu_dst_traits>
     eu_dst_calc;
  ...
  check("eu label on dst boundary invalid", 
        eu_dst_calc::local_is_dst(eu_invalid1.date(),eu_invalid1.time_of_day())==pdalboost::date_time::invalid_time_label);
  check("eu label on dst boundary invalid",   
        eu_dst_calc::local_is_dst(eu_invalid2.date(),eu_invalid2.time_of_day())==pdalboost::date_time::invalid_time_label);
  check("eu check if time is dst -- not",   
        eu_dst_calc::local_is_dst(eu_notdst1.date(),eu_notdst1.time_of_day())==pdalboost::date_time::is_not_in_dst);
  check("check if time is dst -- yes",   
        eu_dst_calc::local_is_dst(eu_isdst1.date(),eu_isdst1.time_of_day())==pdalboost::date_time::is_in_dst);
  //backward shift boundary
  check("eu check if time is dst -- yes",   
        eu_dst_calc::local_is_dst(eu_isdst2.date(),eu_isdst2.time_of_day())==pdalboost::date_time::is_in_dst);
  check("eu check if time is dst -- ambig", 
        eu_dst_calc::local_is_dst(eu_amgbig1.date(),eu_amgbig1.time_of_day())==pdalboost::date_time::ambiguous);
  check("eu check if time is dst -- ambig", 
        eu_dst_calc::local_is_dst(eu_amgbig2.date(),eu_amgbig2.time_of_day())==pdalboost::date_time::ambiguous);
  check("eu check if time is dst -- not",   
        eu_dst_calc::local_is_dst(eu_notdst2.date(),eu_notdst2.time_of_day())==pdalboost::date_time::is_not_in_dst);
...
/******************** post release 1 -- new dst calc engine - gb dst ********/
  ...
  */
  ...
  typedef pdalboost::date_time::uk_dst_trait<date> uk_dst_traits;

  typedef pdalboost::date_time::dst_calc_engine<date, time_duration, uk_dst_traits>  uk_dst_calc;
  ...
  check("uk label on dst boundary invalid", 
        uk_dst_calc::local_is_dst(uk_invalid1.date(),uk_invalid1.time_of_day())==pdalboost::date_time::invalid_time_label);
  check("uk label on dst boundary invalid",   
        uk_dst_calc::local_is_dst(uk_invalid2.date(),uk_invalid2.time_of_day())==pdalboost::date_time::invalid_time_label);
  check("uk check if time is dst -- not",   
        uk_dst_calc::local_is_dst(uk_notdst1.date(),uk_notdst1.time_of_day())==pdalboost::date_time::is_not_in_dst);
  check("uk check if time is dst -- yes",   
        uk_dst_calc::local_is_dst(uk_isdst1.date(),uk_isdst1.time_of_day())==pdalboost::date_time::is_in_dst);
  //backward shift boundary
  check("uk check if time is dst -- yes",   
        uk_dst_calc::local_is_dst(uk_isdst2.date(),uk_isdst2.time_of_day())==pdalboost::date_time::is_in_dst);
  check("uk check if time is dst -- ambig", 
        uk_dst_calc::local_is_dst(uk_amgbig1.date(),uk_amgbig1.time_of_day())==pdalboost::date_time::ambiguous);
  check("uk check if time is dst -- ambig", 
        uk_dst_calc::local_is_dst(uk_amgbig2.date(),uk_amgbig2.time_of_day())==pdalboost::date_time::ambiguous);
  check("uk check if time is dst -- not",   
        uk_dst_calc::local_is_dst(uk_notdst2.date(),uk_notdst2.time_of_day())==pdalboost::date_time::is_not_in_dst);
...
//   //Define dst rule for Paraguay which is transitions forward on Oct 1 and back Mar 1
  ...
  typedef pdalboost::date_time::dst_calc_engine<date, time_duration, 
...

  {
    ...
    check("pg label on dst boundary invalid", 
          pg_dst_calc::local_is_dst(pg_invalid1.date(),pg_invalid1.time_of_day())==pdalboost::date_time::invalid_time_label);
    check("pg label on dst boundary invalid",   
          pg_dst_calc::local_is_dst(pg_invalid2.date(),pg_invalid2.time_of_day())==pdalboost::date_time::invalid_time_label);
    check("pg check if time is dst -- not",   
          pg_dst_calc::local_is_dst(pg_notdst1.date(),pg_notdst1.time_of_day())==pdalboost::date_time::is_not_in_dst);
    check("check if time is dst -- yes",   
          pg_dst_calc::local_is_dst(pg_isdst1.date(),pg_isdst1.time_of_day())==pdalboost::date_time::is_in_dst);
    //backward shift boundary
    check("pg check if time is dst -- yes",   
          pg_dst_calc::local_is_dst(pg_isdst2.date(),pg_isdst2.time_of_day())==pdalboost::date_time::is_in_dst);
    check("pg check if time is dst -- ambig", 
          pg_dst_calc::local_is_dst(pg_amgbig1.date(),pg_amgbig1.time_of_day())==pdalboost::date_time::ambiguous);
    check("pg check if time is dst -- ambig", 
          pg_dst_calc::local_is_dst(pg_amgbig2.date(),pg_amgbig2.time_of_day())==pdalboost::date_time::ambiguous);
    check("pg check if time is dst -- not",   
          pg_dst_calc::local_is_dst(pg_notdst2.date(),pg_notdst2.time_of_day())==pdalboost::date_time::is_not_in_dst);
    // a couple not on the boudnary
    check("pg check if time is dst -- yes",   
          pg_dst_calc::local_is_dst(pg_indst,time_duration(0,0,0))==pdalboost::date_time::is_in_dst);
    check("pg check if time is dst -- not",   
          pg_dst_calc::local_is_dst(pg_notdst,time_duration(0,0,0))==pdalboost::date_time::is_not_in_dst);
...
//   //Define dst rule for Adelaide australia
  ...
  typedef pdalboost::date_time::acst_dst_trait<date> acst_dst_traits;
  typedef pdalboost::date_time::dst_calc_engine<date, time_duration, 
...
//               << std::endl;
    ...
    check("acst label on dst boundary invalid", 
          acst_dst_calc::local_is_dst(acst_invalid1.date(),acst_invalid1.time_of_day())==pdalboost::date_time::invalid_time_label);
    check("acst label on dst boundary invalid",   
          acst_dst_calc::local_is_dst(acst_invalid2.date(),acst_invalid2.time_of_day())==pdalboost::date_time::invalid_time_label);
    check("acst check if time is dst -- not",   
          acst_dst_calc::local_is_dst(acst_notdst1.date(),acst_notdst1.time_of_day())==pdalboost::date_time::is_not_in_dst);
    check("check if time is dst -- yes",   
          acst_dst_calc::local_is_dst(acst_isdst1.date(),acst_isdst1.time_of_day())==pdalboost::date_time::is_in_dst);
    //backward shift boundary
    check("acst check if time is dst -- yes",   
          acst_dst_calc::local_is_dst(acst_isdst2.date(),acst_isdst2.time_of_day())==pdalboost::date_time::is_in_dst);
    check("acst check if time is dst -- ambig", 
          acst_dst_calc::local_is_dst(acst_amgbig1.date(),acst_amgbig1.time_of_day())==pdalboost::date_time::ambiguous);
    check("acst check if time is dst -- ambig", 
          acst_dst_calc::local_is_dst(acst_amgbig2.date(),acst_amgbig2.time_of_day())==pdalboost::date_time::ambiguous);
    check("acst check if time is dst -- not",   
          acst_dst_calc::local_is_dst(acst_notdst2.date(),acst_notdst2.time_of_day())==pdalboost::date_time::is_not_in_dst);
    // a couple not on the boudnary
    check("acst check if time is dst -- yes",   
          acst_dst_calc::local_is_dst(acst_indst,time_duration(0,0,0))==pdalboost::date_time::is_in_dst);
    check("acst check if time is dst -- not",   
          acst_dst_calc::local_is_dst(acst_notdst,time_duration(0,0,0))==pdalboost::date_time::is_not_in_dst);
