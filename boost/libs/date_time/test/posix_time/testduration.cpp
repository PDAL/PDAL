...
//     check("default construction -- 0 min", td.minutes() == 0); 
  ...
  if (time_duration::resolution() >= pdalboost::date_time::micro) {
  ...
  if (time_duration::resolution() >= pdalboost::date_time::tenth) {
  ...
  if (time_duration::resolution() >= pdalboost::date_time::milli) {
    millisec ms(9);
...
#endif
    if (time_duration::resolution() == pdalboost::date_time::nano) {
  ...
  pdalboost::int64_t tms = static_cast<pdalboost::int64_t>(3600)*1000000*1001; //ms per sec
  check("total microseconds 1000 hours", hours(1001).total_microseconds() == tms);
  tms = static_cast<pdalboost::int64_t>(3600)*365*24*1000;
  check("total milliseconds - one year", (t2-t1).total_milliseconds() == tms);
  tms = 3600*365*24*static_cast<pdalboost::int64_t>(1000000000);
