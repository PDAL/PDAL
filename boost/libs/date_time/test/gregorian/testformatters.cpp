...
main() 
{
  ...
  pdalboost::gregorian::date d1(2002,01,02);
  std::string ds1 = pdalboost::gregorian::to_simple_string(d1);
  check("check string:     " + ds1, ds1 == "2002-Jan-02");
 
  std::string ids1(pdalboost::gregorian::to_iso_string(d1));
  //  std::cout << pdalboost::gregorian::to_iso_string(d1) << std::endl;
  check("check iso normal: " + ids1, ids1 == "20020102");

  std::string sds1 = pdalboost::gregorian::to_sql_string(d1);
  check("check sql string: "+sds1, sds1 == "2002-01-02");

  pdalboost::gregorian::date d2(2001,12,30);
  std::string ds2 = pdalboost::gregorian::to_simple_string(d2);
  check("check string:     "+ds2, ds2 == "2001-Dec-30");
  std::string ids2 = pdalboost::gregorian::to_iso_extended_string(d2);
