...
// Extended CCYY-MM-DD
  ...
  //bad lexical cast?
  try {
    pdalboost::gregorian::date d(pdalboost::gregorian::from_string(s));
...
/* not currently passing due to a bug in pdalboost::offset_separator (reported 2005-Aug-02)
  ...
  */      
    ...
    std::wstring w1_string(pdalboost::lexical_cast<std::wstring>("2000-Oct-31 is Halloween 2k!"));
    ...
    try {
      ...
      std::cout << "Shouldn't be reached." << 
        pdalboost::gregorian::to_simple_string(bd) << std::endl;
    }
    catch(pdalboost::gregorian::bad_month&){
  ...
  try {
    ...
    pdalboost::gregorian::date bad_day(pdalboost::gregorian::from_string(s2)); //won't construct
    ...
    std::cout << "Oh oh, this shouldn't be reached: "
              << pdalboost::gregorian::to_iso_string(bad_day) << std::endl;

  }
  catch(pdalboost::gregorian::bad_day_of_month&) { //expected
  ...
  try {
    ...
    pdalboost::gregorian::date bad_month(pdalboost::date_time::parse_date<pdalboost::gregorian::date>(s2));
    check("check bad month", false); //fail the test
    //The line below won't execute, but make the compiler think
    //we are using bad day....
    std::cout << "Oh oh, this shouldn't be reached: "
              << pdalboost::gregorian::to_iso_string(bad_month) << std::endl;

  }
  catch(pdalboost::gregorian::bad_month&) { //expected
