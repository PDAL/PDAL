...
void test_month_decrement_iterator(const pdalboost::gregorian::date *Answers, int array_len){
  using namespace pdalboost::gregorian;
  typedef pdalboost::date_time::month_functor<date> mfg;
    
  pdalboost::date_time::date_itr<mfg, date> ditr(Answers[array_len-1]);
  int i = array_len-1;
...
void test_base_iterator(pdalboost::gregorian::date end,
                        pdalboost::gregorian::date_iterator& di,
                        std::string& data)
{
  using namespace pdalboost::gregorian;
  for (; di < end; ++di) {
...
main() 
{
  ...
  typedef pdalboost::date_time::day_functor<date> dfg;
  
  {
    const date DayAnswers[] = {date(2000,Jan,20),date(2000,Jan,22),date(2000,Jan,24)};
    pdalboost::date_time::date_itr<dfg,date> ditr(date(2000,Jan,20),2);
    int i=0;
  ...
  typedef pdalboost::date_time::week_functor<date> wfg;
  {
    const date WeekAnswers[] = {date(2000,Jan,20),date(2000,Jan,27),date(2000,Feb,3)};
    pdalboost::date_time::date_itr<wfg, date> ditr(date(2000,Jan,20));
    int i=0;
...

  {
    ...
    pdalboost::date_time::date_itr<wfg, date> ditr(date(2000,Jan,20),2);
    int i=0;
    for (; ditr < date(2000,Feb,6); ++ditr) {
      //std::cout << *ditr << " ";
      check("week iterator", WeekAnswers[i] == *ditr);
  ...
  typedef pdalboost::date_time::month_functor<date> mfg;
  {
    ...
    pdalboost::date_time::date_itr<mfg, date> ditr(date(2000,Jan,1));
    int i = 0;
    try { 
...

  {
    ...
    pdalboost::date_time::date_itr<mfg, date> ditr(date(2000,Jan,31));
    int i = 0;
    try { 
...

  {
    ...
    pdalboost::date_time::date_itr<mfg, date> ditr(date(2000,Feb,29));
    int i = 0;
    try { 
  ...
  { // running a span of 5 years to verify snap to end doesn't occur at next leap year
    ...
    pdalboost::date_time::date_itr<mfg, date> ditr(date(2000,Feb,28));
    int i = 0;
    try { 
  ...
  typedef pdalboost::date_time::year_functor<date> yfg;
  {
    ...
    pdalboost::date_time::date_itr<yfg, date> d3(date(2000,Jan,1));
    int i = 0;
  ...
  { // WON'T snap top end of month
    ...
    pdalboost::date_time::date_itr<yfg, date> d3(date(2000,Feb,28));
    int i = 0;
  ...
  {// WILL snap top end of month
    ...
    pdalboost::date_time::date_itr<yfg, date> d3(date(2000,Feb,29));
    int i = 0;
    for (; d3 < date(2010,Mar,1); ++d3) {
      //std::cout << *d3 << " ";
      check("year iterator: " + to_iso_string(*d3), YearAnswers[i] == *d3);
...

  {
    ...
    pdalboost::date_time::date_itr<yfg, date> d3(date(2000,Jan,1),2);
    int i = 0;
    for (; d3 < date(2010,Jan,2); ++d3) {
      //std::cout << *d3 << " ";
      check("year iterator: " + to_iso_string(*d3), YearAnswers[i] == *d3);
