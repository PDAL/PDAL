...
To run this tutorial, we need to create a new {ref}`Conda environment<conda>` with PDAL installed.

### Environment setup

1. Make and activate a new environment with PDAL installed. We're going to call ours `tindex`
...
be using them from the cloud in an S3 bucket location. For the "Creating a New
Index" section, you can download the list of S3 URLs
[here](https://s3.amazonaws.com/hobu-lidar/WI_Adams_2019/filelist.txt)


```{note}
If you want to skip the creation of the tile index, move to the ["Merging the
Index" section.](#merging-the-index)
```

## Creating an Index

The `pdal tindex create` command uses [OGR] to write a new vector dataset, with
separate polygon features for each file. The features can be highly detailed 2D
...
`tindex create` can take input from
[stdin](https://en.wikipedia.org/wiki/Standard_streams) (via the ``-s`` flag)
...
In our case, since we're fetching from URLs (which are non-globbable) we will
specify stdin with a specific list of filenames:
```
curl https://s3.amazonaws.com/hobu-lidar/WI_Adams_2019/filelist.txt | pdal tindex create WI_Adams.parquet -f Parquet -s
```
...
```
```{note}
PDAL 2.9 introduced support for parallel creation of boundaries in
`tindex create` with the `--threads` option. Consider that many
