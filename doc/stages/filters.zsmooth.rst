...
All points within some radius in the X-Y plane are ordered by Z and the point's new
Z value is chosen to be that of the Nth median value, where N is specified as the
_`medianpercent` option.
...
Compute the smoothed Z value as the median Z value of the neighbors withing 2 units and
...
radius
  Radius in the X/Y plane from specifying the points to select to determine the smoothed
  Z value. [Default: 1]

medianpercent
  A value between 0 and 100 that specifies the position in the ordered Z values of neighbors
